...
test("homechart.app", async () => {
	...
	if (process.env.PUPPETEER_SCREENSHOT !== undefined) {
	...
	for (const device of devices) {
		...
		await page.goto(`${process.env.PUPPETEER_URL_HOMECHART === undefined || process.env.PUPPETEER_URL_HOMECHART === "" ?
			"http://localhost" :
			process.env.PUPPETEER_URL_HOMECHART}`);
