...
const username = `e2e-test-${Math.random()
	.toString(36)
	.substring(7)}@${process.env.HOMECHART_APP_NOEMAILDOMAINS === undefined ?
	"example.com" :
...
const utilities = {
	...
	openMenu: async (page: Page, component: string, view?: string, item?: string): Promise<void> => {
	...
	screenshot: async (page: Page, waitForElement: string, view: string): Promise<void> => {
		console.log(`Screenshot ${view}${append}`); // eslint-disable-line

		await utilities.waitFor(page, waitForElement)
			.catch(async (err) => {
	...
	visible: async (page: Page, selector: string): Promise<boolean> => {
		return page.$eval(selector, (e) => {
			return e !== undefined && window.getComputedStyle(e)
