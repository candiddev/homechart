...
const views = {
	...
	budgetTransactions: async (page: Page): Promise<void> => {
		...
		await utilities.screenshot(page, "#table-container-transactions", "budget_transactions");
	},
	...
	planTasks: async (page: Page): Promise<void> => {
		...
		if (utilities.modeScreenshot()) {
			await page.evaluate(() => {
				(document.querySelector("#tab-household") as HTMLElement)
					.click();
			});
			await utilities.screenshot(page, ".PlanTasks__name--header", "plan_household");
		} else {
			await page.evaluate(() => {
				(document.querySelector("#tab-personal") as HTMLElement)
					.click();
			});
			await utilities.screenshot(page, ".PlanTasks__name--header", "plan_personal");
	...
	secretsValue: async (page: Page): Promise<void> => {
		if (utilities.modeScreenshot()) {
			...
			await utilities.input(page, "#form-item-input-properties-key-1", "Username");
			await utilities.input(page, "#form-item-input-properties-value-1", "user@example.com");
			await utilities.click(page, "#button-add-value");
