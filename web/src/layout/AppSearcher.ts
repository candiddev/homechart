...
const searchables = [
	...
	},
	{
		...
		linkFormatter: (data: RewardCard): string => {
			if (data.senders.includes(AuthAccountState.data().id!)) { // eslint-disable-line @typescript-eslint/no-non-null-assertion
	...
	},
	{
		...
		linkFormatter: (data: SecretsValueValues): string => {
			return `/secrets/${data.secretsVaultID}`;
		},
	},
	{
...
export function AppSearcher (search: string): DropdownMenuAttrsItem[] {
	...
	for (const searchable of searchables) {
		for (let i = 0; i < searchable.data().length; i++) {
			if ((searchable.data()[i].name as string).toLowerCase()
				.includes(search.toLowerCase())) {
				items.push({
					href: searchable.linkFormatter === undefined ?
						searchable.link :
