...
export async function Handler(msg: WorkerMessage): Promise<void> {
  switch (msg.action) {
    case WorkerAction.FilterSortChildren:
      return FilterSortChildrenWkr(msg as WorkerMessageFilterSortChildren).then(
        (res) => {
          ctx.postMessage(res);
        },
      );
    case WorkerAction.Read:
      return ReadWkr(msg as WorkerMessageRead).then((res) => {
        ctx.postMessage(res);
      });
    case WorkerAction.ReadAll:
      return ReadAllWkr(msg as WorkerMessageReadAll).then((res) => {
        ctx.postMessage(res);
      });
