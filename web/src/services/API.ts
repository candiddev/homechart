  ...
  fetcher = async (
    ...
    return new Promise((resolve, reject): void | Err => {
      const request: XHRRequest = {
        ...
        path: path.toString(), // eslint-disable-line
      ...
      if (testing.mocks.responses.length > 0) {
        const response = {
          // eslint-disable-line
          dataHash:
            testing.mocks.responses[0].dataHash === undefined
              ? ""
              : testing.mocks.responses[0].dataHash,
          dataIDs:
            testing.mocks.responses[0].dataIDs === undefined
              ? []
              : testing.mocks.responses[0].dataIDs,
          dataTotal:
            testing.mocks.responses[0].dataTotal === undefined
              ? 0
              : testing.mocks.responses[0].dataTotal,
          dataType:
            testing.mocks.responses[0].dataType === undefined
              ? ""
              : testing.mocks.responses[0].dataType,
          dataValue:
            testing.mocks.responses[0].dataValue === undefined
              ? []
              : [...testing.mocks.responses[0].dataValue],
          message:
            testing.mocks.responses[0].message === undefined
              ? "Mocked"
              : testing.mocks.responses[0].message,
          requestID: "",
          status:
            testing.mocks.responses[0].status === undefined
              ? 200
        ...
        const resp = new Response(
          response.status === 204 ? null : JSON.stringify(response),
          {
      ...
      if (!path.toString().includes("/this/should/fail")) {
        NewErr(
          `API.fetcher: Invalid request\n${JSON.stringify(request)}`,
          "Invalid request",
        );
      }
      ...
      reject(
        new Response(null, {
          status: path.toString().includes("fail0") ? 0 : 500, // eslint-disable-line
        }),
      );
    });
  };
...
export const API = {
  ...
  fetch: async (
    ...
    if (apiEndpoint().id === "") {
      await IndexedDB.get("APIAuth").then((data) => {
        if (data !== null) {
    ...
    return (
      fetcher(`${await API.getHostname()}${path}`, {
        ...
        .then(async (response: Response) => {
          if (response.status === 204) {
            Log.debug(
              `API.fetch ${method} ${path} returned ${response.status}`,
            );
          ...
          return response.json().then((json) => {
            ...
            if (IsAPIResponse(json)) {
              Log.debug(
                `API.fetch ${method} ${path} returned:\n${JSON.stringify(json)}`,
              );
              return json;
            }
        ...
        .catch((err: any) => {
          ...
          if (path !== "/this/should/fail") {
            NewErr(
              `API.fetch: unknown response for ${path}:\n${JSON.stringify(err)}`,
  ...
  getHostname: async (): Promise<string> => {
    if (apiEndpoint().hostname === null) {
      await IndexedDB.get("APIHostname").then(async (data) => {
        if (data !== null) {
