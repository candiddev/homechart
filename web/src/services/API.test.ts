...
describe("API", () => {
	...
	test("fetch", async () => {
		...
		expect(await API.fetch("GET", "/api/v1", {}))
			.toStrictEqual({
				...APIResponse(),
				message: "Mocked",
				status: 200,
		...
		expect(await API.fetch("POST", "/api/v1", {
			body: {
				id: "1",
			},
			updated: "today",
		...
		await expect(API.fetch("GET", "/this/should/fail", {}))
		...
		await expect(API.fetch("GET", "/this/should/fail0", {}))
			.resolves.toStrictEqual({
				...APIResponse(),
	...
	test("get/setHostname", async () => {
		...
		await expect(IndexedDB.get("APIHostname"))
			.resolves.toBe(null);
	});
	test("setAuth", async () => {
		API.clearAPIEndpoint();
