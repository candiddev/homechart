...
import { ActionAdd, ActionCancel, ActionDelete, ActionEdit, ActionNew } from "@lib/yaml8n";
...
import { ObjectCook, ObjectNote, ObjectNotes, ObjectRecipe, ObjectRecipeNote, ObjectRecipes, WebCookRecipesIDCookTime, WebCookRecipesIDCookTimeTooltip, WebCookRecipesIDCopyRecipe, WebCookRecipesIDDirections, WebCookRecipesIDDirectionsTooltip, WebCookRecipesIDIngredientsToShoppingList, WebCookRecipesIDNewRecipeName, WebCookRecipesIDNoteTooltip, WebCookRecipesIDPrepTime, WebCookRecipesIDPrepTimeTooltip, WebCookRecipesIDPublic, WebCookRecipesIDServingsTooltip, WebCookRecipesIDSource, WebCookRecipesIDSourceTooltip, WebGlobalActionDeleteNow, WebGlobalActionImportRecipeFromWebsite, WebGlobalActionRestore, WebGlobalActionSave, WebGlobalDeleted, WebGlobalImage, WebGlobalName, WebGlobalNameTooltip, WebGlobalNever, WebGlobalRecipeComplexity, WebGlobalRecipeComplexityTooltip, WebGlobalRecipeIngredients, WebGlobalRecipeIngredientsTooltip, WebGlobalRecipeLastMade, WebGlobalRecipeRating, WebGlobalRecipeRatingTooltip, WebGlobalRecipeScale, WebGlobalRecipeScaleTooltip, WebGlobalRecipeServings, WebGlobalRecipeTimesMade, WebGlobalTags, WebGlobalTagsTooltip, WebGlobalURL } from "../yaml8n";
...
export function CookRecipesID (): m.Component {
	...
	function setBreadcrumbs (): void {
		AppState.setLayoutApp({
			...
			breadcrumbs: [
				{
					link: AppState.isSessionAuthenticated() ?
						"/cook/recipes" :
						"/signin",
			...
			toolbarActionButtons: [
				{
					icon: Icons.ShopItem,
					name: AuthAccountState.translate(WebCookRecipesIDIngredientsToShoppingList),
					onclick: (): void => {
				...
				AppToolbarActions().newCookRecipe,
				{
					...
					onclick: async (): Promise<void> => {
						return new Promise((resolve) => {
							state.edit(true);
							m.route.set(`/cook/recipes/${state.cookRecipe.id}?copy&edit`, {}, {
								state: {
									key: Date.now(),
								},
							});
					...
					permitted: GlobalState.permitted(PermissionComponentsEnum.Cook, true) && m.route.param().id !== "new",
					requireOnline: true,
				},
				{
					...
					permitted: GlobalState.permitted(PermissionComponentsEnum.Cook, true, state.cookRecipe.authHouseholdID),
					requireOnline: true,
				},
			],
		});
	...
	return {
		oninit: (): void => {
			state.lift = Stream.lift(async (_cookRecipeState, edit, id) => {
				...
				setBreadcrumbs();
			}, CookRecipeState.data, state.edit, state.id);
			...
			if (m.route.param().scale !== undefined && m.route.param().scale !== "0") {
			...
			if (id === "new") {
				...
				state.cookRecipe.name = AuthAccountState.translate(WebCookRecipesIDNewRecipeName);
		...
		view: (): m.Children => {
			return [
				AppState.isSessionAuthenticated() && Permission.isPermitted(AuthSessionState.data().permissionsHouseholds, PermissionComponentsEnum.Cook, PermissionEnum.View, state.cookRecipe.authHouseholdID) ?
					[
						m(FormImportRecipe, {
							toggle: (v) => {
								state.formVisible = v ?
				...
				m(Form, {
					...
					title: {
						buttonLeft: {
							...
							onclick: async (): Promise<void> => {
								if (state.edit()) {
									...
									recipe.deleted = Timestamp.now()
										.toString();
									return CookRecipeState.update(recipe)
										.then(() => {
											m.route.set("/cook/recipes");
										});
								}
								return CookRecipeState.delete(m.route.param().id)
									.then(() => {
										m.route.set("/cook/recipes");
									});
							},
						...
						buttonRight: {
							icon: state.cookRecipe.deleted === null ?
								state.edit() ?
									Icons.Save :
									Icons.Edit :
								Icons.Restore,
							name: state.cookRecipe.deleted === null ?
								state.edit() ?
							...
							onclick: async (): Promise<void | Err> => {
								if (state.cookRecipe.deleted !== null) {
									state.cookRecipe.deleted = null;
									return CookRecipeState.update(state.cookRecipe)
										.then(() => {
											state.cookRecipe = CookRecipeState.findID(state.cookRecipe.id);
										});
								} else if (state.edit()) {
									if (state.cookRecipe.id === null) {
										return CookRecipeState.create(state.cookRecipe)
											.then((recipe) => {
												if (IsErr(recipe)) {
													return recipe;
												}
								...
								state.edit(true);
								m.route.set(`/cook/recipes/${state.cookRecipe.id}?edit`, {}, {
									state: {
										key: Date.now(),
									},
								});
							...
							permitted: state.cookRecipe.id === null || GlobalState.permitted(PermissionComponentsEnum.Cook, true, state.cookRecipe.authHouseholdID),
							requireOnline: true,
						},
						name: state.loaded ?
							state.cookRecipe.name :
					...
					m("div.CookRecipesID__heading", [
						...
						m(FormItemDuration, {
							...
							tooltip: AuthAccountState.translate(WebCookRecipesIDPrepTimeTooltip),
						}),
						m(FormItemDuration, {
							disabled: !state.edit(),
							getDuration: (): number => {
							...
							tooltip: AuthAccountState.translate(WebCookRecipesIDCookTimeTooltip),
						}),
						m(FormItem, {
							input: {
						...
						m(FormItem, {
							...
							tooltip: AuthAccountState.translate(WebCookRecipesIDSourceTooltip),
						}),
						state.cookRecipe.deleted === null ?
							[] :
							m(FormItem, {
					...
					state.edit() ?
						...
						m(FormItemIcons, {
							...
							tooltip: AuthAccountState.translate(WebGlobalRecipeComplexityTooltip),
					...
					state.edit() ?
						...
						m(FormItemIcons, {
							...
							tooltip: AuthAccountState.translate(WebGlobalRecipeRatingTooltip),
					...
					m(FormItem, {
						input: {
							...
							type: "text",
							value: CookRecipeState.scaleIngredient(state.cookRecipe.servings, state.scale),
						},
						name: AuthAccountState.translate(WebGlobalRecipeServings),
						tooltip: AuthAccountState.translate(WebCookRecipesIDServingsTooltip),
					}),
					state.edit() ?
						m(FormCheckbox, {
						...
						m(FormItem, {
							...
							tooltip: AuthAccountState.translate(WebGlobalRecipeScaleTooltip),
						}),
					m(FormItem, {
						autocompleteOptions: [
							`${AuthAccountState.translate(ObjectCook)} > ${ObjectRecipe}`,
						],
						...
						textArea: {
							...
							value: state.edit() ?
								...
								CookRecipeState.scaleIngredients(state.cookRecipe.ingredients, state.scale),
						},
						tooltip: AuthAccountState.translate(WebGlobalRecipeIngredientsTooltip),
					}),
					m(FormItem, {
						autocompleteOptions: [
							`${AuthAccountState.translate(ObjectCook)} > ${ObjectRecipe}`,
					...
					state.cookRecipe.public ?
						m(FormItem, {
							input: {
								...
								oninput: () => {},
								type: "text",
								value: `${apiEndpoint().hostname === "" ?
									window.location.origin :
									apiEndpoint().hostname}${window.location.pathname}`,
							},
					...
					m("div.CookRecipesID__notes#notes", [
						m("div", [
							...
							m(Button, {
								...
								permitted: GlobalState.permitted(PermissionComponentsEnum.Cook, true) && state.cookRecipe.id !== null,
								requireOnline: true,
							}),
						]),
						m("div", [
							state.cookRecipe.notes !== undefined && state.cookRecipe.notes !== null ?
								state.cookRecipe.notes.map((_note, index) => {
									return m("div", [
										m(Title, {
											buttonLeft: {
												icon: state.editLog === state.cookRecipe.notes[index].date
													?	Icons.Cancel :
													Icons.Delete,
												name: state.editLog === state.cookRecipe.notes[index].date ?
													AuthAccountState.translate(ActionCancel) :
													AuthAccountState.translate(ActionDelete),
												onclick: async (): Promise<void> => {
													if (state.editLog === state.cookRecipe.notes[index].date) {
														state.editLog = null;
														return;
													}
											...
											name: CivilDate.fromString(state.cookRecipe.notes[index].date!) // eslint-disable-line @typescript-eslint/no-non-null-assertion
												.toString(AuthAccountState.data().preferences.formatDateOrder, AuthAccountState.data().preferences.formatDateSeparator),
										}),
										m("div.CookRecipesID__wrap", [
											m("div.CookRecipesID__wrap", [
												m(FormItemIcons, {
													disabled: state.editLog !== state.cookRecipe.notes[index].date,
													iconSelect: Icons.RatingSelect,
													iconUnselect: Icons.RatingUnselect,
													idPostfix: `${index}`,
													max: 5,
											...
											m("div.CookRecipesID__wrap", [
												m(FormItem, {
													...
													tooltip: AuthAccountState.translate(WebCookRecipesIDNoteTooltip),
												}),
											]),
										]),
									]);
