...
import { ObjectBudget, ObjectCategory, ObjectItems, ObjectLists, ObjectShop, WebGlobalName, WebGlobalPersonal } from "../yaml8n";
...
export function ShopLists (): m.Component {
	...
	return {
		oninit: async (): Promise<void> => {
			...
			data = Stream.lift((lists, columns, sort) => {
				m.redraw();

				return Filter.array(lists, columns, sort);
			}, ShopListState.data, state.columns, state.sort);
