...
import { ObjectItems, ObjectShop, ObjectStores, WebFormOverlayBudgetPayeeAddress, WebGlobalName } from "../yaml8n";
...
export function ShopStores (): m.Component {
	...
	return {
		oninit: async (): Promise<void> => {
			...
			data = Stream.lift((payees, columns, sort) => {
				return Filter.array(payees.filter((payee) => {
			...
			AppState.setLayoutApp({
				...
				toolbarActionButtons: [
					AppToolbarActions().newShopStore,
				],
			});
