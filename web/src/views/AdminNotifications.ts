...
import {
  ...
  WebAdminNotificationsSendAfter,
  WebAdminNotificationsSubject,
  WebAdminNotificationsTo,
  WebGlobalAdmin,
} from "../yaml8n";
...
export function AdminNotifications(): m.Component {
  ...
  async function update(): Promise<void> {
    return NotificationState.read().then((notifications) => {
              ...
              Form,
              {
                buttons: [
                  {
                    ...
                    permitted:
                      !state.form.deleteConfirm && state.form.data.id !== null,
                    requireOnline: true,
                  },
                  {
                  ...
                  },
                  {
                    ...
                    onclick: async (): Promise<void> => {
                      return NotificationState.delete(state.form.data.id).then(
                        async () => {
                          state.form.visible = false;
                          return update();
                        },
                ...
                onsubmit: async () => {
                  if (state.form.data.id === null) {
                    return NotificationState.create(state.form.data).then(
                      async () => {
                        state.form.visible = false;
                        return update();
                      },
                  ...
                  return NotificationState.update(state.form.data).then(
                    async () => {
                      state.form.visible = false;
                      return update();
                    },
                  ...
                  : m(FormItem, {
                      input: {
                        oninput: (e: string): void => {
                          state.form.data.authAccountID = e === "" ? null : e;
                        },
                        type: "text",
                        value:
                          state.form.data.authAccountID === null
                  ...
                  input: {
                    ...
                    value: state.form.data.newsletter
                      ? Timestamp.fromString(
                          state.form.data.sendAfter as string,
                        ).toPrettyString(
                      ...
                      : state.form.data.sendAfter === "" ||
                          state.form.data.sendAfter === null
                        ? ""
                        : Timestamp.fromString(
                            state.form.data.sendAfter,
                          ).toHTMLDate(),
