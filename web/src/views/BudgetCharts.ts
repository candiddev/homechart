...
import type { ChartCanvasAttrs, ChartCanvasData } from "@lib/components/ChartCanvas";
...
import { ChartDatasetState, ChartDatasetTypesEnum } from "../states/ChartDataset";
import { GetHelp } from "../utilities/GetHelp";
import { ObjectBudget, ObjectBudgetCategoryGroupingIncome, ObjectCategory, ObjectChange, ObjectPayee, WebBudgetChartsDateRangeFrom, WebBudgetChartsDateRangeFromTooltip, WebBudgetChartsDateRangeTo, WebBudgetChartsDateRangeToTooltip, WebBudgetChartsExpense, WebBudgetChartsFilterChart, WebBudgetChartsLastThreeMonths, WebBudgetChartsThisMonth, WebBudgetChartsTotal, WebBudgetChartsUseGroupings, WebBudgetChartsYearToDate, WebGlobalBudgetAmount, WebGlobalBudgetCharts, WebGlobalBudgetChartsIncomeExpense, WebGlobalBudgetChartsSpendingByCategory, WebGlobalBudgetChartsSpendingByPayee, WebGlobalBudgetMonth, WebGlobalCustom } from "../yaml8n";

function filterChartDataset (chartDataset: ChartDataset[], filters: string[]): ChartDataset[] {
...
export function BudgetCharts (): m.Component {
	const thisMonth = CivilDate.now()
		.toYearMonth();

	const state = {
		authHousehold: Clone(AuthHouseholdState.findID(AuthAccountState.data().primaryAuthHouseholdID)),
			...
			plugins: {
				datalabels: {
					...
					formatter: (_value, context) => {
						return (context.chart.data.labels![context.dataIndex] as string).split(" "); // eslint-disable-line @typescript-eslint/no-non-null-assertion
					},
					padding: 6,
				},
				tooltip: {
					callbacks: {
						label: (context) => {
							...
							if (context.parsed.y === undefined) {
								label += Currency.toString(context.parsed, state.authHousehold.preferences.currency);
							} else {
								label += Currency.toString(context.parsed.y, state.authHousehold.preferences.currency);
							}
	...
	Stream.lift(async (dateRange, chartType, columns, rollup, sort) => {
		switch (chartType) {
		case "categories":
		case "payees":
			config.type = ChartTypesEnum.Doughnut;
			const payeeDataset = await ChartDatasetState.read(state.authHousehold.id, dateRange, chartType === "categories" ?
				rollup ?
		...
		case "income-expense":
			...
			config.options!.plugins!.datalabels = { // eslint-disable-line @typescript-eslint/no-non-null-assertion
				display: false,
			};
			config.options!.scales = { // eslint-disable-line @typescript-eslint/no-non-null-assertion
				y: {
					ticks: {
						callback: (value): string => {
							if (typeof value === "number") {
								return Currency.toString(value, state.authHousehold.preferences.currency);
							}
			...
			const data = await ChartDatasetState.read(state.authHousehold.id, dateRange, ChartDatasetTypesEnum.BudgetIncomeExpense);
			if (IsErr(data)) {
				return;
			}
			...
			if (state.chartDataset().length > 0) {
				...
				config.data({
					datasets: [
						...
						{
							backgroundColor: `${getComputedStyle(app)
								.getPropertyValue("--color_primary")}`,
							data: data.map((d) => {
								return d.values[1].value;
							}),
							label: AuthAccountState.translate(ObjectBudgetCategoryGroupingIncome),
							type: "bar",
						},
						{
							backgroundColor: `${getComputedStyle(app)
		...
		m.redraw();
	}, dateRange, state.chartType, state.columns, state.rollup, state.sort);
	...
	return {
		oninit: async (): Promise<void> => {
			...
			if (m.route.param().household === undefined) {
				...
				state.authHousehold = AuthHouseholdState.findID(m.route.param().household);
			}
			...
			if (state.chartType() !== "income-expense") {
				Stream.lift((data, filters, columns, sort) => {
					if (data.length > 0) {
					...
					m.redraw();
				}, state.chartDataset, state.chartFilters, state.columns, state.sort);
			}
			...
			return m(Form, {
				...
				m("div.BudgetCharts__chart", [
					m("div.BudgetCharts__date-ranges", [
						m("span", {
							class: dateRange() === dateRangeThisMonth ?
						...
						m("span", {
							class: dateRange() === dateRangeLastThreeMonths ?
								"BudgetCharts__date-range--active" :
								undefined,
						...
						m("span", {
							class: dateRange() === dateRangeYearToDate ?
								"BudgetCharts__date-range--active" :
								undefined,
						...
						state.chartType() === "categories" ?
							m("span", {
								...
								onclick: () => {
									...
									const el = document.getElementById(`form-item-select-filter-${state.chartType()}`) as HTMLSelectElement; // eslint-disable-line @typescript-eslint/consistent-type-assertions
									el.selectedIndex = -1;
									state.chartFilters([]);
								},
					...
					state.customDateRange ?
						[
							m(FormItem, {
								...
								name: AuthAccountState.translate(WebBudgetChartsDateRangeFrom),
								tooltip: AuthAccountState.translate(WebBudgetChartsDateRangeFromTooltip),
							}),
							m(FormItem, {
								input: {
									oninput: (e: string) => {
								...
								name: AuthAccountState.translate(WebBudgetChartsDateRangeTo),
								tooltip: AuthAccountState.translate(WebBudgetChartsDateRangeToTooltip),
							}),
						] :
						[],
					state.chartType() === "income-expense" ?
						...
						m(FormItem, {
							buttonArray: {
								...
								value: state.chartType() === "categories" ?
									state.rollup() ?
					...
					state.loaded ?
						config.data() === undefined || config.data()!.labels!.length === 0 ? // eslint-disable-line @typescript-eslint/no-non-null-assertion
							m("span.BudgetCharts__none", "No transactions found") :
							m(ChartCanvas, config) :
						m(`span.BudgetCharts__loading.${Animate.class(Animation.Pulse)}`),
				]),
				m(Table, {
					actions: [],
					class: "BudgetCharts__table",
					data: state.chartType() === "income-expense" ?
						state.chartDataset() :
						state.chartDatasetFiltered(),
					filters: [],
					...
					tableColumns: [
						{
							...
							name: state.chartType() === "categories" ?
