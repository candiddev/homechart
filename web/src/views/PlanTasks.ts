...
import {
  PlanTaskChildrenStatus,
  PlanTaskState,
  PlanTaskTemplateType,
} from "../states/PlanTask";
...
import {
  ...
  WebPlanTasksNone,
  WebPlanTasksOverdue,
  WebPlanTasksTomorrow,
} from "../yaml8n";

const thisWeekTime = Timestamp.now();
thisWeekTime.addDays(7);
const thisWeek = thisWeekTime.toCivilDate();
...
function onDrag(srcID: string, x: number, y: number): void {
  ...
  if (
    elNew !== null &&
    (elNew.classList.contains("PlanTasks__item") ||
      elNew.classList.contains("PlanTasks__list")) &&
    !elNew.parentElement!.classList.contains("PlanTasks")
  ) {
    ...
    if (elNew.id === srcID) {
      ...
      if (
        Drag.state.target === "" ||
        Drag.state.target.includes(srcID) ||
        Drag.state.target.includes("list")
      ) {
        return;
      }
    ...
    if (
      srcID.includes("task") ||
      (srcID.includes("project") && newType.includes("project"))
    ) {
      ...
      if (
        (srcID.includes("task") && newType.includes("list")) ||
        (srcID.includes("project") && newType === "projectlist")
      ) {
        ...
        elNew.nextElementSibling !== null &&
        elNew.nextElementSibling.tagName === "UL"
      ) {
...
async function onDragEndProject(project: PlanProject): Promise<void | Err> {
  ...
  if (Drag.state.target !== "") {
    ...
    switch (type) {
      case "project":
        ...
        updateProject.position = Position.adjacent(
          projectAfter,
          PlanProjectState.findAdjacent(projectAfter),
          Drag.state.before,
        );
      ...
      case "projectlist":
        ...
        if (
          projectParent.children === undefined ||
          projectParent.children.length === 0
        ) {
          updateProject.position = "0";
        } else {
          updateProject.position = Position.increase(
            projectParent.children[projectParent.children.length - 1].position,
            true,
          );
        }
        break;
  ...
  return PlanProjectState.update(updateProject).then(async (err) => {
    if (IsErr(err)) {
    ...
    if (updateProject.parentID === project.parentID) {
      // If the element moves out of its list, remove it
...
async function onDragEndTask(task: PlanTask): Promise<void | Err> {
  ...
  if (Drag.state.target !== "") {
    ...
    switch (type) {
      ...
      case "projectlist":
        ...
        if (tasks.length > 0) {
          updateTask.position = Position.increase(
      ...
      case "task":
        ...
        updateTask.position = Position.adjacent(
          taskAfter,
          PlanTaskState.findAdjacent(taskAfter),
          Drag.state.before,
        );
      ...
      case "tasklist":
        ...
        if (
          taskParent.children === undefined ||
          taskParent.children.length === 0
        ) {
          updateTask.position = "0";
        } else {
          updateTask.position = Position.increase(
            taskParent.children[taskParent.children.length - 1].position,
            true,
          );
        }
        break;
  ...
  return PlanTaskState.update(updateTask).then(async (err) => {
    if (IsErr(err)) {
      if (Drag.state.parent !== null) {
        Drag.state.parent.insertBefore(
          el,
    ...
    if (
      ...
      updateTask.planProjectID !== task.planProjectID
    ) {
      // If the element moves out of its list, remove it
      el.remove();
    } else {
    ...
    if (AuthAccountState.data().collapsedPlanTasks.includes(task.id!)) {
      // eslint-disable-line @typescript-eslint/no-non-null-assertion
...
async function onDragStart(
  ...
  if (
    projectID !== null &&
    !AuthAccountState.data().collapsedPlanProjects.includes(projectID)
  ) {
    return AuthAccountState.collapsePlanProject(projectID);
  } else if (
    taskID !== null &&
    !AuthAccountState.data().collapsedPlanProjects.includes(taskID)
  ) {
...
function RenderObjects(): m.Component<{
  ...
  return {
    view: (vnode): m.Children => {
      return vnode.attrs.items.length === 0 &&
        vnode.attrs.noHeaders === true &&
        vnode.attrs.tasks.length === 0
        ? m("div.PlanTasks__none", [
            ...
            AppState.data.layoutAppToolbarActionButtons.map((action) => {
              if (
                (action.permitted && AppState.isSessionOnline()) ||
        ...
        : [
            vnode.attrs.items.map((item) => {
              return m(
                ...
                [
                  m(
                    "div.PlanTasks__item.PlanTasks__item--item.PlanTasks__item--info",
                    {
                      onclick: () => {
                    ...
                    [
                      m(Icon, {
                        classes:
                          "PlanTasks__button PlanTasks__button--drag PlanTask__button--hidden",
                        icon: Icons.Drag,
                        style: {
                          visibility: "hidden",
                        },
                      ...
                      GlobalState.permitted(
                        ...
                        true,
                        item.authHouseholdID,
                      )
                        ? m(Icon, {
                            classes:
                              "PlanTasks__button PlanTasks__button--checkbox",
                            icon: item.inCart
                        ...
                        [
                          ...
                          m("div.PlanTasks__details", [
                            item.price === 0
                              ? []
                              : m(
                                  "span.ButtonArray__item.ButtonArray__item--small",
                                  [
                                    m(
                                      "span",
                                      Currency.toString(
                            ...
                            item.budgetPayeeID === null
                              ...
                              : m(
                                  ...
                                  {
                                    class:
                                      "ButtonArray__item ButtonArray__item--small",
                              ...
                              [
                                ...
                                m(
                                  ...
                                  item.shopListID === null
                                    ...
                                    : ShopListState.findID(item.shopListID)
                                        .name,
                                ),
                              ],
                            ),
            ...
            vnode.attrs.tasks.map((task) => {
              if (
                (!state.showComplete() && task.done) ||
                (state.showComplete() &&
                  !task.done &&
                  PlanTaskState.getChildrenDone(task) ===
                    PlanTaskChildrenStatus.none)
              ) {
                return m(
                  "div",
                  {
              ...
              return m(
                `li.${Animate.class(Animation.Fade)}`,
                {
                  id: `task_${task.id}`,
                  key: `task_${task.id}`,
                ...
                [
                  m(
                    "div.PlanTasks__item.PlanTasks__item--task",
                    {
                      onclick: () => {
                        if (
                          task.authAccountID === null &&
                          task.authHouseholdID === null &&
                          !AppState.isSessionAdmin()
                        ) {
                          return;
                        }
                    ...
                    [
                      m("div.PlanTasks__item--info", [
                        ...
                        (task.authAccountID !== null ||
                          ...
                          ? m(Icon, {
                              classes:
                                "PlanTasks__button PlanTasks__button--drag",
                              draggable: true,
                              icon: Icons.Drag,
                        ...
                        GlobalState.permitted(
                          ...
                          true,
                          task.authHouseholdID,
                        )
                          ? m(Icon, {
                              classes:
                                "PlanTasks__button PlanTasks__button--checkbox",
                              icon: task.done ? Icons.CheckYes : Icons.CheckNo,
                              onclick: async (e: m.Event<MouseEvent>) => {
                                e.stopPropagation();
                                return PlanTaskState.updateDone(task).then(
                                  async (err) => {
                                    if (IsErr(err)) {
                                      return;
                                    }
                              ...
                              style: {
                                ...
                                display: task.template ? "none" : undefined,
                              },
                            })
                          : [],
                        m(
                              ...
                              task.dueDate === null
                                ...
                                : m(
                                    m.route.Link,
                                    {
                                      class:
                                        "ButtonArray__item ButtonArray__item--small",
                                      href: `/plan/tasks?date=${Timestamp.fromString(
                                        task.dueDate,
                                      )
                                        .toCivilDate()
                                        .toJSON()}`,
                              ...
                              task.duration > 0
                                ? m(
                                    ...
                                    [
                                      ...
                                      m(
                                        "span",
                                        Duration.toString(task.duration),
                                      ),
                                    ],
                                  )
                                : [],
                        ...
                        m("div.PlanTasks__actions", [
                          ...
                          GlobalState.permitted(
                            ...
                            ? m("div.PlanTasks__menu-container", [
                                m(Icon, {
                                  classes:
                                    "GlobalButtonIconAdd PlanTasks__arrow",
                                  icon: Icons.Add,
                                  id: `taskmenutoggle_${task.id}`,
                                  onclick: async (e: m.Event<MouseEvent>) => {
                                    e.stopPropagation();

                                    AppState.setComponentsDropdownMenu(
                                      `taskmenu_${task.id}`,
                                      e.clientY,
                                    );
                                  },
                                  ...
                                  items: [
                                    {
                                      ...
                                      onclick: (): void => {
                                        ...
                                        t.position = Position.adjacent(
                                          task,
                                          PlanTaskState.findAdjacent(task),
                                          true,
                                        );
                            ...
                            : [],
                          m(
                            `i.PlanTasks__arrow.GlobalButtonIconExpand${
                              AuthAccountState.data().collapsedPlanTasks.includes(
                                `${task.id}`,
                              )
                                ? ""
                            ...
                            }`,
                            {
                              ...
                              onclick: async (e: m.Event<MouseEvent>) => {
                                e.stopPropagation();
                                return AuthAccountState.collapsePlanTask(
                                  task.id,
                                );
                              },
                              style: {
                                visibility:
                                  ...
                                  (!state.showComplete() &&
                                    PlanTaskState.getChildrenDone(task) ===
                                      PlanTaskChildrenStatus.allDone) ||
                                  (state.showComplete() &&
                                    PlanTaskState.getChildrenDone(task) ===
                                      PlanTaskChildrenStatus.none)
                                    ? "hidden"
                      ...
                      state.expandID === task.id
                        ? m(
                            `div.PlanTasks__item--details.${Animate.class(Animation.Expand)}`,
                            {
                  ...
                  (task.children !== undefined && task.children.length === 0) ||
                  (!state.showComplete() &&
                    PlanTaskState.getChildrenDone(task) ===
                      PlanTaskChildrenStatus.allDone) ||
                  (state.showComplete() &&
                  ...
                  AuthAccountState.data().collapsedPlanTasks.includes(
                    ...
                    ? []
                    : m(
                        `ul.PlanTasks__list.${Animate.class(Animation.Expand)}`,
                        {
                        ...
                        m(RenderObjects, {
                          ...
                          tasks:
                            task.children === undefined ? [] : task.children,
                        }),
                      ),
                ],
              );
      ...
      return vnode.attrs.headers !== undefined && vnode.attrs.headers.length > 0
        ? vnode.attrs.headers.map((header) => {
            const budgetCategory = BudgetCategoryState.findID(
              typeof header === "string" ? "" : header.budgetCategoryID,
            );
            ...
            const tasks =
              typeof header === "string"
                ? header === AuthAccountState.translate(WebPlanTasksOverdue)
                  ...
                  : PlanTaskState.findDueDate(
                      header === AuthAccountState.translate(WebGlobalToday) ||
                        header ===
                          AuthAccountState.translate(WebPlanTasksTomorrow)
                        ? header === AuthAccountState.translate(WebGlobalToday)
                          ? state.dateToday.toCivilDate().toJSON()
                          : state.dateTomorrow.toCivilDate().toJSON()
                        : header,
                    )
                : header.created === null
            ...
            return m(
              ...
              [
                m(
                  "div.PlanTasks__item.PlanTasks__item--header",
                  {
                    onclick: () => {
                      if (
                        typeof header !== "string" &&
                  ...
                  [
                        ...
                        ? m("span.PlanTasks__name.PlanTasks__name--header", [
                            typeof header === "string"
                              ? header === "Tomorrow" ||
                                ...
                                header === "Overdue"
                                ? header
                                : CivilDate.fromString(header) <= thisWeek &&
                                    CivilDate.fromString(header) >= today
                                  ? Timestamp.fromCivilDate(
                                      CivilDate.fromString(header),
                                    ).toDay()
                                  : CivilDate.fromString(header).toString(
                                      AuthAccountState.data().preferences
                                        .formatDateOrder,
                                      AuthAccountState.data().preferences
                                        .formatDateSeparator,
                        ...
                        : m(
                            ...
                            [
                              ...
                              PlanProjectState.findCount(header) > 0
                                ? m(
                                    "span.GlobalCount",
                                    PlanProjectState.findCount(header),
                                  )
                                : [],
                            ],
                          ),
                    ...
                    m("div.PlanTasks__actions", [
                              ...
                              items: [
                                {
                                  ...
                                  onclick: (): void => {
                                    const t = PlanTaskState.new();
                                    const tasks =
                                      PlanTaskState.findPlanProjectID(
                                        header.id,
                                      );
                                  ...
                                  permitted:
                                    (header.authAccountID !== null &&
                                      GlobalState.permitted(
                                        PermissionComponentsEnum.Plan,
                                        true,
                                      )) ||
                                    (header.authHouseholdID !== null &&
                                      GlobalState.permitted(
                                        PermissionComponentsEnum.Plan,
                                        true,
                                      )) ||
                                ...
                                },
                                {
                                  ...
                                  onclick: (): void => {
                                    ...
                                    const projects =
                                      PlanProjectState.findChildren(header.id);

                                    if (projects.length > 0) {
                                      p.position = Position.increase(
                                        projects[projects.length - 1].position,
                                        true,
                                      );
                                    } else {
                                      p.position = "0";
                      ...
                      typeof header === "string"
                        ...
                        : m(
                            `i.PlanTasks__arrow.GlobalButtonIconExpand${
                              AuthAccountState.data().collapsedPlanProjects.includes(
                                `${header.id}`,
                              )
                                ? ""
                                : ".GlobalButtonIconExpand--active"
                            }`,
                            {
                              ...
                              onclick: async (e: m.Event<MouseEvent>) => {
                                e.stopPropagation();
                                return AuthAccountState.collapsePlanProject(
                                  header.id,
                                );
                              },
                              style: {
                ...
                AuthAccountState.data().collapsedPlanProjects.includes(
                  ...
                  ? []
                  : m(
                      `ul.PlanTasks__list.PlanTasks__list--header.${Animate.class(Animation.Expand)}`,
                      {
                        id:
                          typeof header === "string"
                            ? `datelist_${header}`
...
export function PlanTasks(): m.Component {
  return {
    oninit: async (): Promise<void> => {
        ...
        toolbarActionButtons: [
          {
            ...
            ...{
              onclick: (): void => {
                ...
                if (tasks.length > 0) {
                  task.position = Position.increase(
                    tasks[tasks.length - 1].position,
                    true,
                  );
                } else {
          ...
          },
          {
            ...
            ...{
              onclick: (): void => {
                ...
                if (projects.length > 0) {
                  project.position = Position.increase(
                    projects[projects.length - 1].position,
                    true,
                  );
                } else {
      ...
      state.projects = Stream.lift(
        (_projects, _projectsNested, filter, tag) => {
          if (m.route.param().project !== undefined) {
            state.project = PlanProjectState.findID(m.route.param().project);
            state.project.children = PlanProjectState.findChildren(
              state.project.id,
            );
            return [];
          }
      ...
      state.tasks = Stream.lift(
        (_tasks, _tasksNested, filter, tag) => {
          if (
            filter === "templates" ||
            m.route.param().project !== undefined ||
            m.route.param().date !== undefined
          ) {
            return [];
          }
      ...
      switch (state.filter()) {
        case "upcoming":
          ...
          for (let i = 2; i < 7; i++) {
            ...
            d.addDays(i);
            state.dates.push(d.toCivilDate().toJSON());
          }
      ...
      if (m.route.param().date !== undefined) {
        state.filter("");
        state.dates = [m.route.param().date];
      }
          ...
          [
            m(Title, {
              ...
              tabs: [
                ...
                {
                  active:
                    m.route.param().filter === undefined &&
                    m.route.param().project === undefined &&
                    m.route.param().date === undefined,
                ...
                ...(m.route.param().project !== undefined &&
                state.project.parentID !== null
                  ? [
                      {
                        active: false,
                        href: `/plan/tasks?project=${state.project.parentID}`,
                        name: PlanProjectState.findID(state.project.parentID)
                          .name,
                      },
                    ]
                  : []),
            ...
            loaded
              ? m(Toolbar, {
                  actions:
                    state.filter() === "today" || state.filter() === "upcoming"
                      ? []
                      : [
                          {
                            icon: Icons.Show,
            ...
            m(
              ...
              loaded
                ...
                : [...Array(5)].map(() => {
                    return m(
                      `div.PlanTasks__item.PlanTasks__item--loading.${Animate.class(Animation.Pulse)}`,
                      [m("span"), m("span")],
                    );
                  }),
            ),
