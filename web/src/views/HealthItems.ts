...
import { ObjectHealth, ObjectInput, ObjectInputs, ObjectOutput, ObjectOutputs, WebGlobalActionAddDefaults, WebGlobalActionAddDefaultsSuccess, WebGlobalHouseholdMember, WebGlobalName, WebGlobalSearch, WebGlobalType, WebHealthItemsTotalCorrelations } from "../yaml8n";
...
export function HealthItems (): m.Component {
		...
		return {
			oninit: (vnode): void => {
				correlations = Object.keys(vnode.attrs.correlations)
					.sort((a,b) => {
						return (vnode.attrs.correlations[b] as number) - (vnode.attrs.correlations[a] as number);
					})
					.map((i) => {
			...
			view: (vnode): m.Children => {
				return m("div", {
					...
					m(ButtonArray, {
						name: "tags",
						onclick: () => {
							m.route.set(`/health/items?type=${state.output() ?
								"inputs" :
								"outputs"}&id=${AuthHouseholdState.findMember(state.id()).id}`, {}, {
								state: {
									key: Date.now(),
								},
							});
	...
	return {
		oninit: async (): Promise<void> => {
			...
			data = Stream.lift((_items, _logs, columns, id, output, search, sort) => {
			...
			}, HealthItemState.data, HealthLogState.data, state.columns, state.id, state.output, state.search, state.sort);
				...
				actions: [
					{
						...
						onclick: async (): Promise<void> => {
							...
							AppState.setLayoutAppAlert({
								message: AuthAccountState.translate(WebGlobalActionAddDefaultsSuccess),
							});
						},
				...
				filters: [
					{
						...
						selected: (): string[] => {
							return [
								AuthHouseholdState.findMember(state.id()).name,
							];
						},
						value: GlobalState.permitted(PermissionComponentsEnum.Auth, true, AuthAccountState.data().primaryAuthHouseholdID) && AuthHouseholdState.membersChildren().length > 0 ?
							AuthHouseholdState.findMemberNames(AuthAccountState.data().primaryAuthHouseholdID, true) :
							[],
					},
					{
