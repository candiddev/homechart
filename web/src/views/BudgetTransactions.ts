...
import {
  ...
  WebBudgetTransactionsTransferFrom,
  WebBudgetTransactionsTransferTo,
  WebGlobalActionImportTransactions,
  WebGlobalActionNext,
  WebGlobalActionPrevious,
  WebGlobalActionReconcile,
  WebGlobalActionSkip,
  WebGlobalAllTransactions,
...
function updateYearMonthRecurrence(budgetRecurrence: BudgetRecurrence): void {
  if (budgetRecurrence.template.date !== null) {
    const dateYM = CivilDate.fromString(
      budgetRecurrence.template.date,
    ).toYearMonth();
    const nextYM = Recurrence.nextTimestamp(
      budgetRecurrence.recurrence,
      Timestamp.fromCivilDate(
        CivilDate.fromString(budgetRecurrence.template.date),
      ),
    )
      .toCivilDate()
      .toYearMonth();
        ...
        m("div", [
              ...
              return BudgetTransactionState.create(data.template)
                .then(async () => {
                  if (data.template.date !== null) {
                    data.template.date = Recurrence.nextCivilDate(
                      data.recurrence,
                      CivilDate.fromString(data.template.date),
                    ).toJSON();
                  }
          ...
          m(Button, {
            ...
            onclick: async (): Promise<void | Err> => {
              ...
              updateYearMonthRecurrence(data);
              return BudgetRecurrenceState.update(data).then(async () => {
                return BudgetTransactionState.read(true);
              });
            },
            permitted: GlobalState.permitted(
...
export function BudgetTransactions(): m.Component {
  const state: {
    ...
    recurrenceCount: 0,
    recurring: false,
    yearMonth: Timestamp.now().toCivilDate().toYearMonth(),
  };

  const recurrences = Stream.lift(
    (recurrences, account) => {
      m.redraw();
  ...
  function getTitle(): string | undefined {
    if (
      BudgetTransactionState.data.length === 0 ||
      BudgetTransactionState.data.length === BudgetTransactionState.total
    ) {
      return undefined;
    }

    return `${
      state.page === 1 ? "1" : 50 * (state.page - 1) + 1
    } - ${50 * (state.page - 1) + BudgetTransactionState.data().length} ${AuthAccountState.translate(WebGlobalOf)} ${BudgetTransactionState.total} ${AuthAccountState.translate(ObjectTransactions)}`;
  }
  ...
  function generateDefaults(): BudgetTransaction {
    ...
    if (state.budgetCategory.id !== null) {
      ...
      if (state.budgetCategoryMonth !== null && transaction.date !== null) {
        transaction.categories[0].yearMonth = CivilDate.fromString(
          transaction.date,
        )
          .toYearMonth()
          .toNumber();
  ...
  return {
    oninit: async (): Promise<void> => {
      ...
      s = Stream.lift(
        async (_budgetAccounts, _budgetCategories, _budgetPayees, importID) => {
          if (m.route.param().account !== undefined) {
            state.budgetAccount(
              BudgetAccountState.findID(m.route.param().account),
            );
            state.recurrenceCount =
              BudgetRecurrenceState.findBudgetAccountIDToday(
                state.budgetAccount().id,
              );
          }

          if (m.route.param().category !== undefined) {
            state.budgetCategory = BudgetCategoryState.findID(
          ...
          AppState.setLayoutApp({
            ...
            breadcrumbs: [
              ...
              {
                link: `/budget/${getParent().toLowerCase()}`,
                name: `${getParent()}`,
              },
              {
                name: getName(),
            ...
            toolbarActionButtons: [
              {
                ...
                onclick:
                  state.importID() !== null // eslint-disable-line no-negated-condition
                    ? async (): Promise<void | Err> => {
                        ...
                        for (const transaction of BudgetTransactionState.data()) {
                          ...
                          await BudgetTransactionState.create(
                            transaction,
                          ).catch(() => {
                        ...
                        m.route.set(
                          `/budget/transactions?account=${state.budgetAccount().id}`,
                        );
                        state.importID(null);
                        setColumns();
                        return BudgetTransactionState.read(true);
      ...
      return [
        m(FormImportCSV, {
              ...
              oninput: async (file): Promise<void> => {
                ...
                reader.onload = async (): Promise<void> => {
                  if (typeof reader.result === "string") {
                    ...
                    BudgetTransactionState.data(
                      await OFXToBudgetTransactions(
                        reader.result,
                        state.budgetAccount().id,
                      ),
                    );
                    m.route.set(
                      `/budget/transactions?import=${state.budgetAccount().shortID}`,
                      {},
                      {
                        state: {
                          key: Date.now(),
          ...
          onsubmit: async (csvData): Promise<void> => {
            BudgetTransactionState.data(
              await CSVToBudgetTransactions(
                csvData,
                state.budgetAccount().id,
                state.formImportFields,
              ),
            );
            m.route.set(
              `/budget/transactions?import=${state.budgetAccount().shortID}`,
        ...
        m(Table, {
          actions: [
            {
              ...
              onclick: async (): Promise<void | Err> => {
                ...
                const transactions: BudgetTransaction[] = [];
                for (const transaction of BudgetTransactionState.data()) {
                  await BudgetTransactionState.create(transaction).catch(() => {
                    transactions.push(transaction);
                  });
                }
            ...
            },
            {
              ...
              onclick: async (): Promise<void | Err> => {
                return BudgetAccountState.reconcile(
                  state.budgetAccount().id,
                ).then(async () => {
                  return BudgetTransactionState.read(true);
                });
              ...
              permitted:
                GlobalState.permitted(PermissionComponentsEnum.Budget, true) &&
                state.budgetAccount().id !== null &&
                BudgetAccountState.findID(state.budgetAccount().id)
                  .budgetTransactionAmountCleared !==
                  BudgetAccountState.findID(state.budgetAccount().id)
                    .budgetTransactionAmountReconciled &&
                state.importID() === null,
              requireOnline: true,
            },
          ],
          data: state.recurring ? recurrences() : BudgetTransactionState.data(),
          ...
          id: "transactions",
          loaded:
            (m.route.param().recurring !== undefined &&
              BudgetRecurrenceState.isLoaded()) ||
            BudgetTransactionState.loaded,
          noFilters: true,
            ...
            ? [
                ...
                },
                {
                  formatter: (b: BudgetRecurrence): string => {
                    if (b.template.budgetPayeeID !== null) {
                      return BudgetPayeeState.findID(b.template.budgetPayeeID)
                        .name;
                    } else if (
                      b.template.accounts !== null &&
                      b.template.accounts.length === 2
                    ) {
                      for (const account of b.template.accounts) {
                        if (
                          account.budgetAccountID !== state.budgetAccount().id
                        ) {
                          return `${
                            account.amount > 0
                          ...
                          } ${BudgetAccountState.findID(account.budgetAccountID).name}`;
                        }
                      }
                    }
                ...
                },
                {
                    ...
                    if (b.template.categories !== null) {
                      if (b.template.categories.length === 1) {
                        return BudgetCategoryState.findIDHeaderName(
                          b.template.categories[0].budgetCategoryID,
                        );
                      }
                      return AuthAccountState.translate(WebGlobalMultiple);
                    }
                  ...
                  typeCheck: (b: BudgetRecurrence): TableDataType => {
                    if (
                      b.template.categories !== null &&
                      b.template.categories.length > 1
                    ) {
                ...
                },
                {
                  currencyFormat: AuthHouseholdState.findID(
                    state.budgetAccount().id === null
                      ...
                      : state.budgetAccount().authHouseholdID,
                ...
                },
                {
                  formatter: (b: BudgetTransaction): string => {
                    ...
                    if (b.categories.length === 1) {
                      return BudgetCategoryState.findIDHeaderName(
                        b.categories[0].budgetCategoryID,
                      );
                    }
                    return AuthAccountState.translate(WebGlobalMultiple);
                  },
                    ...
                    if (state.cleared.accounts !== null) {
                      ...
                      for (const account of state.cleared.accounts) {
                        if (
                          ...
                          if (
                            account.status ===
                            BudgetTransactionAccountStatusEnum.Reconciled
                          ) {
                            return;
                          }
                          if (
                            account.status >=
                            BudgetTransactionAccountStatusEnum.Cleared
                          ) {
                            account.status = 0;
                          } else {
                            account.status = 1;
                          }
                          await BudgetTransactionState.update(
                            state.cleared,
                          ).then(async () => {
                            updated = true;
                          });
          ...
          title: {
            ...
            buttonRight:
              state.page * 50 > BudgetTransactionState.total || state.recurring
                ? undefined
                : {
                    href: `${getHref()}&page=${state.page + 1}`,
                    icon: Icons.Next,
                ...
                : [
                    {
                      ...
                      value: Currency.toString(
                        state.budgetAccount().budgetTransactionAmount,
                    ...
                    },
                    {
                      ...
                      value: Currency.toString(
                        state.budgetAccount().budgetTransactionAmountCleared,
                        AuthHouseholdState.findID(
                          state.budgetAccount().authHouseholdID,
                        ).preferences.currency,
                      ),
                    },
                    {
                      ...
                      value: Currency.toString(
                        state.budgetAccount().budgetTransactionAmountReconciled,
                        AuthHouseholdState.findID(
                          state.budgetAccount().authHouseholdID,
                        ).preferences.currency,
                      ),
            ...
            tabs:
              state.budgetAccount().id === null || state.importID() !== null
                ? undefined
                : [
                    {
