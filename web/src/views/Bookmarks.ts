...
import { ObjectBookmarks, ObjectHousehold, ObjectLink, WebBookmarksSearch, WebFormOverlayBookmarkShowHome, WebGlobalActionShowTag, WebGlobalAll, WebGlobalName, WebGlobalPersonal, WebGlobalTag } from "../yaml8n";
...
export function Bookmarks (): m.Component {
	...
	const bookmarks = Stream.lift((bookmarks, columns, filter, search, sort, tag) => {
		const data = BookmarkState.filter(bookmarks, false, filter, search, tag, (bookmark) => {
			return bookmark.name.toLowerCase()
				.includes(search) || bookmark.link.toLowerCase()
				.includes(search);
		});
		...
		return data.data;
	}, BookmarkState.data, state.columns, state.filter, state.search, state.sort, state.tag);
					...
					tableColumns: [
						...
						},
						{
							...
							permitted: (b: Bookmark): boolean => {
								return GlobalState.permitted(PermissionComponentsEnum.Auth, true, b.authHouseholdID);
							},
							property: "home",
