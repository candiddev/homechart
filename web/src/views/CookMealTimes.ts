...
import { ObjectCook, ObjectMealTimes, WebFormOverlayCookMealTimeTime, WebGlobalName } from "../yaml8n";
...
export function CookMealTimes (): m.Component {
	...
	return {
		oninit: async (): Promise<void> => {
			...
			d = Stream.lift((data, columns, sort) => {
				m.redraw();

				return Filter.array(data, columns, sort);
			}, CookMealTimeState.data, state.columns, state.sort);
			...
			AppState.setLayoutApp({
				...
				toolbarActionButtons: [
					AppToolbarActions().newCookMealTime,
				],
			});
				...
				tableColumns: [
					...
					},
					{
						formatter: (e: CookMealTime): string => {
							if (e.time !== null) {
								return CivilTime.fromString(e.time)
