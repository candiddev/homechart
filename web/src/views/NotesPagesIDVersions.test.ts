...
test("NotesPagesIDVersions", async () => {
	...
	testing.text(`#table-data-${seed.notesPageVersions[4].id}-updated`, Timestamp.fromString(seed.notesPageVersions[4].updated)
	...
	testing.text(`#table-data-${seed.notesPageVersions[4].id}-createdby`, seed.authAccounts[1].name);
	testing.text(`#table-data-${seed.notesPageVersions[3].id}-updated`, Timestamp.fromString(seed.notesPageVersions[3].updated)
		.toPrettyString(AuthAccountState.data().preferences.formatDateOrder, AuthAccountState.data().preferences.formatDateSeparator, AuthAccountState.data().preferences.formatTime24));
	testing.text(`#table-data-${seed.notesPageVersions[3].id}-createdby`, seed.authAccounts[0].name);
});
