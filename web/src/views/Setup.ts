...
import { ObjectAccount, ObjectHousehold, WebGlobalActionFinish, WebGlobalActionNext, WebGlobalActionSetup, WebGlobalName, WebGlobalNameTooltip, WebGlobalSubscription, WebSetupAlertHousehold, WebSetupOverview, WebSetupWelcome, WebSetupWelcomeTo } from "../yaml8n";

export const Setup = (): m.Component => {
	const state = {
		authAccount: AuthAccountState.data(),
		authHousehold: AuthHouseholdState.findID(AuthAccountState.data().primaryAuthHouseholdID),
	};
	...
	return {
		...
		view: (): m.Children => {
			return m(Form, {
				buttons: [
					{
						...
						permitted: ! AuthAccountState.isDemo() && ! AuthAccountState.data().child && m.route.param().target !== "subscription" || AuthAccountState.isDemo() && m.route.param().target !== "household" || AuthAccountState.data().child && m.route.param().target !== "account",
						requireOnline: true,
					},
					{
						name: AuthAccountState.isDemo() && m.route.param().target === "household" || AuthAccountState.data().child && m.route.param().target === "account" || m.route.param().target === "subscription" ?
				...
				onsubmit: async () => {
					...
					case "household":
						...
						if (! AuthAccountState.isDemo()) {
							...
							m.route.set("/setup/subscription", {}, {
								state: {
									key: Date.now(),
								},
							});
				...
				progressCurrent: m.route.param().target === "account" ?
					1 :
				...
				m.route.param().target === "household" ?
					AuthSessionState.data().permissionsHouseholds === null || AuthSessionState.data().permissionsHouseholds!.length === 0 ? // eslint-disable-line @typescript-eslint/no-non-null-assertion
						m(FormJoinHousehold, {
							add: true,
							onjoin: (household) => {
					...
					m.route.param().target === "subscription" ?
						...
						m("div.Setup#welcome", [
							...
							m("p.Setup__welcome", `${AuthAccountState.translate(WebSetupWelcomeTo)} Homechart`),
							m("p.Setup__overview", AuthAccountState.translate(WebSetupOverview)),
						]),
			);
		},
	};
