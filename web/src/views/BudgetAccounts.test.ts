...
test("BudgetAccounts", async () => {
	...
	testing.text(`#table-data-${seed.budgetAccounts[0].id}-name`, seed.budgetAccounts[0].name);
	testing.text(`#table-data-${seed.budgetAccounts[0].id}-budgettransactionamount`, Currency.toString(seed.budgetAccounts[0].budgetTransactionAmount, AuthHouseholdState.data()[0].preferences.currency));
	testing.text(`#table-data-${seed.budgetAccounts[0].id}-budget`, Icons.CheckYes);
