...
import {
  ...
  WebGlobalActionRedeem,
  WebGlobalActionReset,
  WebGlobalDetails,
  WebGlobalInProgress,
  WebGlobalStatus,
} from "../yaml8n";
  ...
  return {
    oninit: async (): Promise<void> => {
      ...
      AppState.setLayoutApp({
        ...
        breadcrumbs:
          filter === "" || filter === "all"
            ...
            : [
                ...
                {
                  link:
                    filter === "sent" && m.route.param().to !== undefined
                      ? "/sent"
                      : undefined,
                  name:
                    filter === "inprogress"
        ...
        toolbarActionButtons: [AppToolbarActions().newRewardCard],
      });
              ...
              return m(
                ...
                [
                  m("div.RewardCards__card", [
                    ...
                    card.recipients.length !== 1 ||
                    (card.recipients.length === 1 &&
                      card.recipients[0] === AuthAccountState.data().id)
                      ? []
                      : m("div.RewardCards__field", [
                          m(
                            "p",
                          ...
                          m(
                            ...
                            card.recipients
                              .map((recipient) => {
                                return AuthHouseholdState.findMemberName(
                                  recipient,
                                );
                              })
                              .join(", "),
                  ...
                  m(
                    "div.RewardCards__stamps",
                    {
                      onclick: async () => {
                        return RewardCardState.punch(card);
                      },
                    },
                    Array.from(Array(card.stampGoal).keys()).map((num) => {
                      if (
                        (!card.invert && num + 1 <= card.stampCount) ||
                        (card.invert &&
                          num + 1 <= card.stampGoal - card.stampCount)
                      ) {
                        return m(
                          "span.RewardCards__stamp.RewardCards__stamp--stamped",
                        );
                      }
