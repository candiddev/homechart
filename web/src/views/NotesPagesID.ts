...
import { ObjectNotes, ObjectPage, ObjectParentPage, WebAdminNotificationsBody, WebGlobalActionSave, WebGlobalName, WebGlobalNameTooltip, WebGlobalTags, WebGlobalTagsTooltip, WebNotesPagesIDParentPageTooltip } from "../yaml8n";
...
export function NotesPagesID (): m.Component {
	...
	return {
		oninit: async (): Promise<void> => {
			state.lift = Stream.lift((_notesPage, _notesPageVersion, edit, id, parentID, version) => {
				...
				if (!edit || state.notesPageVersion.id === null) {
					const page = NotesPageVersionState.findNotesPageIDLatest(state.notesPage.id);
				...
				if (state.notesPage.name === "") {
					if (m.route.param().tag === "personal" || AuthAccountState.data().primaryAuthHouseholdID === null) {
						state.notesPage.authAccountID = AuthAccountState.data().id;
					} else {
						state.notesPage.authHouseholdID = AuthAccountState.data().primaryAuthHouseholdID;
					}
				}
					...
					toolbarActionButtons: [
						{
							...
							onclick: (): void => {
								state.edit(false);
								m.route.set(`/notes/${state.notesPage.authAccountID === null ?
									"household" :
									"personal"}/new`, {
									edit: "",
							...
							permitted: GlobalState.permitted(PermissionComponentsEnum.Notes, true, state.notesPage.authHouseholdID),
							requireOnline: true,
						},
					],
				});
				...
				m.redraw();
			}, NotesPageState.data, NotesPageVersionState.data, state.edit, state.id, state.parentID, state.version);
		...
		view: (): m.Children => {
			const loaded = NotesPageState.isLoaded() && NotesPageVersionState.isLoaded();
			...
			return state.edit() ?
				m(Form, {
					...
					onsubmit: async (): Promise<void | Err> => {
						if (m.route.param().id === "new") {
							...
							return NotesPageState.create(state.notesPage)
								.then(async (page) => {
									...
									return NotesPageVersionState.create(state.notesPageVersion, true);
								})
								.then(() => {
									m.route.set(`/notes/${state.notesPage.authHouseholdID === null ?
										"personal" :
										"household"}/${id}`, undefined, {
										state: {
											key: Date.now(),
										},
									});
						...
						if (state.notesPageVersion.body !== NotesPageVersionState.findID(state.notesPageVersion.id).body) {
							state.notesPageVersion.updated = null;
						...
						m.route.set(`/notes/${state.notesPage.authHouseholdID === null ?
							"personal" :
							"household"}/${state.notesPage.id}`, undefined, {
							state: {
								key: Date.now(),
							},
						});
					...
					title: {
						buttonLeft: {
							...
							onclick: async (): Promise<void> => {
								m.route.set(`/notes/${m.route.param().tag}${state.notesPage.id === null ?
									"" :
									`/${state.notesPage.id}`}`, {}, {
									state: {
										key: Date.now(),
					...
					m(FormItemSelectNested, {
						...
						tooltip: AuthAccountState.translate(WebNotesPagesIDParentPageTooltip),
						value:  state.notesPage.parentID === null ?
							state.notesPage.authAccountID === null ?
								"household" :
								"personal" :
					...
					m(Title,
						{
							buttonLeft: {
								accent: true,
								icon: state.notesPage.deleted === null ?
									Icons.Delete :
									Icons.DeleteForever,
								name: state.notesPage.deleted === null ?
									"Delete" :
									"Delete Forever",
								onclick: async (): Promise<void> => {
									if (state.notesPage.deleted === null) {
										const page = NotesPageState.findID(m.route.param().id);
										page.deleted = Timestamp.now()
											.toString();
										return NotesPageState.update(page)
											.then(() => {
									...
									return NotesPageState.delete(state.notesPage.id)
										.then(() => {
							...
							buttonRight: {
								icon: state.notesPage.deleted === null && m.route.param().version === undefined ?
									Icons.Edit :
									Icons.Restore,
								name: state.notesPage.deleted === null && m.route.param().version === undefined ?
									"Edit" :
									"Restore",
								onclick: async (): Promise<void> => {
									if (state.notesPage.deleted !== null || m.route.param().version !== undefined) {
										if (m.route.param().version !== undefined) {
											await NotesPageVersionState.create(state.notesPageVersion);
										}

										state.notesPage.deleted = null;
										return NotesPageState.update(state.notesPage)
											.then(() => {
												state.edit(false);
												m.route.set(`/notes/${state.notesPage.authAccountID === null ?
													"household" :
													"personal"}/${m.route.param().id}`, {}, {
													state: {
														key: Date.now(),
													},
												});
									...
									m.route.set(`/notes/${state.notesPage.authAccountID === null ?
										"household" :
										"personal"}/${state.notesPage.id}?edit`, {}, {
										state: {
											key: Date.now(),
										},
									});
								...
								permitted: GlobalState.permitted(PermissionComponentsEnum.Notes, true, state.notesPage.authHouseholdID),
								primary: m.route.param().version !== undefined || state.notesPage.deleted !== null,
								requireOnline: true,
							},
							loaded: loaded,
							name: state.notesPage.name === "" ?
						...
						[
									...
									m("div.NotesPageID__toc", {
										...
										m("div", state.notesPageVersion.body.split("\n")
											.reduce((state: {
												...
												} else if (! state.codeBlock && line.includes("# ")) {
													state.components.push(m("a.GlobalLink", {
							...
							m("div.GlobalFooter", [
								state.notesPage.tags.length > 0 ?
									...
									[],
								m("span.NotesPageID__modified", {
									id: "updated",
								}, [
									`${AuthAccountState.translate(FormLastUpdated)}: `,
									m(m.route.Link, {
										href: `/notes/${m.route.param().tag}/${m.route.param().id}/versions`,
									}, AppState.formatCivilDate(Timestamp.fromString(state.notesPageVersion.updated!) // eslint-disable-line @typescript-eslint/no-non-null-assertion
										.toCivilDate()
										.toJSON())),
								]),
							]),
