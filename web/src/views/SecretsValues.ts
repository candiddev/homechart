...
import { ObjectSecrets, WebGlobalActionShowTag, WebGlobalAll, WebGlobalName, WebSecretsEditVault, WebSecretsSearch } from "../yaml8n";
...
export function SecretsValues (): m.Component {
	...
	return {
		oninit: async (): Promise<void> => {
			data = Stream.lift((values, columns, search, sort, tag, vaultID) => {
				m.redraw();

				return Filter.array(Object.keys(values)
					.map((v) => {
						return {
					...
					.filter((value) => {
						...
						return (vaultID === "all" || value.secretsVaultID === vaultID) && (tag === "" || value.tags.includes(tag)) && (search === ""
					|| s.toLowerCase()
						.includes(search.toLowerCase())
						);
					}), columns, sort);
			}, SecretsValueState.values, state.columns, state.search, state.sort, state.tag, state.vaultID);

			s = Stream.lift((_vaults, id) => {
				if (id !== "all") {
				...
				m.redraw();
			}, SecretsVaultState.data, state.vaultID);
				...
				actions: [
					{
						...
						permitted: GlobalState.permitted(PermissionComponentsEnum.Secrets, true) && state.vaultID() !== "all",
						requireOnline: true,
					},
				],
				data: data(),
						...
						.map((property) => {
							return {
								formatter: (s: SecretsValueDecrypted): string => {
									...
									if (s.data[0] !== undefined && s.data[0][property] !== undefined) {
