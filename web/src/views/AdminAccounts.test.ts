...
test("AdminAccounts", async () => {
	...
	testing.text(`#table-data-${seed.authAccounts[0].id}-id`, seed.authAccounts[0].id);
	testing.text(`#table-data-${seed.authAccounts[0].id}-primaryauthhouseholdid`, seed.authAccounts[0].primaryAuthHouseholdID);
	testing.text(`#table-data-${seed.authAccounts[0].id}-emailaddress`, seed.authAccounts[0].emailAddress);
	testing.text(`#table-data-${seed.authAccounts[0].id}-created`, AppState.formatCivilDate(Timestamp.fromString(seed.authAccounts[0].created)
		.toCivilDate()));
	testing.text(`#table-data-${seed.authAccounts[0].id}-lastactivity`, AppState.formatCivilDate(Timestamp.fromString(seed.authAccounts[0].lastActivity)
		.toCivilDate()));
