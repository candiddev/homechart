...
export function BudgetCategories(): m.Component {
  const state: {
    ...
    collapsedCategories: Stream([AuthAccountState.translate(WebGlobalHidden)]),
    columns: Stream<FilterType>({
      budgetCategoryName: "",
  ...
  const categories = Stream.lift(
    (month, collapsed, columns) => {
      m.redraw();

      return Filter.array(
        month.budgetMonthCategories.filter((category) => {
  ...
  return {
    oninit: async (): Promise<void> => {
      ...
      AppState.setLayoutApp({
        ...
        toolbarActionButtons: [AppToolbarActions().newBudgetCategory],
      });
      ...
      if (parseInt(m.route.param().yearMonth, 10) >= 201801) {
        BudgetMonthState.yearMonth = YearMonth.fromNumber(
          parseInt(m.route.param().yearMonth, 10),
        );
      } else {
        BudgetMonthState.yearMonth = CivilDate.now().toYearMonth();
      }

      if (
        BudgetMonthState.data().yearMonth ===
        CivilDate.now().toYearMonth().toNumber()
      ) {
        await BudgetMonthState.load().then(async (month: BudgetMonth) => {
          BudgetMonthState.data(month);
        });
      }

      return BudgetMonthState.read(state.householdID).then(() => {
    ...
    view: (): m.Children => {
      return m(Table, {
        actions: [
          {
            ...
            onclick: async (): Promise<void | Err> => {
              if (BudgetMonthState.data().budgetMonthCategories !== null) {
                ...
                BudgetMonthState.data().budgetMonthCategories.map(async (b) => {
                  // eslint-disable-line @typescript-eslint/no-non-null-assertion
                  if (
                    b.budgetCategory.grouping === "" ||
                    b.budgetCategory.grouping ===
        ...
        tableColumns: [
          {
            ...
            onclick: (data): void => {
              ...
              if (state.collapsedCategories().includes(data)) {
          ...
          {
            currencyFormat: AuthHouseholdState.findID(state.householdID)
            ...
            formatter: (category: BudgetMonthCategory): number | string => {
              ...
              return category.budgetCategory.name !== "Hidden" &&
                (category.yearMonth === BudgetMonthState.yearMonth.toNumber() ||
                  category.budgetCategory.grouping === "")
                ? category.amount
                : 0;
            },
            ...
            formatter: (category: BudgetMonthCategory): number | string => {
              ...
              return Currency.toString(
                category.yearMonth === BudgetMonthState.yearMonth.toNumber()
                  ...
                  : 0,
                AuthHouseholdState.findID(state.householdID).preferences
                  .currency,
              );
            },
            linkFormatter: (category: BudgetMonthCategory): string => {
        ...
        title: {
          buttonLeft: {
            href: `/budget/categories/${YearMonth.fromNumber(
          ...
          buttonRight: {
            ...
            icon: Icons.Next,
            iconRight: true,
            name: BudgetMonthState.yearMonth.addMonths(1).toString(),
            permitted: true,
            requireOnline: true,
          },
          ...
          subtitles: [
            {
              color:
                BudgetMonthState.data().budgetTransactionAmountIncomeRemaining +
                  BudgetMonthState.data().budgetMonthCategoryAmount -
                  BudgetMonthState.data().budgetTransactionAmountIncome <
                0
                  ? "var(--color_negative)"
                  : "var(--color_positive)",
            ...
            },
            {
              color:
                BudgetMonthState.data().yearMonth ===
                  BudgetMonthState.yearMonth.toNumber() &&
                BudgetMonthState.data().budgetTransactionAmountIncome < 0
                  ? "var(--color_negative)"
                  : "var(--color_positive)",
            ...
            {
              color:
                -1 * BudgetMonthState.data().budgetMonthCategoryAmount < 0
                  ? "var(--color_negative)"
                  : "var(--color_positive)",
              ...
              value:
                ...
                BudgetMonthState.yearMonth.toNumber()
                  ? Currency.toString(
                      BudgetMonthState.data().budgetMonthCategoryAmount * -1,
                      AuthHouseholdState.findID(state.householdID).preferences
                        .currency,
                    )
                  : Currency.toString(
            ...
            },
            {
              ...
              value:
                ...
                BudgetMonthState.yearMonth.toNumber()
                  ? Currency.toString(
                      -1 * (BudgetMonthState.data().targetAmount as number),
                      AuthHouseholdState.findID(state.householdID).preferences
                        .currency,
                    )
                  : Currency.toString(
