...
import { ObjectBackup, WebGlobalSelfHostedAddress, WebGlobalSelfHostedAddressTooltip, WebGlobalSubscription, WebSubscriptionActions, WebSubscriptionBackupKey, WebSubscriptionBackupKeyTooltip, WebSubscriptionCloudBackups, WebSubscriptionCloudFeatures, WebSubscriptionEnableCloudBackups, WebSubscriptionEnableCloudProxy } from "../yaml8n";
...
export function Subscription (): m.Component {
	...
	async function reloadBackups (id: NullUUID): Promise<void> {
		return CloudBackupState.readAll(id)
			.then(async (d) => {
				if (!IsErr(d)) {
					state.backups(d);
					state.loaded = true;
		...
		oninit: async (): Promise<void> => {
			...
			s = AuthHouseholdState.data.map(async (households) => {
				if (state.authHousehold.id === null) {
					state.authHousehold = Clone(AuthHouseholdState.findID(AuthAccountState.data().primaryAuthHouseholdID));

					if (state.authHousehold.id === null) {
						state.authHousehold = Clone(AuthHouseholdState.findID(m.route.param().household));
					}

					if (state.authHousehold.id !== null && m.route.param().success !== undefined) {
						Alerts.purchase();
				...
				for (const household of households) {
					if (state.authHousehold.id === household.id && household.backupEncryptionKey !== "") {
							...
							toolbarActionButtons: [
								{
									...
									onclick: async (): Promise<void> => {
										return CloudBackupState.create(household.id)
											.then(async () => {
												return reloadBackups(household.id);
											});
									},
									permitted: GlobalState.permitted(PermissionComponentsEnum.Auth, true, household.id) && ! InfoState.data().cloud && household.backupEncryptionKey !== "",
									requireOnline: true,
								},
							],
						});
			...
			if (InfoState.data().cloud) {
				state.url = AuthHouseholdState.findID(state.authHousehold.id).selfHostedURL;
			} else {
				state.url = CloudHouseholdState.findID(state.authHousehold.id).selfHostedURL;
			}

			if (! InfoState.data().cloud && AuthHouseholdState.findID(state.authHousehold.id).backupEncryptionKey !== "") {
				await reloadBackups(state.authHousehold.id);
			}
			...
			return [
				...
				m(Form, {
					...
					InfoState.data().cloud ?
						...
						: [
							m(FormCheckbox, {
								name: AuthAccountState.translate(WebSubscriptionEnableCloudBackups),
								onclick: async (): Promise<void> => {
									if (AuthHouseholdState.findID(state.authHousehold.id).backupEncryptionKey !== "") {
								...
								value: state.showBackups || AuthHouseholdState.findID(state.authHousehold.id).backupEncryptionKey !== "",
							}),
							state.showBackups || AuthHouseholdState.findID(state.authHousehold.id).backupEncryptionKey !== "" ?
								[
									m(FormItem, {
										input: {
											oninput: (e): void => {
												...
												setTimeout(async () => {
													if (state.authHousehold.backupEncryptionKey === e && AuthHouseholdState.findID(state.authHousehold.id).backupEncryptionKey !== e) {
														return AuthHouseholdState.update(state.authHousehold);
													}
												}, 500);
											},
											type: state.showEncryptionKey ?
												"text" :
												"password",
											value: state.authHousehold.backupEncryptionKey,
										},
										name: AuthAccountState.translate(WebSubscriptionBackupKey),
										tooltip: AuthAccountState.translate(WebSubscriptionBackupKeyTooltip),
									}),
									m(FormCheckbox, {
						...
						onclick: async (): Promise<void> => {
							if (InfoState.data().cloud) {
								...
								return CloudHouseholdState.update({
									...
									...{
										selfHostedURL: CloudHouseholdState.findID(state.authHousehold.id).selfHostedURL === "" ?
									...
									.then(() => {
										state.url = CloudHouseholdState.findID(state.authHousehold.id).selfHostedURL;
										state.showURL = CloudHouseholdState.findID(state.authHousehold.id).selfHostedURL !== "";
										m.redraw();
									});
							}
						},
						value: state.showURL || CloudHouseholdState.findID(state.authHousehold.id).selfHostedURL !== "" || AuthHouseholdState.findID(state.authHousehold.id).selfHostedURL !== "",
					}),
					state.showURL === true || AuthHouseholdState.findID(state.authHousehold.id).selfHostedURL !== "" || CloudHouseholdState.findID(state.authHousehold.id).selfHostedURL !== "" ?
						m(FormItem, {
							input: {
								oninput: (e): void => {
									state.url = e;
				...
				! InfoState.data().cloud && state.authHousehold.backupEncryptionKey !== "" ?
					m(Table, {
						actions: [],
						data: state.backups(),
						filters: [],
												...
												icons: [
													{
														icon: Icons.Backup,
														onclick: async (): Promise<void> => {
															return CloudBackupState.restore(state.authHousehold.id, vnode.attrs.id)
																.then(async () => {
													...
													},
													{
														icon: Icons.Delete,
														onclick: async (): Promise<void> => {
															return CloudBackupState.delete(state.authHousehold.id, vnode.attrs.id)
																.then(async () => {
