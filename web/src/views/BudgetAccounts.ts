...
import { ObjectAccounts, ObjectBudget, WebFormOverlayBudgetAccountOnBudget, WebGlobalActionShowHidden, WebGlobalBudgetBalance, WebGlobalName } from "../yaml8n";
...
export function BudgetAccounts (): m.Component {
	...
	return {
		oninit: async (): Promise<void> => {
			data = Stream.lift((accounts, authHouseholdID, columns, hide, sort) => {
				m.redraw();

				return Filter.array(accounts.filter((account: BudgetAccount) => {
					return account.hidden === hide && authHouseholdID === null || account.authHouseholdID === authHouseholdID;
				}), columns, sort);
			}, BudgetAccountState.data, state.authHouseholdID, state.columns, state.hidden, state.sort);
			...
			AppState.setLayoutApp({
				...
				toolbarActionButtons: [
					AppToolbarActions().newBudgetAccount,
				],
			});
		...
		view: (): m.Children => {
			return m(Table, {
				...
				tableColumns: [
					...
					{
						currencyFormat: AuthHouseholdState.findID(state.authHouseholdID()).preferences.currency,
