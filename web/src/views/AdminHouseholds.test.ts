...
AuthHouseholdState.readAdmin = async (): Promise<APIResponse<AuthHousehold[]>> => {
...
test("AdminHouseholds", async () => {
	...
	testing.text(`#table-data-${seed.authHouseholds[0].id}-id`, seed.authHouseholds[0].id);
	testing.text(`#table-data-${seed.authHouseholds[0].id}-countmembers`, `${seed.authHouseholds[0].countMembers}`);
	testing.text(`#table-data-${seed.authHouseholds[0].id}-created`, AppState.formatCivilDate(Timestamp.fromString(seed.authHouseholds[0].created)
		.toCivilDate()));
	testing.text(`#table-data-${seed.authHouseholds[0].id}-subscriptionexpires`, AppState.formatCivilDate(seed.authHouseholds[0].subscriptionExpires));
	testing.text(`#table-data-${seed.authHouseholds[0].id}-subscriptionprocessor`, "Free Trial");
});
