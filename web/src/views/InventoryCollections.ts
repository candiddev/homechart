...
import { ObjectInventory, WebGlobalAll, WebGlobalAllCollections, WebGlobalGrouping, WebGlobalName } from "../yaml8n";
...
export function InventoryCollections (): m.Component {
	...
	return {
		oninit: async (): Promise<void> => {
			data = Stream.lift((collections, columns, sort) => {
				return Filter.array([
					{
			...
			}, InventoryCollectionState.data, state.columns, state.sort);
