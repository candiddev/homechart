...
test("BudgetCategories", async () => {
  ...
  const current = Timestamp.now().toCivilDate().toYearMonth();
  ...
  testing.text(
    "#table-data-foodpistachios-budgetcategorytargetamount",
    " $575.00",
  );
  ...
  testing.mocks.responses = [
    {
      ...
      dataValue: [seed.budgetCategories],
    },
    {
      dataType: "BudgetMonth",
      dataValue: [
      ...
      dataValue: [
        {
          ...seed.budgetMonths[0],
          ...{
            yearMonth: year.addMonths(1).toNumber(),
          },
        },
      ],
    },
