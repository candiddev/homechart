...
describe("PlanTasks", () => {
	...
	test("drag.projectToProject", async () => {
		...
		expect(AuthAccountState.data().collapsedPlanProjects)
			.toContain(seed.planProjects[10].id);
		...
		expect(target1.previousSibling)
			.toBe(project);
		...
		expect(project.previousSibling)
			.toBe(target2);
	...
	test("drag.taskToTask", async () => {
		...
		drag.dispatchEvent(new Event("dragstart"));
		await testing.sleep(100);
		expect(AuthAccountState.data().collapsedPlanTasks)
			.toContain(seed.planTasks[9].id);
	...
	test("headers.dates", async () => {
		...
		testing.text(d.getElementsByClassName("PlanTasks__name--header")[0], "Tomorrow");
	});
	...
	test("headers.projects", async () => {
		...
		testing.text(testing.find(`#project_${seed.planProjects[5].id}`)
			.getElementsByClassName("PlanTasks__name")[0], "Cleaning1paymentsCleaning $50.00/ $12.00tagchores");
		testing.text(testing.find(`#project_${seed.planProjects[7].id}`)
			.getElementsByClassName("PlanTasks__name")[0], "Kitchen4schedule6h 10m");
		testing.notFind("#button-cancel");
	...
	test("items", async () => {
		...
		// name
		testing.text(itemEl.getElementsByClassName("PlanTasks__name")[0], "Broom $12.00categoryToolsstoreJane's General StorelistPick Up");
	});
	...
	test("loading", async () => {
		...
		testing.mocks.params = {};
		testing.mount(PlanTasks);
		expect(document.getElementsByClassName("PlanTasks__name"))
			.toHaveLength(0);
		expect(document.getElementsByClassName("PlanTasks__item--loading"))
			.toHaveLength(5);
	...
	test("none", async () => {
		...
		testing.mount(App, routeOptions, PlanTasks);
		const none = document.getElementsByClassName("PlanTasks__none")[0].getElementsByTagName("button");
		expect(none)
			.toHaveLength(3);
	...
	test("tasks", async () => {
		...
		testing.requests([
			{
				body: {
					...task,
					...{
						dueDate: Recurrence.nextTimestamp(task.recurrence, Timestamp.fromString(task.dueDate))
							.toString(),
						lastDoneBy: AuthAccountState.data().id,
						lastDoneDate: CivilDate.now()
							.toJSON(),
					},
				},
				method: "PUT",
		...
		// name
		const name = taskEl.getElementsByClassName("PlanTasks__item")[0].getElementsByTagName("div")[0] as HTMLElement;
		testing.notFind("#button-cancel");
		testing.click(name);
		testing.click("#button-cancel");
		await testing.sleep(100);
		...
		testing.text(duedate, `calendar_month${Timestamp.fromString(task.dueDate)
			.toDueDate(seed.authAccounts[0].preferences.formatTime24)}notificationsrepeat`);
		testing.hasClass(duedate, "ButtonArray__item");
		expect(details.getElementsByClassName("PlanTasks__notification"))
			.toHaveLength(1);
		expect(details.getElementsByClassName("PlanTasks__recurrence"))
			.toHaveLength(1);
		testing.text(details, "calendar_month11:50 PMnotificationsrepeatschedule30mpersonJanetagatagb");
		expect(details.getElementsByClassName("PlanTasks__project"))
			.toHaveLength(0);
	...
	test.each([
		...
		expect(document.getElementsByClassName("Title__tab"))
			.toHaveLength(tabs);
	});
	...
	test("templates", async () => {
		...
		testing.text(personal.getElementsByClassName("PlanTasks__item--header")[0], "labelPersonaladdexpand_less");
		expect(personal.getElementsByClassName("PlanTasks__item--task"))
			.toHaveLength(1);
		testing.text(personal.getElementsByClassName("PlanTasks__item--task")[0], "drag_indicatorcheck_box_outline_blankTemplate 1addexpand_less");

		const household = testing.find("#project_household");
		testing.text(household.getElementsByClassName("PlanTasks__item--header")[0], "labelHouseholdaddexpand_less");
		expect(household.getElementsByClassName("PlanTasks__item--task"))
			.toHaveLength(1);
		testing.text(household.getElementsByClassName("PlanTasks__item--task")[0], "drag_indicatorcheck_box_outline_blankTemplate 2addexpand_less");

		const homechart = testing.find("#project_homechart");
		testing.text(homechart.getElementsByClassName("PlanTasks__item--header")[0], "labelHomechartexpand_less");
		expect(homechart.getElementsByClassName("PlanTasks__item--task"))
			.toHaveLength(1);
		testing.text(homechart.getElementsByClassName("PlanTasks__item--task")[0], "check_box_outline_blankTemplate 3addexpand_less");
