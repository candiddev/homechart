...
import { ActionAdd, ActionCancel, ActionDelete, ActionDeleteConfirm, ActionNew, ActionUpdate, FormCreated, FormItemNewPasswordPassword } from "@lib/yaml8n";
...
import { ObjectAccount, ObjectAccounts, ObjectHousehold, WebAdminAccountsLastActivity, WebAdminAccountsSearch, WebGlobalActionDeleteSessions, WebGlobalActionResetPassword,WebGlobalAdmin, WebGlobalEmailAddress, WebGlobalEmailAddressInviteTooltip, WebGlobalOf, WebGlobalOIDCProvider, WebGlobalVerified } from "../yaml8n";
...
export function AdminAccounts (): m.Component {
	...
	async function update (): Promise<void> {
		...
		return AuthAccountState.readAll(state.filter, offset)
			.then((response) => {
				if (! IsErr(response)) {
	...
	const a = Stream.lift((authAccounts, columns) => {
		return Filter.array(authAccounts, columns, {});
	}, state.authAccounts, state.columns);
			...
			return [
				state.form.visible ?
					m(Form, {
						buttons: [
							{
								accent: true,
								name: AuthAccountState.translate(WebGlobalActionDeleteSessions),
							...
							{
								name: AuthAccountState.translate(WebGlobalActionResetPassword),
								onclick: async (): Promise<void> => {
									return AuthAccountState.createReset(state.form.data)
										.catch((err) => {
							...
							},
							{
								...
								onclick: async (): Promise<void> => {
									return AuthAccountState.delete(state.form.data.id)
										.then(async () => {
											state.form.visible = false;
											return update();
										});
						...
						onsubmit: async () => {
							...
							return aa.update(state.form.data)
								.then(async () => {
									state.form.visible = false;
									return update();
								});
						...
						m(FormItem, {
							input: {
								...
								oninput: (): void => {},
								type: "text",
								value: OIDCProviderType.values[state.form.data.oidcProviderType],
							},
				...
				m(Table, {
					...
					title: {
						...
						name: state.authAccounts().length === 0 ?
