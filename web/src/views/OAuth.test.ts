...
describe("OAuth", () => {
	test("amazon", async () => {
		...
		testing.text("#oauth-title", "Amazon Alexawants to access your Homechart account");
	});
	...
	test("google", async () => {
		...
		testing.text("#oauth-title", "Google Assistantwants to access your Homechart account");
	...
	window = Object.create(window); // eslint-disable-line
		...
		expect((testing.mocks.requests[0].body as AuthSession).name)
			.toBe("Google Assistant");

		expect(window.location.href)
			.toBe("https://oauth-redirect.googleusercontent.com/r/test#access_token=1_2_3&token_type=bearer&state=1");
	});
});
