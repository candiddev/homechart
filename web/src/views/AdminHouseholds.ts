...
import { ObjectHousehold, ObjectHouseholds, WebAdminHouseholdsLastTransactionID, WebAdminHouseholdsMemberCount, WebAdminHouseholdsSearch, WebAdminHouseholdsSubscriptionCustomerID, WebAdminHouseholdsSubscriptionID, WebAdminHouseholdsSubscriptionProcessor, WebFormSubscriptionExpires, WebFormSubscriptionSelfHostedID, WebGlobalAdmin,WebGlobalExpires, WebGlobalOf } from "../yaml8n";
...
export function AdminHouseholds (): m.Component {
	...
	async function update (): Promise<void> {
		...
		state.loaded = false;
		return AuthHouseholdState.readAdmin(state.filter, offset)
			.then((response) => {
				if (! IsErr(response)) {
	...
	const a = Stream.lift((households, columns) => {
		return Filter.array(households, columns, {});
	}, state.authHouseholds, state.columns);
			...
			return [
				state.form.visible ?
					m(Form, {
						...
						onsubmit: async () => {
							return AuthHouseholdState.update(state.form.data)
								.then(async () => {
									state.form.visible = false;
									return update();
								});
						...
						m(FormItem, {
							name: AuthAccountState.translate(WebAdminHouseholdsSubscriptionProcessor),
							select: {
								oninput: (e: string): void => {
									state.form.data.subscriptionProcessor = Translations.subscriptionProcessors.indexOf(e);
								},
								options: Translations.subscriptionProcessors,
								required: true,
								value: Translations.subscriptionProcessors[state.form.data.subscriptionProcessor],
							},
							tooltip: "",
						}),
						m(FormItem, {
							...
							name: AuthAccountState.translate(WebAdminHouseholdsSubscriptionCustomerID),
							tooltip: "",
						}),
						m(FormItem, {
							input: {
							...
							name: AuthAccountState.translate(WebAdminHouseholdsSubscriptionID),
							tooltip: "",
						}),
						m(FormItem, {
							input: {
							...
							name: AuthAccountState.translate(WebAdminHouseholdsLastTransactionID),
							tooltip: "",
						}),
						m(FormItem, {
							input: {
				...
				m(Table, {
					...
					tableColumns: [
						...
						},
						{
							formatter: (a: AuthHousehold): string => {
								return Translations.subscriptionProcessors[a.subscriptionProcessor];
							},
							name: AuthAccountState.translate(WebAdminHouseholdsSubscriptionProcessor),
					...
					title: {
						...
						buttonRight: state.page * 50 > state.total ?
							undefined :
							{
								href: `/admin/households?page=${state.page + 1}`,
								icon: Icons.Next,
						...
						name: state.authHouseholds().length === 0 ?
