...
import {
  ...
  ObjectMealPlan,
  ObjectRecipes,
  WebCookRecipesSearch,
  WebGlobalActionImportRecipeFromWebsite,
  WebGlobalActionImportRecipes,
  WebGlobalActionImportRecipesFromCSV,
  WebGlobalActionShowDeleted,
...
export function CookRecipes(): m.Component {
  function filterIngredients(ingredients: string, filter: string): boolean {
    if (ingredients !== null) {
      if (ingredients.toLowerCase().includes(filter)) {
        return true;
      }
    }
    return false;
                ...
                ? m(Icon, {
                    ...
                    onclick: async (e: m.Event<MouseEvent>) => {
                      ...
                      AppState.setComponentsDropdownMenu(
                        `cookrecipe_${StringToID(vnode.attrs.name)}`,
                        e.clientY,
                      );
                    },
                  })
  ...
  return {
    oninit: async (): Promise<void> => {
      ...
      recipes = Stream.lift(
        (recipestate, columns, deleted, search, sort, tag) => {
          const data = CookRecipeState.filter(
            recipestate,
            deleted,
            "",
            search,
            ...
            (recipe) => {
              return (
                ...
                filterIngredients(recipe.ingredients, search.toLowerCase())
              );
            },
          );
    ...
    view: (): m.Children => {
      return [
        m(FormImportCSV, {
          ...
          toggle: (v) => {
            state.formVisible = v ? visible.ImportCSV : visible.None;
          },
        ...
        m(FormShopItemIngredients, {
          ...
          toggle: (v): void => {
            state.formVisible = v ? visible.ShopItemIngredients : visible.None;
          },
