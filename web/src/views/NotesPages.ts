...
export function NotesPages(): m.Component {
  ...
  return {
    oninit: async (): Promise<void> => {
      data = Stream.lift(
        (pagestate, _versions, columns, deleted, filter, search, sort, tag) => {
          const data = NotesPageState.filter(
            pagestate,
            deleted,
            filter,
            search,
            ...
            (page) => {
              return (
                ...
                NotesPageVersionState.findNotesPageIDLatest(page.id)
                  .body.toLowerCase()
        ...
        toolbarActionButtons: [
          {
            ...
            onclick: (): void => {
              m.route.set(
                `/notes/${
                  state.filter() === "" ? "personal" : state.filter()
                }/new?edit`,
              );
            },
      ...
      return [
        m(Table, {
          ...
          tableColumns: [
            ...
            {
              formatter: (page: NotesPage): string => {
                return AppState.formatCivilDate(
