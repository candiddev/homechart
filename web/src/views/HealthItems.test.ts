...
describe("HealthItems", () => {
	test("inputs/table/form", async () => {
		...
		testing.text(`#table-data-${HealthItemState.findName("Beans").id}-name`, "Beans");
		testing.text(`#table-data-${seed.healthItems[1].id}-name`, "GingerRestless Legs (1)");
		testing.text(`#table-data-${seed.healthItems[1].id}-totalcorrelations`, "1");
		...
		testing.text(`#table-data-${seed.healthItems[1].id}-name`, `${seed.healthItems[1].name}1Restless Legs (1)`);
	...
	test("outputs", async () => {
		...
		testing.text(`#table-data-${HealthItemState.findName("Restless Legs").id}-name`, "Restless LegsPineapple (2)Ginger (1)");
	});
});
