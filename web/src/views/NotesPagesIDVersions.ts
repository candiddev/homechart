...
import { ObjectHousehold, ObjectNotes, ObjectVersions, WebGlobalCreator, WebGlobalPersonal } from "../yaml8n";
...
export function NotesPagesIDVersions (): m.Component {
	...
	return {
		oninit: async (): Promise<void> => {
			data = Stream.lift((_page, _pageVersions, columns, id, sort) => {
				...
				AppState.setLayoutApp({
					...
					breadcrumbs: [
						...
						{
							link: `/notes/${m.route.param().tag === "personal" ?
								"personal" :
								"household"}`,
							name: `${m.route.param()
								.tag === "personal" ?
				...
				return Filter.array(NotesPageVersionState.findNotesPageIDAll(id), columns, sort);
			}, NotesPageState.data, NotesPageVersionState.data, state.columns, state.id, state.sort);
		...
		view: (): m.Children => {
			return m(Table, {
				...
				editOnclick: (version: NotesPageVersion) => {
					m.route.set(`/notes/${m.route.param().tag}/${m.route.param().id}?version=${version.id}`);
				},
				filters: [],
