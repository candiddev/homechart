...
test("SecretsValues", async () => {
  ...
  testing.text(
    `#table-data-${seed.secretsValues[0].id}-name`,
    "Homecharttagjanetagapp",
  );
  testing.text(
    `#table-data-${seed.secretsValues[0].id}-username`,
    "********visibility_offcontent_paste",
  );
  testing.click(`#table-data-${seed.secretsValues[0].id}-username i`);
  testing.text(
    `#table-data-${seed.secretsValues[0].id}-username`,
    "janevisibilitycontent_paste",
  );
