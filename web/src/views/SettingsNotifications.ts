...
import { ObjectNotifications, WebAdminNotificationsNewsletter, WebGlobalDisabled, WebGlobalPersonal, WebGlobalSettings, WebGlobalType, WebSettingsNotificationsDailyAgenda, WebSettingsNotificationsDailyAgendaTime, WebSettingsNotificationsDailyAgendaTooltip, WebSettingsNotificationsDevice, WebSettingsNotificationsDisableDevice, WebSettingsNotificationsEmail, WebSettingsNotificationsEnableDevice, WebSettingsNotificationsEventReminders, WebSettingsNotificationsRecipeCookReminders, WebSettingsNotificationsRecipePrepReminders, WebSettingsNotificationsTaskCompleteUpdates, WebSettingsNotificationsTaskReminders } from "../yaml8n";
	...
	return {
				...
				const n: Notification[] = [
					{
						...
						type: AuthAccountState.translate(WebSettingsNotificationsDailyAgenda),
					},
					{
						deviceProperty: null,
		...
		view: (): m.Children => {
			return m(Form, {
				buttons: [
					{
						...
						name: AuthSessionState.data().webPush === null ?
							AuthAccountState.translate(WebSettingsNotificationsEnableDevice) :
							AuthAccountState.translate(WebSettingsNotificationsDisableDevice),
						onclick: async (): Promise<void | Err> => {
							return AuthSessionState.toggleNotifications()
								.then(() => {
									if ("Notification" in window && Notification.permission !== "granted") {
										AppState.setLayoutAppAlert({
											message: "Your device is blocking notifications, please allow them for Homechart",
										});
									}
								});
						},
				...
				m(FormItem, {
					name: AuthAccountState.translate(WebSettingsNotificationsDailyAgendaTime),
					select: {
						oninput: async (e: string) => {
							state.authAccount.dailyAgendaTime = CivilTime.fromString(`${`0${e}`.slice(-2)}:00`)
								.toString(true);
						...
						options: [
							...[
								...
								.map((_, n) => {
									return {
										id: `${n}`,
										name: CivilTime.fromString(`${`0${n}`.slice(-2)}:00`)
											.toString(AuthAccountState.data().preferences.formatTime24),
									};
								}),
						],
						value: state.authAccount.dailyAgendaNext === null ?
							AuthAccountState.translate(WebGlobalDisabled) :
							CivilTime.fromString(state.authAccount.dailyAgendaTime)
								.hour,
					},
					tooltip: AuthAccountState.translate(WebSettingsNotificationsDailyAgendaTooltip),
				}),
				m(Table, {
					actions: [],
					data: data(),
					...
					tableColumns: [
						...
						{
							checkboxOnclick: async (data: Notification): Promise<void | Err> => {
								const property = data.deviceProperty;
								if (property !== null) {
									if (data.authHouseholdID === undefined) {
										state.authAccount.preferences[property] = state.authAccount.preferences[property] === false;
									} else {
										if (state.authAccount.preferences.notificationsHouseholds === null || state.authAccount.preferences.notificationsHouseholds.length === 0) {
										...
										let i = state.authAccount.preferences.notificationsHouseholds.findIndex((ah) => {
											return ah.authHouseholdID === data.authHouseholdID;
										});

										if (i < 0) {
											i = state.authAccount.preferences.notificationsHouseholds.push({
												...AuthAccountState.newNotificationsHouseholds(),
												...{
													authHouseholdID: data.authHouseholdID,
												},
										...
										state.authAccount.preferences.notificationsHouseholds[i][property] = state.authAccount.preferences.notificationsHouseholds[i][property] === false;
									}
							...
							formatter: (n: Notification): boolean | string => {
								...
								if (n.authHouseholdID === undefined) {
									return ! (state.authAccount.preferences[n.deviceProperty] as boolean);
								}

								return ! (AuthAccountState.findNotificationsHousehold(state.authAccount.preferences.notificationsHouseholds, n.authHouseholdID)[n.deviceProperty] as boolean);
							},
							name: AuthAccountState.translate(WebSettingsNotificationsDevice),
							permitted: (): boolean => {
								return GlobalState.permitted(PermissionComponentsEnum.Auth, true);
							},
							property: "device",
						...
						},
						{
							checkboxOnclick: async (data: Notification): Promise<void | Err> => {
								...
								if (property !== null) {
									if (data.authHouseholdID === undefined) {
										...
										if (state.authAccount.preferences.notificationsHouseholds === null || state.authAccount.preferences.notificationsHouseholds === undefined) {
											state.authAccount.preferences.notificationsHouseholds = [];
										}
							...
							formatter: (n: Notification): boolean | string => {
								...
								if (n.authHouseholdID === undefined) {
									return ! (state.authAccount.preferences[n.emailProperty] as boolean);
								}

								return ! (AuthAccountState.findNotificationsHousehold(state.authAccount.preferences.notificationsHouseholds, n.authHouseholdID)[n.emailProperty] as boolean);
							},
