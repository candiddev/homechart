...
test("SecretsVaults", async () => {
	...
	testing.text(`#table-data-${seed.secretsVaults[1].id}-name`, seed.secretsVaults[1].name);
	testing.text(`#table-data-${seed.secretsVaults[1].id}-icon`, seed.secretsVaults[1].icon);
	...
	expect(testing.mocks.route)
		.toBe("/secrets?filter=personal");
	testing.click("#tab-household");
	expect(testing.mocks.route)
		.toBe("/secrets?filter=household");
