...
describe("CookRecipesID", () => {
	test("details", async () => {
		...
		testing.text("#breadcrumbs", `home > Cook > Recipes > ${seed.cookRecipes[0].name}`);

		testing.find("#form-image-image");
		testing.text("#form-item-input-servings", `${seed.cookRecipes[0].servings}`);
		...
		testing.text("#form-item-text-area-ingredients p", CookRecipeState.scaleIngredients(seed.cookRecipes[0].ingredients, "2")
			.replace(/\n/g, ""));
		testing.text("#form-item-input-times-made", `${seed.cookRecipes[0].cookMealPlanCount}`);
		testing.text("#form-item-input-last-made", `${AppState.formatCivilDate(seed.cookRecipes[0].cookMealPlanLast)}`);
	...
	test("ID - unauthenticated", async () => {
		...
		expect(testing.mocks.route)
			.toBe(`/cook/recipes/${seed.cookRecipes[0].id}`);
		testing.redraw();
		testing.title(`Cook - Recipes - ${seed.cookRecipes[0].name}`);
		testing.text("#breadcrumbs", `home > Cook > Recipes > ${seed.cookRecipes[0].name}`);
	...
	test("delete", async () => {
		...
		expect(CookRecipeState.findName(seed.cookRecipes[0].name).deleted)
			.not.toBeNull();
		expect(testing.mocks.route)
			.toBe("/cook/recipes");
	});
