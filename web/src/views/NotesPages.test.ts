...
describe("NotesPages", () => {
	...
	test.each([
		[
			...
			AppState.formatCivilDate(Timestamp.fromString(seed.notesPageVersions[2].updated)
				.toCivilDate()
				.toJSON()),
		],
		[
			...
			AppState.formatCivilDate(Timestamp.fromString(seed.notesPageVersions[5].updated)
				.toCivilDate()
				.toJSON()),
		],
		[
			...
			"",
			AppState.formatCivilDate(Timestamp.fromString(seed.notesPageVersions[4].updated)
				.toCivilDate()
				.toJSON()),
		],
		[
			...
			"Deletedtagatagb",
			"",
			AppState.formatCivilDate(Timestamp.now()
				.toCivilDate()
				.toJSON()),
		],
	])("routes.%s", async (_name, params, rows, row0name, row0parent, row0updated) => {
	...
	test("search", async () => {
		...
		await testing.sleep(1200);
		testing.text(`#table-data-${seed.notesPages[1].id}-name`, `${seed.notesPages[1].name}tag${seed.notesPages[1].tags.join("tag")}`);
	});
});
