      ...
      view: (vnode): m.Children => {
        ...
        return m("div.Home__change", [
          m("p", [
            m(
              "span",
              `${member.name === "" ? member.emailAddress : member.name} `,
            ),
            m(Markdown, {
              ...
              ? events().length > 0
                ...
                : [
                    m("div.Home__nothing", [
                      ...
                      m(
                        "p.Home__agenda",
                        AuthAccountState.translate(WebHomeAgendaNothing),
                      ),
                    ]),
                  ]
              : [...Array(5)].map(() => {
          ...
          m("div.Home__right", [
            BookmarkState.data().length > 0
              ? m("div.Home__bookmarks", [
                  ...
                  m(
                    "div.Home__bookmarks-buttons",
                    BookmarkState.data().map((bookmark) => {
                      if (!bookmark.home) {
                        return null;
                      }

                      return m(
                        bookmark.link.startsWith("/")
                          ? m.route.Link
            ...
            m(Table, {
              ...
              tableColumns: [
                {
                  formatter: (c: { updated: NullTimestamp }): string => {
                    return Timestamp.fromString(`${c.updated}`).toPrettyString(
