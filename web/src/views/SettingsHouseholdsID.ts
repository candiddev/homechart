...
import { ActionCancel, ActionDelete, ActionDeleteConfirm, ActionUpdate, FormItemSelectColorName } from "@lib/yaml8n";
...
import { ObjectHousehold, ObjectHouseholds, ObjectMealPlan, ObjectMembers, ObjectRecurringTransaction, ObjectTask, WebGlobalActionLeaveHousehold, WebGlobalHideComponents, WebGlobalName, WebGlobalNameTooltip, WebGlobalPreferences, WebGlobalSettings, WebSettingsHouseholdExportHousehold, WebSettingsHouseholdImportHousehold } from "../yaml8n";
				...
				m(Form, {
					buttons: state.tab === tabs.preferences ?
						state.confirm === confirmation.None
							? [
								{
									...
									permitted: Permission.isPermitted(AuthSessionState.data().permissionsHouseholds, PermissionComponentsEnum.Auth, PermissionEnum.Edit, state.authHousehold.id),
									requireOnline: true,
								},
								{
									name: AuthAccountState.translate(WebGlobalActionLeaveHousehold),
									onclick: async (): Promise<void> => {
										return new Promise((resolve) => {
								...
								{
									name: AuthAccountState.translate(WebSettingsHouseholdExportHousehold),
									onclick: async (): Promise<void> => {
										return fetch(`/api/v1/auth/households/${state.authHousehold.id}/export`, {
											headers: {
									...
									permitted: GlobalState.permitted(PermissionComponentsEnum.Auth, true, state.authHousehold.id),
									requireOnline: true,
								},
								InfoState.data().cloud ?
									null :
									{
										name: AuthAccountState.translate(WebSettingsHouseholdImportHousehold),
										oninput: async (file): Promise<void> => {
											return fetch(`/api/v1/auth/households/${state.authHousehold.id}/import`, {
												body: file,
												headers: {
													"x-homechart-id": apiEndpoint().id,
													"x-homechart-key": apiEndpoint().key,
							...
							[
								...
								{
									accent: true,
									name: `${AuthAccountState.translate(ActionDeleteConfirm)} ${state.confirm === confirmation.Delete ?
										`${AuthAccountState.translate(ActionDelete)} ${AuthAccountState.translate(ObjectHousehold)}` :
										AuthAccountState.translate(WebGlobalActionLeaveHousehold)}`,
									onclick: async (): Promise<void> => {
										if (state.confirm === confirmation.Delete) {

											return AuthHouseholdState.delete(state.authHousehold.id)
												.then(async () => {
										...
										return AuthAccountAuthHouseholdState.delete(state.authHousehold.id, AuthAccountState.data().id)
											.then(async () => {
												SSEState.clearRefreshed();
					...
					[
						...
						m(FormItemSelectColor, {
							disabled: !GlobalState.permitted(PermissionComponentsEnum.Auth, true, state.authHousehold.id),
