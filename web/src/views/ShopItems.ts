...
import { ObjectCategories, ObjectCategory, ObjectList, ObjectListPickUp, ObjectLists, ObjectMeal, ObjectProject, ObjectRecipe, ObjectShop, ObjectStore, WebFormItemsShopItemPrice, WebGlobalActionSkip, WebGlobalAll, WebGlobalBudgeted, WebGlobalName, WebGlobalPriority, WebGlobalSearch, WebShopItemsClearCart, WebShopItemsClearCartSucccess, WebShopItemsClearPurchased, WebShopItemsClearPurchasedSuccess, WebShopItemsCost, WebShopItemsInCart, WebShopItemsPurchased, WebShopItemsToGet } from "../yaml8n";
				...
				vnode.attrs.recurrence === null ?
					...
					m(Button, {
						...
						onclick: async (): Promise<void | Err> => {
							...
							data.nextDate = Recurrence.nextCivilDate(data.recurrence as RecurrenceInterval, CivilDate.fromString(`${data.nextDate}`))
								.toJSON();
...
export function ShopItems (): m.Component {
	const state = {
		...
		search: Stream(""),
		sort: Stream({ // eslint-disable-line @typescript-eslint/consistent-type-assertions
	...
	return {
		oninit: async (): Promise<void> => {
			...
			data = Stream.lift((_stores, _categories, _lists, _items, categoryID, columns, search, sort, status, listID, storeID) => {
				...
				if (listID !== null) {
					...
					newItems = ShopItemState.data()
						.filter((item: ShopItem) => {
							return item.shopListID === state.list().id;
						});
				} else if (storeID !== null) { // eslint-disable-line no-negated-condition
					...
					newItems = ShopItemState.getPickupItems()
						.filter((item: ShopItem) => {
							return item.budgetPayeeID === state.store().id || item.budgetPayeeID === null;
						});
				} else {
					breadcrumbs = [
						{
				...
				newItems = newItems.filter((item: ShopItem) => {
					...
					if (search !== "" && ! item.name.toLowerCase()
				...
				state.positions = Position.sort(newItems)
					.map((newItems) => {
						return newItems.id;
					});
			...
			}, BudgetPayeeState.data, ShopCategoryState.data, ShopListState.data, ShopItemState.data, state.categoryID, state.columns, state.search, state.sort, state.status, state.listID, state.storeID);
		...
		view: (): m.Children => {
			const budgetCategory = BudgetCategoryState.findID(state.list().budgetCategoryID);
			...
			return m(Table, {
				actions: [
					{
						...
						onclick: async (): Promise<void> => {
							for (const item of data()
								...
								if (item.inCart === true) {
									if (item.recurrence !== null && item.nextDate !== null) {
										item.nextDate = Recurrence.nextCivilDate(item.recurrence, CivilDate.fromString(item.nextDate))
											.toJSON();
										item.inCart = false;
							...
							AppState.setLayoutAppAlert({
								message: state.listID() === null ?
									AuthAccountState.translate(WebShopItemsClearCartSucccess) :
									AuthAccountState.translate(WebShopItemsClearPurchasedSuccess),
							});
						},
						permitted: GlobalState.permitted(PermissionComponentsEnum.Shop, true) && data()
							.filter((item) => {
								return item.inCart;
							}).length > 0 && state.status() !== "toget",
						requireOnline: true,
					},
					...state.listID() === null ?
						[] :
						[
							{
								...
								onclick: async (): Promise<void> => {
									return new Promise((resolve) => {
										AppState.setLayoutAppForm(FormOverlayShopList, state.list());

										return resolve();
									});
								},
								permitted: GlobalState.permitted(PermissionComponentsEnum.Shop, true, state.list().authHouseholdID),
								requireOnline: true,
							},
						],
				],
				...
				loaded: BudgetPayeeState.isLoaded() && PlanProjectState.isLoaded() && ShopCategoryState.isLoaded() && ShopItemState.isLoaded(),
				search: {
				...
				tableColumns: [
					{
						...
						ondragend: async (): Promise<void | Err> => {
							if (Drag.state.target !== "") {
								const source = ShopItemState.findID(Drag.state.source.split("_")[1]);
								const target = ShopItemState.findID(Drag.state.target.split("_")[1]);

								source.position = Position.adjacent(target, ShopItemState.findAdjacent(target), Drag.state.before);
						...
						permitted: (s: ShopItem): boolean => {
							return GlobalState.permitted(PermissionComponentsEnum.Shop, true, s.authHouseholdID);
						},
						property: "position",
					...
					{
						currencyFormat: AuthHouseholdState.findID(AuthAccountState.data().primaryAuthHouseholdID).preferences.currency,
						formatter: (s: ShopItem): number | string => {
							return s.price === undefined || s.price === 0 ?
								"" :
								-1 * s.price;
						},
					...
					{
						formatter: (e: ShopItem): string => {
							return CookRecipeState.findID(CookMealPlanState.findID(e.cookMealPlanID).cookRecipeID).name;
						},
						linkFormatter: (e: ShopItem): string => {
					...
					subtitles:
						[
							...state.list().budgetCategoryID === null ?
								...
								[
									{
										color: budgetCategory.budgetMonthCategoryAmount + budgetCategory.budgetTransactionAmount > 0 ?
											"var(--color_positive)" :
											"var(--color_negative)",
										key: AuthAccountState.translate(WebGlobalBudgeted),
										value: Currency.toString(budgetCategory.budgetMonthCategoryAmount + budgetCategory.budgetTransactionAmount, AuthHouseholdState.findID(AuthAccountState.data().primaryAuthHouseholdID).preferences.currency),
									},
								],
							...state.amount > 0 ?
								[
									{
										color: budgetCategory.budgetMonthCategoryAmount + budgetCategory.budgetTransactionAmount >= state.amount ?
											"var(--color_positive)" :
											"var(--color_negative)",
										key: AuthAccountState.translate(WebShopItemsCost),
										value: Currency.toString(state.amount, AuthHouseholdState.findID(AuthAccountState.data().primaryAuthHouseholdID).preferences.currency),
									},
								] :
								[],
						],
						...
						[
							{
								...
								href: `/shop/items${m.route.param().store === undefined ?
									"" :
							...
							},
							{
								...
								href: `${m.parsePathname(m.route.get()).path}?filter=incart${m.route.param().store === undefined ?
									"" :
									`&store=${m.route.param().store}`}`,
							...
							},
							{
								...
								href: `/shop/items?filter=toget${m.route.param().store === undefined ?
									"" :
									`&store=${m.route.param().store}`}`,
