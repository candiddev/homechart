...
test("CookMealTimes", async () => {
  ...
  testing.text(
    `#table-data-${seed.cookMealTimes[3].id}-name`,
    seed.cookMealTimes[3].name,
  );
  testing.text(
    `#table-data-${seed.cookMealTimes[3].id}-time`,
    CivilTime.fromString(seed.cookMealTimes[3].time).toString(false),
  );
