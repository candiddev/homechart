...
test("CookRecipes", async () => {
  ...
  testing.text(
    `#table-data-${seed.cookRecipes[0].id}-name`,
    `${seed.cookRecipes[0].name}addtag${seed.cookRecipes[0].tags.join("tag")}`,
  );
  testing.click(
    `#table-data-${seed.cookRecipes[0].id}-name .GlobalButtonIconAdd`,
  );
  ...
  testing.text(
    `#table-data-${seed.cookRecipes[0].id}-timecooktimeprep`,
    Duration.toString(
      seed.cookRecipes[0].timeCook + seed.cookRecipes[0].timePrep,
    ),
  );
  ...
  testing.text(
    `#table-data-${seed.cookRecipes[1].id}-name`,
    `${seed.cookRecipes[1].name}addtag${seed.cookRecipes[1].tags.join("tag")}`,
  );
  ...
  await testing.sleep(100);
  testing.text(
    `#table-data-${seed.cookRecipes[3].id}-name`,
    `${seed.cookRecipes[3].name}addtag${seed.cookRecipes[3].tags.join("tag")}`,
  );
  testing.findAll("tbody tr", 1);
  testing.click("#button-array-show-tag-coffee");
  await testing.sleep(100);
