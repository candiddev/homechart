...
describe("BudgetTransactions", () => {
	...
	test("renders OK", async () => {
		const transactions: BudgetTransaction[] = Array(50)
			.fill(transaction);
		...
		testing.text(`#table-data-${seed.budgetRecurrences[0].id}-nextdate`, `${AppState.formatCivilDate(seed.budgetRecurrences[0].template.date)}AddSkip`);
		testing.text(`#table-data-${seed.budgetRecurrences[0].id}-payee`, seed.budgetPayees[1].name);
		testing.text(`#table-data-${seed.budgetRecurrences[0].id}-category`, `Income > ${seed.budgetPayees[1].name}`);
		testing.text(`#table-data-${seed.budgetRecurrences[0].id}-amount`, Currency.toString(seed.budgetRecurrences[0].template.accounts[0].amount, AuthHouseholdState.data()[0].preferences.currency));
		testing.notFind("#button-cancel");
		testing.click(`#table-transactions-row_${seed.budgetRecurrences[0].id}`);
		await testing.sleep(100);
		testing.value("#form-item-input-date", seed.budgetRecurrences[0].template.date);
		...
		testing.mount(App, routeOptions, BudgetTransactions);
		await testing.sleep(500);
		testing.text("#subtitle-total", `Total:${Currency.toString(account.budgetTransactionAmount, AuthHouseholdState.data()[0].preferences.currency)}`);
		testing.text("#subtitle-cleared", `Cleared:${Currency.toString(account.budgetTransactionAmountCleared, AuthHouseholdState.data()[0].preferences.currency)}`);
		testing.text("#subtitle-reconciled", `Reconciled:${Currency.toString(account.budgetTransactionAmountReconciled, AuthHouseholdState.data()[0].preferences.currency)}`);
		testing.find("#table-transactions");
		testing.findAll("tbody tr", 50);
		testing.text(`#table-data-${seed.budgetTransactions[0].id}-date`, AppState.formatCivilDate(transaction.date));
		testing.text(`#table-data-${seed.budgetTransactions[0].id}-payee`, BudgetPayeeState.findID(seed.budgetTransactions[0].budgetPayeeID).name);
		testing.text(`#table-data-${seed.budgetTransactions[0].id}-category`, BudgetCategoryState.findIDHeaderName(seed.budgetTransactions[0].categories[0].budgetCategoryID));
		testing.text(`#table-data-${seed.budgetTransactions[0].id}-cleared`, "check_box");
		...
		expect(testing.mocks.requests)
			.not.toBe([]);
		testing.text(`#table-data-${seed.budgetTransactions[0].id}-reconciled`, "check_box_outline_blank");
		testing.mocks.requests = [];
		...
		expect(testing.mocks.requests)
			.not.toBe([]);
		testing.text(`#table-data-${seed.budgetTransactions[0].id}-amount`, Currency.toString(seed.budgetTransactions[0].accounts[0].amount, AuthHouseholdState.data()[0].preferences.currency));
		testing.text(`#table-data-${seed.budgetTransactions[0].id}-balance`, Currency.toString(seed.budgetTransactions[0].balance, AuthHouseholdState.data()[0].preferences.currency));
		testing.notFind("#button-cancel");
	...
	test("render import", async () => {
		const transactions: BudgetTransaction[] = Array(75)
			.fill(seed.budgetTransactions[0]);
		...
		testing.text(`#table-data-${seed.budgetTransactions[0].id}-date`, AppState.formatCivilDate(seed.budgetTransactions[0].date));
		testing.text(`#table-data-${seed.budgetTransactions[0].id}-payee`, BudgetPayeeState.findID(seed.budgetTransactions[0].budgetPayeeID).name);
		testing.text(`#table-data-${seed.budgetTransactions[0].id}-category`, BudgetCategoryState.findIDHeaderName(seed.budgetTransactions[0].categories[0].budgetCategoryID));
