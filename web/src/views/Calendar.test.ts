...
describe("Calendar", () => {
	...
	test("picker", async () => {
		...
		testing.value(`#${title.id} input`, now.toCivilDate()
			.toYearMonth()
			.toValue());

		testing.text(testing.findAll("#picker .Calendar__date--day")[now.getWeekday() === 7 ?
			0 :
			now.getWeekday()], Weekday.values[now.getWeekday()][0]);
		let range = now.toCivilDateRangeCalendar(false);
		testing.text(testing.findAll("#picker .Calendar__date--active")[now.getWeekday() === 7 ?
			0 :
			now.getWeekday()], `${now.toCivilDate()
			.day}•••••`);
		if (range.to.day !== now.toCivilDate().day) {
			testing.text(testing.find("#picker").lastChild as Element, `${range.to.day}•`);
		}

		const date = Timestamp.fromCivilDate(CivilDate.fromString("2020-01-01"));
		testing.input("#picker-title input", "2020-01");
		testing.value("#picker-title input", date.toCivilDate()
			.toYearMonth()
			.toValue());
	...
	test("current date", async () => {
		...
		testing.find(`#button${StringToID(AppState.formatCivilDate(previous.toCivilDate()))}`);
		testing.find(`#button${StringToID(AppState.formatCivilDate(next.toCivilDate()))}`);
		...
		testing.findAll(".CalendarDay__items", Timestamp.getCivilDatesFromDateRange(month).length);
	});
	...
	test("icalendar", async () => {
		...
		testing.click(`#table-icalendars-row_${CalendarICalendarState.data()[0].id}`);
