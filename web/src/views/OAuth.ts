...
import {
  WebGlobalActionAllow,
  WebGlobalActionDeny,
  WebOAuthGrant,
  WebOAuthRequest,
  WebOAuthRequestFooter,
  WebOAuthUnknown,
} from "../yaml8n";
...
const assistantNames = ["", "Amazon Alexa", "Google Assistant"];
  ...
  return {
    oninit: async (): Promise<void> => {
      ...
      switch (m.route.param().client_id) {
        case "amazon_alexa":
          if (
            redirectURI.startsWith("https://alexa.amazon.co.jp/spa/skill") ||
            redirectURI.startsWith("https://pitangui.amazon.com/spa/skill") ||
            redirectURI.startsWith("https://layla.amazon.com/spa/skill")
          ) {
      ...
      return m(
        Form,
        {
          ...
          onsubmit: async () => {
            ...
            return authSession.create().then(() => {
              window.location.href = `${m.route.param().redirect_uri}#access_token=${
                InfoState.data().cloud
                  ? ""
                  : AuthAccountState.data().primaryAuthHouseholdID
              }_${authSession.data().id}_${authSession.data().key}&token_type=bearer&state=${m.route.param().state}`;
            });
          },
        },
        [
