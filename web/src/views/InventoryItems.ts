...
import { ObjectInventory, WebFormOverlayInventoryItemQuantity, WebGlobalAllItems, WebGlobalImage, WebGlobalLocation, WebGlobalName, WebInventoryImportItems, WebInventoryItemFromBarcode, WebInventoryItemFromBarcodeSuccess, WebInventoryItemsEditCollection, WebInventoryItemsImportFromCSV, WebInventoryItemsResetCollection, WebInventoryItemsSearch, WebInventoryItemsStopScanning, WebInventoryItemsUpdateCollection } from "../yaml8n";
		...
		view: (vnode): m.Children => {
			return m("div.InventoryItems__buttons", [
				m(Icon, {
					...
					style: {
						visibility: vnode.attrs.quantity > 0 && GlobalState.permitted(PermissionComponentsEnum.Inventory, true) ?
							undefined :
							"hidden",
					},
				}),
				...
				GlobalState.permitted(PermissionComponentsEnum.Inventory, true) ?
					m(Icon, {
						...
						style: {
							visibility: GlobalState.permitted(PermissionComponentsEnum.Inventory, true) ?
								undefined :
								"hidden",
						},
					}) :
				...
				GlobalState.permitted(PermissionComponentsEnum.Shop, true) ?
					m(Icon, {
						classes: `GlobalButtonIconAdd${ShopItemState.findName(vnode.attrs.name).name === "" ?
							"" :
...
export function InventoryItems (): m.Component {
	...
	const state = {
		...
		household: AuthHouseholdState.findID(AuthAccountState.data().primaryAuthHouseholdID),
		scanning: false,
		search: Stream(""),
		sort: Stream({
			invert: false,
	...
	return {
		oninit: async (): Promise<void> => {
			Stream.lift((_inventoryCollection, id) => {
				if (id === "all" || id === "") {
					state.collection = InventoryCollectionState.new();
					state.collection.columns = Clone(defaultColumns);
					state.collection.name = AuthAccountState.translate(WebGlobalAllItems);
				} else {
					...
					toolbarActionButtons: [
						{
							...
							...{
								onclick: (): void => {
									AppState.setLayoutAppForm(FormOverlayInventoryItem, setFormInventoryItemStateProperties());
								},
							},
						},
						{
							...
							onclick: async (): Promise<void> => {
									...
									if (result !==  null) {
										if (result.boxes !== null) {
											...
											result.boxes.filter((box) => {
												...
												.forEach((box) => {
													Quagga.ImageDebug.drawPath(box, { x: 0,
														y: 1 }, drawingCtx, { color: "green",
														lineWidth: 2 });
												});
										}

										if (result.box !== undefined) {
											Quagga.ImageDebug.drawPath(result.box, { x: 0,
												y: 1 }, drawingCtx, { color: "#00F",
												lineWidth: 2 });
										}

										if (result.codeResult !== undefined && result.codeResult.code !== undefined) {
											Quagga.ImageDebug.drawPath(result.line, { x: "x",
												y: "y" }, drawingCtx, { color: "red",
												lineWidth: 3 });
										}
									}
								...
								Quagga.onDetected(async (result) => {
									if (result.codeResult.code !== null && ! state.wait) {
										...
										let item =  InventoryItemState.findUPC(result.codeResult.code);

										if (item.id === null) {
											item = setFormInventoryItemStateProperties();
											item.upc = result.codeResult.code;
											AppState.setLayoutAppForm(FormOverlayInventoryItem, item);
										} else {
											item.quantity++;
											return InventoryItemState.update(item, true)
												.then(() => {
													AppState.setLayoutAppAlert({
							...
							permitted: GlobalState.permitted(PermissionComponentsEnum.Inventory, true),
							requireOnline: true,
						},
						{
							icon: Icons.ImportExport,
							name: AuthAccountState.translate(WebInventoryItemsImportFromCSV),
							onclick: (): void => {
				...
				m.redraw();
			}, InventoryCollectionState.data, state.collectionID);

			data = Stream.lift((items, columns, search, sort) => {
			...
			}, InventoryItemState.data, state.columns, state.search, state.sort);
					...
					actions: [
						{
							accent: true,
							icon: Icons.Cancel,
							name: AuthAccountState.translate(WebInventoryItemsResetCollection),
							onclick: async (): Promise<void> => {
								return new Promise((resolve) => {
									state.columns(InventoryCollectionState.findID(state.collection.id).columns);

									return resolve();
								});
							},
							permitted: GlobalState.permitted(PermissionComponentsEnum.Inventory, true) && state.collection.id !== null && JSON.stringify(state.columns()) !== JSON.stringify(InventoryCollectionState.findID(state.collection.id).columns),
							requireOnline: true,
						},
						{
							icon: Icons.Edit,
							...
							permitted: GlobalState.permitted(PermissionComponentsEnum.Inventory, true) && state.collection.id !== null,
							requireOnline: true,
						},
						{
							icon: Icons.Save,
							name: AuthAccountState.translate(WebInventoryItemsUpdateCollection),
							onclick: async (): Promise<void> => {
								state.collection.columns = state.columns();

								return InventoryCollectionState.update(state.collection)
									.then(() => {
										state.columns(InventoryCollectionState.findID(state.collection.id).columns);
									});
							},
