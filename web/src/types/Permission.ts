...
export const Permission = {
	...
	isPermitted: (permissions: PermissionsHousehold[] | null, cmp: PermissionComponentsEnum, p: PermissionEnum, authHouseholdID?: NullUUID): boolean => {
		if (permissions !== null) {
			for (const permission of permissions) {
				if (permission.permissions[Permission.components[cmp]] <= p && (authHouseholdID === undefined || permission.authHouseholdID === authHouseholdID)) {
					return true;
				}
			}
		}
