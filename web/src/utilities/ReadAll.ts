...
export async function ReadAll<T extends Data>(
  args: ReadAllArguments<T>,
): Promise<ReadAllResponse<T> | Err> {
  let hash = args.hash;
  let newData: T[] = [];
  ...
  if (args.data.length === 0) {
    ...
    if (
      ...
      result.length > 0 &&
      Object.keys(result[0]).length === Object.keys(args.newObj).length
    ) {
      newData = result;
  ...
  return API.read(args.path, {
    ...
    if (response.dataType !== args.typeArray) {
      NewErr(
        `ReadAll: unknown API response for ${args.typeArray}: ${response.dataType} ${JSON.stringify(response.dataValue)}`,
      );
    ...
    if (response.dataIDs.length > 0) {
      const merge = await MergeData(
        newData,
        args.path,
        response.dataValue as T[],
        response.dataIDs,
