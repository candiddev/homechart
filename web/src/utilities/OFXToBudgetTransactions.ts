...
function parseTransaction(
  ...
  transaction.budgetPayeeID = BudgetPayeeState.findName(
  ...
  if (transaction.budgetPayeeID !== null) {
    transaction.budgetPayeeName = "";
    const categoryID = BudgetPayeeState.findID(
...
export async function OFXToBudgetTransactions(
  data: string,
  budgetAccountID: NullUUID,
  ...
  xml2js.parseString(xmlString, (_err: string, result: OFX) => {
    if (result.OFX.BANKMSGSRSV1 !== undefined) {
      for (const transaction of result.OFX.BANKMSGSRSV1[0].STMTTRNRS[0]
        .STMTRS[0].BANKTRANLIST[0].STMTTRN) {
        transactions.push(parseTransaction(transaction, budgetAccountID));
      }
    } else if (result.OFX.CREDITCARDMSGSRSV1 !== undefined) {
      for (const transaction of result.OFX.CREDITCARDMSGSRSV1[0].CCSTMTTRNRS[0]
        .CCSTMTRS[0].BANKTRANLIST[0].STMTTRN) {
        transactions.push(parseTransaction(transaction, budgetAccountID));
      }
    }
  });
