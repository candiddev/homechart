...
describe("MergeData", () => {
	...
	test("deletes only", async () => {
		...
		const got = await MergeData(mergeClass.data(), mergeClass.path, newData, ids);
		Sort(got.data, {
			invert: mergeClass.sortInvert,
			property: mergeClass.sortField,
		});
	...
	test("no changes", async () => {
		...
		const output = await MergeData(mergeClass.data(), mergeClass.path, newData, ids);
		Sort(output.data, {
			invert: mergeClass.sortInvert,
			property: mergeClass.sortField,
		});
		expect(output.data)
			.toStrictEqual(mergeClass.data());
	...
	test("full sync", async () => {
		...
		const newData: Data[] = [];
		const ids: APIResponseDataID[] = Array.from(Array(10)
			.keys())
			.map((i) => {
				return {
					id: `${i + 1}`,
		...
		const readData = Array.from(Array(10)
			.keys())
			.map((i) => {
				return {
					...mergeClass.new(),
