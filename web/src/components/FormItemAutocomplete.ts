...
import { ObjectAccount, ObjectBookmark, ObjectBudget, ObjectCard, ObjectCategory, ObjectCollection, ObjectCook, ObjectInventory, ObjectItem, ObjectList, ObjectNotes, ObjectPage, ObjectPayee, ObjectPlan, ObjectProject, ObjectRecipe, ObjectReward, ObjectSecrets, ObjectShop, ObjectStore, ObjectTask, ObjectValue, ObjectVault } from "../yaml8n";
...
export const FormItemAutocomplete = {
	...
	parse: (input: string): FormItemAutocompleteParseOutput => {
		...
		if (FormItemAutocomplete.type === "") {
			output.splice= output.splice.toLowerCase()
				.replace(" > ", "");
			...
			case "budgetaccount":
				...
				break;
			case "budgetcategory":
				output.splice = BudgetCategoryState.findGroupName(output.splice).shortID;
				break;
			case "budgetpayee":
			...
			case "cookrecipe":
				...
				break;
			case "inventorycollection":
				output.splice = InventoryCollectionState.findName(output.splice).shortID;
				break;
			case "inventoryitem":
