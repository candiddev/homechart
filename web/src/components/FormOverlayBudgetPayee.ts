...
import { ObjectPayee, ObjectStore, WebFormContactUsEmailAddressTooltip, WebFormOverlayBudgetPayeeAddress, WebFormOverlayBudgetPayeeDefaultCategory, WebFormOverlayBudgetPayeeDefaultCategoryTooltip, WebFormOverlayBudgetPayeeShoppingStore, WebGlobalName, WebGlobalNameTooltip } from "../yaml8n";

export function FormOverlayBudgetPayee (): m.Component<FormOverlayComponentAttrs<BudgetPayee>> {
	return {
			...
			return m(FormOverlay, {
				...
				m.route.get()
					...
					m(FormItem, {
						...
						select: {
							oninput: (e: string): void => {
								vnode.attrs.data.budgetCategoryID = BudgetCategoryState.findGroupName(e).id;
							},
							options: [
								"",
								...BudgetCategoryState.names(),
							],
							value: BudgetCategoryState.findIDHeaderName(vnode.attrs.data.budgetCategoryID),
						},
						tooltip: AuthAccountState.translate(WebFormOverlayBudgetPayeeDefaultCategoryTooltip),
					}),
				m(FormCheckbox, {
					name: AuthAccountState.translate(WebFormOverlayBudgetPayeeShoppingStore),
					onclick: () => {
