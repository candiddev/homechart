...
test("FormItemSelectAuthHousehold", async () => {
  ...
  const smith = testing.find(
    `#button-array-owner-${seed.authHouseholds[1].id}`,
  );
  const personal = testing.find(
    `#button-array-owner-${seed.authAccounts[0].id}`,
  );
  ...
  testing.click(smith);
  expect(input.authAccountID).toBeNull();
  expect(input.authHouseholdID).toBe(seed.authHouseholds[1].id);
  ...
  expect(input.authHouseholdID).toBe(null);
