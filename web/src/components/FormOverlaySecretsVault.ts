...
import {
  ObjectVault,
  WebFormOverlaySecretsVaultHouseholdAccess,
  WebFormOverlaySecretsVaultHouseholdAccessTooltip,
  WebGlobalName,
  WebGlobalNameTooltip,
} from "../yaml8n";
import { FormItemSelectAuthHouseholdMembers } from "./FormItemSelectAuthHouseholdMembers";

export function FormOverlaySecretsVault(): m.Component<
  FormOverlayComponentAttrs<SecretsVault>
> {
  let denied = false;
  ...
  return {
    ...
    oninit: (vnode): void => {
      denied =
        ...
        SecretsVaultState.findKeyIndex(
      ...
      return m(
        FormOverlay,
        {
          ...
          onDelete: async (): Promise<void> => {
            return SecretsVaultState.delete(vnode.attrs.data.id).then(() => {
              if (m.route.get().includes(`/secrets/${vnode.attrs.data.id}`)) {
                m.route.set("/secrets", "", {
                  state: {
                    key: Date.now(),
                  },
          ...
          onSubmit: async (): Promise<SecretsVault | void | Err> => {
            let key: Key | Err =
              SecretsVaultState.keys()[`${vnode.attrs.data.id}`];
            if (key === undefined) {
            ...
            for (let i = 0; i < initMembers.length; i++) {
              const k = ParseKey(
                initMembers[i] === AuthAccountState.data().id
        ...
        [
          ...
          vnode.attrs.data.authAccountID === null
            ? m(FormItemSelectAuthHouseholdMembers, {
                ...
                memberNames: AuthHouseholdState.members().reduce(
                  (names, member) => {
                    if (
                      member.authHouseholdID ===
                        vnode.attrs.data.authHouseholdID &&
                      member.publicKey !== ""
                    ) {
