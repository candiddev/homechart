...
test("FormShopItem", async () => {
	...
	expect(shopItem.shopCategoryID)
		.toBe(seed.shopCategories[0].id);
	testing.findAll("#form-item-select-store > option", BudgetPayeeState.storeNames().length + 1);
