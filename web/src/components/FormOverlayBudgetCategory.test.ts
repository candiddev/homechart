...
test("FormOverlayBudgetCategory", async () => {
  ...
  expect(BudgetCategoryState.delete).toBeCalledWith(id);
  expect(BudgetMonthState.read).toBeCalledWith(
    `${AuthHouseholdState.data()[0].id}`,
  );
  testing.click("#button-cancel");
  testing.click("#button-update");
  await testing.sleep(100);
  expect(BudgetCategoryState.update).toBeCalledTimes(1);
  expect(BudgetMonthState.read).toBeCalledTimes(2);
  budgetCategory.id = null;
  testing.redraw();
  testing.click("#button-add");
  await testing.sleep(100);
  expect(BudgetCategoryState.create).toBeCalledTimes(1);
  expect(BudgetMonthState.read).toBeCalledTimes(3);
  ...
  expect(budgetCategory.amount).toBe(5000);
  budgetCategory.balance = -4000;
  testing.click("#button-budget-to-zero");
  expect(budgetCategory.amount).toBe(4000);
  expect(budgetCategory.balance).toBe(-4000);
