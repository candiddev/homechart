...
test("FormItemPlanProject", async () => {
	...
	testing.findAll("#form-item-select-parent-project > li", seed.planProjects.length + 2);
	testing.hasClass("#form-item-option-parent-project-personal", "FormItemSelectNested__option--selected");
	testing.click("#form-item-option-parent-project-household");
	testing.hasClass("#form-item-option-parent-project-household", "FormItemSelectNested__option--selected");
	expect(input.authAccountID)
		.toBe(null);
	expect(input.authHouseholdID)
		.toBe(seed.authHouseholds[0].id);
	expect(input.parentID)
		.toBeNull();
