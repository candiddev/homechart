...
test("FormOverlaySecretsValue", async () => {
  ...
  expect(SecretsValueState.delete).toBeCalledWith("1");
  testing.click("#button-cancel");
  testing.click("#button-update");
  await testing.sleep(100);
  expect(SecretsValueState.update).toBeCalledTimes(1);
  value.id = null;
  testing.redraw();
  testing.click("#button-add");
  await testing.sleep(100);
  expect(SecretsValueState.create).toBeCalledTimes(1);
  value.id = "1";
  testing.redraw();
  ...
  expect(value.secretsVaultID).toBe(seed.secretsVaults[0].id);

  // Name
  const name = testing.find("#form-item-input-name");
  testing.input(name, "testing");
  testing.value(name, "testing");
  expect(value.name).toBe("testing");

  // Tags
  const tags = testing.find("#form-item-input-tags");
  testing.findAll(
    "#form-item-datalist-tags option",
    SecretsValueState.tags().length,
  );
  testing.input(tags, "a ");
  expect(value.tags).toStrictEqual(["a"]);
  ...
  // Fields
  testing.findAll(
    "#form-item-datalist-properties-key-0 option",
    SecretsValueState.properties().length,
  );
  testing.input("#form-item-input-properties-key-0", "Test");
  expect(value.data[0].Test).toBe("test1");
  testing.redraw();
  ...
  expect(value.data[2].Note).toBe("This is a note");
});
