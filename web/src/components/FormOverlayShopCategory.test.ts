...
test("FormOverlayShopCategory", async () => {
	...
	expect(shopCategory.match)
		.toBe("test");

	// Store
	const store = testing.find("#form-item-select-default-store");
	testing.findAll(`#${store.id} option`, BudgetPayeeState.storeNames().length + 1);
	testing.input(store, seed.budgetPayees[1].name);
	expect(shopCategory.budgetPayeeID)
