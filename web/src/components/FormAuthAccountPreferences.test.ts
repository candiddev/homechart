...
test("FormAuthAccountPreferences", async () => {
  ...
  testing.input(
    d,
    (testing.findAll(`#${d.id} option`)[2] as HTMLOptionElement).value,
  );
  expect(state.preferences.formatDateOrder).toBe(CivilDateOrderEnum.YMD);
  testing.input(
    "#form-item-select-date-separator",
    CivilDateSeparator[CivilDateSeparatorEnum.Dash],
  );
  expect(state.preferences.formatDateSeparator).toBe(
    CivilDateSeparatorEnum.Dash,
  );
  await testing.sleep(100);
  testing.input("#form-item-select-start-of-the-week", "yes");
  expect(state.preferences.formatWeek8601).toBeTruthy();
  testing.input("#form-item-select-time-format", "21:00");
  expect(state.preferences.formatTime24).toBeTruthy();
  ...
  expect(state.preferences.darkMode).toBeFalsy();
  testing.input("#form-item-select-primary-color", "red");
  expect(state.preferences.colorPrimary).toBe("red");
  testing.input("#form-item-select-secondary-color", "yellow");
  expect(state.preferences.colorSecondary).toBe("yellow");
  testing.input("#form-item-select-accent-color", "blue");
  expect(state.preferences.colorAccent).toBe("blue");
  testing.input("#form-item-select-positive-color", "teal");
  expect(state.preferences.colorPositive).toBe("teal");
  testing.input("#form-item-select-negative-color", "pink");
  expect(state.preferences.colorNegative).toBe("pink");
