...
test("FormOverlayRewardCard", async () => {
	...
	expect(RewardCardState.delete)
		.toBeCalledWith(rewardCard.id);
	testing.click("#button-cancel");
	testing.click("#button-update");
	expect(RewardCardState.update)
		.toBeCalled();
	rewardCard.id = null;
	testing.redraw();
	testing.click("#button-add");
	expect(RewardCardState.create)
		.toBeCalled();
