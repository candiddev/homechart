...
import {
  ...
  WebFormContactUsFooter,
  WebFormContactUsMessage,
  WebFormContactUsMessageTooltip,
  WebFormContactUsTitle,
  WebGlobalActionSend,
  ...
  return {
    ...
    view: (): m.Children => {
      return InfoState.data().cloud || !CloudHouseholdState.isExpired()
        ? AppState.isSessionAuthenticated() && !m.route.get().includes("/sign")
          ? m(
              "div",
              {
                style: {
              ...
              m(
                Form,
                {
                  ...
                  onsubmit: async () => {
                    return ContactState.create(state.contact).then(() => {
                      state.formVisible = false;
                    });
                  },
                  title: {
