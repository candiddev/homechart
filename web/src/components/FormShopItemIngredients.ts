...
import {
  ...
  ObjectShop,
  ObjectStore,
  WebFormShopItemIngredientsAddSelectedItemsToPickUp,
  WebFormShopItemIngredientsAddSelectedItemsToPickUpSuccess,
  WebFormShopItemIngredientsTitle,
  WebGlobalName,
} from "../yaml8n";
...
export function FormShopItemIngredients(): m.Component<FormShopItemIngredientsAttrs> {
  ...
  const d: Stream<ShopItem[]> = Stream.lift(
    (columns, data, sort) => {
        ...
        ? [
            m(
              Form,
              {
                ...
                oninit: (): void => {
                  state.data(
                    CookMealPlanState.toShopItems(vnode.attrs.cookMealPlans),
                  );
                  m.redraw();
                },
                overlay: true,
                ...
                tableColumns: [
                  {
                    checkboxOnclick: async (d: ShopItem): Promise<void> => {
                      return new Promise((resolve) => {
                        const i = state.data().findIndex((s) => {
                          return s.id === d.id;
                        });
                  ...
                  Form,
                  {
                    buttons: [
                      ...
                      },
                      {
                        ...
                        } ${AuthAccountState.translate(ObjectItem)}`,
                        permitted: true,
                        requireOnline: true,
                        submit: true,
                      },
                    ...
                    onsubmit: async (): Promise<void> => {
                      return new Promise((resolve): void => {
                        if (state.form.data.id === null) {
                          ...
                          const d = state.data();
                          const index = d.findIndex((item: ShopItem) => {
                            return item.id === state.form.data.id;
                          });
