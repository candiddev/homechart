...
test("FormOverlayPlanProject", async () => {
	...
	expect(PlanProjectState.delete)
		.toBeCalledWith("1");
	expect(testing.mocks.route)
		.toBe("/plan/tasks?filter=today");
	testing.click("#button-cancel");
	testing.click("#button-update");
	expect(PlanProjectState.update)
		.toBeCalledTimes(1);
	planProject.id = null;
	testing.redraw();
	testing.click("#button-add");
	await testing.sleep(100);
	expect(PlanProjectState.create)
		.toBeCalledTimes(1);
	planProject.id = "1";
	...
	expect(planProject.parentID)
		.toBe(seed.planProjects[1].id);
	testing.hasClass(`#form-item-option-parent-project-${seed.planProjects[1].id}`, "FormItemSelectNested__option--selected");

	// Budget Category
	const budget = testing.find("#form-item-select-budget-category");
	testing.input(budget, `${seed.budgetCategories[1].grouping} > ${seed.budgetCategories[1].name}`);
	expect(planProject.budgetCategoryID)
		.toBe(seed.budgetCategories[1].id);
	testing.value(budget, `${seed.budgetCategories[1].grouping} > ${seed.budgetCategories[1].name}`);

	// Icon
	testing.input("#form-item-input-icon", "test");
	expect(planProject.icon)
		.toBe("test");
});
