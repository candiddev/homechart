...
import type {
  BudgetTransaction,
  BudgetTransactionAccount,
  BudgetTransactionCategory,
} from "../states/BudgetTransaction";
...
import {
  ...
  ObjectPayee,
  ObjectTransaction,
  WebBudgetCategoriesRemaining,
  WebBudgetTransactionsTransferTo,
  WebFormOverlayBudgetTransactionAmountTooltip,
  WebFormOverlayBudgetTransactionCategoryTooltip,
  WebFormOverlayBudgetTransactionDateTooltip,
  WebFormOverlayBudgetTransactionDontRoll,
  WebFormOverlayBudgetTransactionMonthTooltip,
  WebFormOverlayBudgetTransactionNoteTooltip,
  WebFormOverlayBudgetTransactionPayeeTooltip,
  ...
  WebGlobalRecurring,
  WebGlobalStatus,
  WebGlobalTransactionStatusCleared,
  WebGlobalTransactionStatusReconciled,
  WebGlobalTransactionStatusUncleared,
} from "../yaml8n";
...
export function FormOverlayBudgetTransaction(): m.Component<
  FormOverlayComponentAttrs<BudgetRecurrence>
> {
  ...
  return {
    ...
    onupdate: (vnode): void => {
      if (
        vnode.attrs.data.template.accounts !== null &&
        JSON.stringify(vnode.attrs.data.template.accounts) !==
      ...
      if (
        vnode.attrs.data.template.categories !== null &&
        JSON.stringify(vnode.attrs.data.template.categories) !==
    ...
    view: (vnode): m.Children => {
      return m(
        FormOverlay,
        {
          ...
          onDelete: async (): Promise<void | Err> => {
            ...
            if (m.route.param().import !== undefined) {
              const i = BudgetTransactionState.data().findIndex(
                (transaction: BudgetTransaction) => {
                  return transaction.id === vnode.attrs.data.id;
                },
              );
              BudgetTransactionState.data().splice(i, 1);
              return;
            ...
            // Else delete the ID
            return BudgetTransactionState.delete(vnode.attrs.data.id).then(
              async () => {
          ...
          onSubmit: async (): Promise<BudgetRecurrence | void | Err> => {
            ...
            if (
              ...
              vnode.attrs.data.budgetAccountID !== null
            ) {
              ...
              return BudgetTransactionState.update(
            ...
            return BudgetTransactionState.create(
              vnode.attrs.data.template,
              ...
              if (
                ...
                if (
                  vnode.attrs.data.template.budgetPayeeName !== undefined &&
                  vnode.attrs.data.template.budgetPayeeName !== ""
                ) {
                  vnode.attrs.data.template.budgetPayeeID =
                    BudgetPayeeState.findName(
                      vnode.attrs.data.template.budgetPayeeName,
                    ).id;
                  vnode.attrs.data.template.budgetPayeeName = "";
                }
                ...
                if (
                  vnode.attrs.data.budgetAccountID === null &&
                  vnode.attrs.data.template.accounts !== null &&
                  vnode.attrs.data.template.accounts.length > 0
                ) {
                  vnode.attrs.data.budgetAccountID =
                    vnode.attrs.data.template.accounts[0].budgetAccountID;
                }

                vnode.attrs.data.template.date = Recurrence.nextCivilDate(
                  vnode.attrs.data.recurrence,
                  CivilDate.fromString(vnode.attrs.data.template.date),
                ).toJSON();
        ...
        [
          ...
          m(FormItem, {
            ...
            select: {
              oninput: (e: string): void => {
                if (
                  vnode.attrs.data.template.accounts !== null &&
                  vnode.attrs.data.template.accounts.length === 1
                ) {
          ...
          (vnode.attrs.data.template.accounts === null ||
            (vnode.attrs.data.template.accounts !== null &&
              vnode.attrs.data.template.accounts.length <= 2))
            ? m(FormItem, {
                input: {
                  datalist: [
                    ...BudgetAccountState.names().reduce(
                      (accounts, account) => {
                        if (
                          vnode.attrs.data.template.accounts !== null &&
                          vnode.attrs.data.template.accounts.length > 0 &&
                          account !==
                            BudgetAccountState.findID(
                              vnode.attrs.data.template.accounts[0]
                                .budgetAccountID,
                            ).name
                        ) {
                          accounts.push(
                            `${AuthAccountState.translate(WebBudgetTransactionsTransferTo)} ${account}`,
                          );
                        }
                  ...
                  oninput: (e: string): void => {
                    if (
                      ...
                      const a = BudgetAccountState.findName(
                        e.replace(
                      ...
                      if (
                        ...
                        vnode.attrs.data.template.accounts[1] = {
                          ...
                          ...{
                            amount:
                              vnode.attrs.data.template.accounts[0].amount * -1,
                    ...
                    if (p.name === "") {
                      ...
                      if (p.budgetCategoryID !== null) {
                        if (
                          vnode.attrs.data.template.categories === null ||
                          vnode.attrs.data.template.categories.length === 0
                        ) {
                          vnode.attrs.data.template.categories = [
                            {
                          ...
                          vnode.attrs.data.template.categories.length === 1
                        ) {
                  ...
                  tableColumns: [
                    {
                      formatter: (e: BudgetTransactionAccount): string => {
                        return BudgetAccountState.findID(e.budgetAccountID)
                          .name;
                      },
                      ...
                      formatter: (e: BudgetTransactionAccount): string => {
                        switch (e.status) {
                          case BudgetTransactionAccountStatusEnum.Cleared:
                            return AuthAccountState.translate(
                              WebGlobalTransactionStatusCleared,
                            );
                        ...
                        : [
                            {
                              ...
                              value: Currency.toString(
                                addAmounts(vnode.attrs.data.template.accounts),
                  ...
                  ? m(
                      Form,
                      {
                        buttons: [
                            ...
                            onclick: async (): Promise<void> => {
                              return new Promise((resolve) => {
                                if (
                                  ...
                                  const i =
                                    vnode.attrs.data.template.accounts.findIndex(
                                      (account) => {
                                        return (
                                          account.id ===
                                          state.formAccount.data.id
                                        );
                                      },
                                    );
                                  vnode.attrs.data.template.accounts.splice(
                                    i,
                                    1,
                                  );
                                }
                          ...
                          {
                            name:
                              state.formAccount.data.id === null
                            ...
                            onclick: async (): Promise<void> => {
                              return new Promise((resolve) => {
                                ...
                                if (state.formAccount.data.id === null) {
                                  ...
                                  vnode.attrs.data.template.accounts.push(
                                  ...
                                  vnode.attrs.data.template.accounts[i] =
                                    state.formAccount.data;
                                }
                      ...
                      [
                        m(FormItem, {
                          ...
                          select: {
                            oninput: (e: string): void => {
                              state.formAccount.data.budgetAccountID =
                                BudgetAccountState.findName(e).id;
                            },
                            options: BudgetAccountState.names(),
                            required: true,
                            value: BudgetAccountState.findID(
                              state.formAccount.data.budgetAccountID,
                            ).name,
                          },
                          tooltip:
          ...
          vnode.attrs.data.template.accounts.length > 1
            ? m(Button, {
                ...
                onclick: async (): Promise<void> => {
                  return new Promise((resolve) => {
                    ...
                    if (vnode.attrs.data.template.accounts !== null) {
                      for (const account of vnode.attrs.data.template
                        .accounts) {
            ...
            : [],
          m(FormItemInputCurrency, {
            format: AuthHouseholdState.findID(vnode.attrs.data.authHouseholdID)
              .preferences.currency,
            name: AuthAccountState.translate(WebGlobalBudgetAmount),
            oninput: (e: number): void => {
              if (
              ...
              if (
                ...
                if (vnode.attrs.data.template.accounts.length === 2) {
                  ...
                  if (
                    ...
                    BudgetAccountState.findID(
                      vnode.attrs.data.template.accounts[1].budgetAccountID,
                    ).budget
                  ) {
                    vnode.attrs.data.template.amount = 0;
                  } else {
            ...
            ? [
                ...
                (vnode.attrs.data.template.categories === null ||
                  (vnode.attrs.data.template.categories !== null &&
                    (vnode.attrs.data.template.categories.length === 0 ||
                      (vnode.attrs.data.template.categories.length === 1 &&
                        vnode.attrs.data.template.categories[0].amount ===
                          vnode.attrs.data.template.amount))))
                  ? [
                      m(FormItem, {
                        ...
                        select: {
                          ...
                          options: ["", ...BudgetCategoryState.nameOptions()],
                          value:
                            vnode.attrs.data.template.categories === null ||
                            vnode.attrs.data.template.categories.length === 0
                              ? ""
                              : vnode.attrs.data.template.categories[0]
                                  .budgetCategoryID,
                        },
                        tooltip: AuthAccountState.translate(
                          WebFormOverlayBudgetTransactionCategoryTooltip,
                            ...
                            input: {
                              oninput: (e: string): void => {
                                if (
                                  ...
                                  const yearMonth = YearMonth.fromNumber(
                                    vnode.attrs.data.template.categories[0]
                                      .yearMonth,
                                  );
                                  yearMonth.fromInput(e);
                                  vnode.attrs.data.template.categories[0].yearMonth =
                                    yearMonth.toNumber();
                                }
                              },
                              required: true,
                              type: "month",
                              value: YearMonth.fromNumber(
                                vnode.attrs.data.template.categories[0]
                                  .yearMonth,
                              )?.toValue(),
                            },
                  ...
                  ? m(
                      Form,
                      {
                        buttons: [
                            ...
                            onclick: async (): Promise<void> => {
                              return new Promise((resolve) => {
                                if (
                                  ...
                                  const i =
                                    vnode.attrs.data.template.categories.findIndex(
                                      (category) => {
                                        return (
                                          category.id ===
                                          state.formCategory.data.id
                                        );
                                      },
                                    );
                                  vnode.attrs.data.template.categories.splice(
                                    i,
                                    1,
                                  );
                                }
                          ...
                          {
                            name:
                              state.formCategory.data.id === null
                                ? AuthAccountState.translate(ActionAdd)
                                : AuthAccountState.translate(ActionUpdate),
                            onclick: async (): Promise<void> => {
                              return new Promise((resolve) => {
                                ...
                                if (state.formCategory.data.id === null) {
                                  ...
                                  vnode.attrs.data.template.categories.push(
                                    state.formCategory.data,
                                  );
                                } else if (
                                  vnode.attrs.data.template.categories !== null
                                ) {
                                  ...
                                  vnode.attrs.data.template.categories[i] =
                                    state.formCategory.data;
                                }
                      ...
                      [
                        m(FormItem, {
                          ...
                          select: {
                            oninput: (e: string): void => {
                              state.formCategory.data.budgetCategoryID =
                        ...
                        m(FormItem, {
                          input: {
                            oninput: (e: string): void => {
                              ...
                              const yearMonth = YearMonth.fromNumber(
                                state.formCategory.data.yearMonth,
                              );
                              yearMonth.fromInput(e);
                              state.formCategory.data.yearMonth =
                                yearMonth.toNumber();
                ...
                m(Button, {
                  ...
                  onclick: async (): Promise<void> => {
                    return new Promise((resolve) => {
                      ...
                      if (vnode.attrs.data.template.categories !== null) {
                        for (const category of vnode.attrs.data.template
                          .categories) {
                          categoryAmount += category.amount;
                        }
                      }

                      state.formCategory.data.amount =
                        vnode.attrs.data.template.amount - categoryAmount;
          ...
          m.route.param().import === undefined
            ? m(FormCheckbox, {
                name: AuthAccountState.translate(WebGlobalRecurring),
                onclick: () => {
                  vnode.attrs.data.recurrence.separation =
                    vnode.attrs.data.recurrence.separation === 0 ? 1 : 0;
                },
