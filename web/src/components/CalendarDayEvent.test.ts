...
test("CalendarDayEvent", async () => {
  ...
  const event = {
    ...
    ...{
      ...
      duration: 30,
      name: "Drop kids off",
      participants: [seed.authAccounts[0].id, "notanid"],
      timeStart: "08:00",
  ...
  // Test time
  testing.text(
    ".CalendarDayEvent__participant",
    `person${seed.authAccounts[0].name}`,
  );
  testing.text(
    ".CalendarDayEvent__description > p:nth-of-type(2)",
    "warningLeave by 7:45 AM",
  );

  /// Test form
  testing.click(`${item.id} span`);
  expect(AppState.getLayoutAppForm().data).toStrictEqual(event);
