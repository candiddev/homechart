...
test("CalendarDay", async () => {
	...
	testing.text(".CalendarDay__date", `${Timestamp.now()
		.toCivilDate().day}add`);
	...
	testing.text(items[0].getElementsByTagName("span")[0], "calendar_monthBirthday");
	testing.text(items[1].getElementsByTagName("span")[0], "calendar_monthOut of Town");
	testing.text(items[2].getElementsByTagName("span")[0], "attach_moneyDeposit Janes General Store");
	testing.text(items[3].getElementsByTagName("span")[0], "calendar_monthDrop kids off8:00 AM - 8:30 AM");
	testing.text(items[4].getElementsByTagName("span")[0], "checkClean the house10:00 AM - 10:30 AM");
	testing.text(items[5].getElementsByTagName("span")[0], "local_diningLunch12:00 PM - 12:30 PM");
	testing.text(items[6].getElementsByTagName("span")[0], "calendar_monthVacation10:00 PM - 12:00 AM");
	testing.text(items[7].getElementsByTagName("span")[0], "calendar_monthNightime snack10:00 PM - 11:00 PM");

	// Test time
	testing.text(items[3].getElementsByTagName("p")[1], `person${seed.authAccounts[0].name}`);
	testing.text(items[3].getElementsByTagName("p")[2], "warningLeave by 7:45 AM");
	testing.text(items[5].getElementsByTagName("p")[1], "warningLeave by 11:40 AM");

	// Test details
	testing.text(items[5].getElementsByClassName("CalendarDayEvent__extra")[0], "local_diningPaninis");
	testing.text(items[7].getElementsByClassName("CalendarDayEvent__details")[0], "Something small!");
	testing.text(items[8].getElementsByClassName("CalendarDayEvent__extra")[0], "lunch_diningPineapple");
	...
	expect(AppState.getLayoutAppForm().data)
		.toStrictEqual(seed.planTasks[0]);
	testing.click(items[5].getElementsByClassName("CalendarDayEvent__extra")[0] as HTMLElement);
	expect(AppState.getLayoutAppForm().data)
		.toStrictEqual(seed.cookMealPlans[0]);
	testing.click(items[8].getElementsByTagName("span")[0]);
	expect(AppState.getLayoutAppForm().data.logs)
		.toStrictEqual([
			seed.healthLogs[0],
			seed.healthLogs[1],
		]);
