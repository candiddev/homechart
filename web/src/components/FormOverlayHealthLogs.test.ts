...
test("FormOverlayHealthLogs", async () => {
  ...
  expect(HealthLogState.delete).toBeCalledWith("notanid");
  testing.click("#button-cancel");
  testing.click("#button-update");
  expect(HealthLogState.update).toBeCalledTimes(1);
  ...
  expect(healthLogs.logs).toHaveLength(0);

  // Timestamp
  const later = Timestamp.now();
  later.addDays(2);
  testing.input("#form-item-input-date", later.toCivilDate().toJSON());

  // Inputs
  testing.click("#button-add-input");
  expect(healthLogs.logs[0].date).toBe(later.toCivilDate().toJSON());
  testing.findAll("#form-item-inputs option", 21);
  testing.input(
    `#form-item-input-inputs-${healthLogs.logs[0].id}`,
    seed.healthItems[3].name,
  );
  expect(healthLogs.logs[0].healthItemID).toBe(seed.healthItems[3].id);
  await testing.sleep(100);
  testing.click("#button-add-input");
  expect(healthLogs.logs[1].nameInput).toBe("");
  testing.input(`#form-item-input-inputs-${healthLogs.logs[1].id}`, "test1");
  expect(healthLogs.logs[1].nameInput).toBe("test1");
  ...
  expect(healthLogs.logs[2].nameOutput).toBe("");
  testing.input(`#form-item-input-outputs-${healthLogs.logs[2].id}`, "test2");
  expect(healthLogs.logs[2].nameOutput).toBe("test2");
});
