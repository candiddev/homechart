...
import { ObjectBudget, ObjectCategory, ObjectProject, WebGlobalName, WebGlobalNameTooltip, WebGlobalTags, WebGlobalTagsTooltip } from "../yaml8n";
import { FormItemSelectAuthHousehold } from "./FormItemSelectAuthHousehold";

export function FormOverlayPlanProject (): m.Component<FormOverlayComponentAttrs<PlanProject>> {
	return {
		...
		view: (vnode): m.Children => {
			return m(FormOverlay, {
				...
				onDelete: async (): Promise<void | Err> => {
					return PlanProjectState.delete(vnode.attrs.data.id)
						.then(() => {
							if (m.route.get()
