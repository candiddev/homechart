...
export const MarkdownLinks = {
	match: /#(bookmark|budgetaccount|budgetcategory|budgetpayee|cookrecipe|inventorycollection|inventoryitem|notespage|planproject|plantask|rewardcard|secretsvalue|secretsvault|shoplist|shopstore)\/\w+/,
	parse: (word: string): MarkdownParseOutput => {
		...
		case word.startsWith("#plantask"): {
			...
			} else if (task.authAccountID !== null) { // eslint-disable-line no-negated-condition
		...
		case word.startsWith("#rewardcard"): {
			...
			if (card.senders.includes(AuthAccountState.data().id!)) { // eslint-disable-line @typescript-eslint/no-non-null-assertion
				path = "sent";
			}
		...
		case word.startsWith("#secretsvalue"): {
			...
			output.icon = options === "" ?
				...
				undefined;
			output.link = options === "" ?
				`/secrets/${value.secretsVaultID}` :
				undefined;
			output.name = options === "" ?
				SecretsValueState.values()[`${value.id}`].name :
				SecretsValueState.values()[`${value.id}`].data[0][options] === undefined ?
					"" :
					SecretsValueState.values()[`${value.id}`].data[0][options] as string;
			break;
		}
		case word.startsWith("#secretsvault"): {
			const vault = SecretsVaultState.findShortID(id);
			output.icon = vault.icon === "" ?
				Icons.SecretsVault :
				vault.icon;
