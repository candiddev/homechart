...
import type {
  SecretsValue,
  SecretsValueData,
  SecretsValueDecrypted,
} from "../states/SecretsValue";
...
import {
  ...
  ObjectVault,
  ObjectVersion,
  WebFormOverlaySecretsValueGenerateValue,
  WebFormOverlaySecretsValueLength,
  WebFormOverlaySecretsValueLengthTooltip,
  WebFormOverlaySecretsValueLowercase,
  WebFormOverlaySecretsValueNoteTooltip,
  WebFormOverlaySecretsValueNumbers,
  WebFormOverlaySecretsValueSpecialCharacters,
  WebFormOverlaySecretsValueUppercase,
  WebFormOverlaySecretsValueUseValue,
  WebFormOverlaySecretsValueValuesTooltip,
  WebFormOverlaySecretsValueVaultTooltip,
  ...
  WebGlobalTOTPCode,
  WebGlobalVersionTooltip,
} from "../yaml8n";

export function FormOverlaySecretsValue(): m.Component<
  FormOverlayComponentAttrs<SecretsValueDecrypted>
> {
  const state = {
      ...
      return m(
        FormOverlay,
        {
          buttons: [
            {
              ...
              onclick: async (): Promise<void | Err> => {
                ...
                const s = await SecretsValueState.encryptValue(
                  vnode.attrs.data,
                );

                if (!IsErr(s)) {
                  return SecretsValueState.update(s).then(() => {
          ...
          permitted: GlobalState.permitted(
            PermissionComponentsEnum.Secrets,
            true,
        ...
        [
                    ...
                    return {
                      ...
                      name:
                        data.updated === state.updated
                          ...
                          : AppState.formatTimestamp(
                              Timestamp.fromString(data.updated),
                            ),
                    };
                  }),
          ...
          m(FormItem, {
            ...
            textArea: {
              ...
              value:
                vnode.attrs.data.data.length === 0 ||
                vnode.attrs.data.data[state.index].Note === undefined
                  ? ""
          ...
          vnode.attrs.data.data.length === 0 ||
          vnode.attrs.data.data[state.index].TOTP === undefined ||
          vnode.attrs.data.data[state.index].TOTP === ""
            ? []
            : m(FormItem, {
                input: {
                  disabled: true,
