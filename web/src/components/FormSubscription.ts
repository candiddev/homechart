...
import {
  ...
  WebFormSubscriptionExpiresTrial,
  WebFormSubscriptionReferralCode,
  WebFormSubscriptionReferralCodeTooltip,
  WebFormSubscriptionReferralStatus1,
  WebFormSubscriptionReferralStatus2,
  WebFormSubscriptionReferralStatus3,
  WebFormSubscriptionReferralStatus4,
  WebFormSubscriptionReferrerCode,
  WebFormSubscriptionReferrerCodeTooltip,
  WebFormSubscriptionRetryLink,
  WebFormSubscriptionSelfHostedID,
  WebFormSubscriptionSelfHostedIDCopied,
  WebFormSubscriptionSelfHostedIDCopy,
  WebFormSubscriptionSelfHostedIDTooltip,
  WebFormSubscriptionStatus,
  WebFormSubscriptionSubscribeLifetime,
  WebFormSubscriptionSubscribeMonthly,
  WebFormSubscriptionTitle,
  WebGlobalNever,
...
export function FormSubscription(): m.Component<FormSubscriptionAttrs> {
    ...
    expires: (id: NullUUID): string => {
      return InfoState.data().cloud
        ? CivilDate.fromString(
            AuthHouseholdState.findID(id).subscriptionExpires as string,
          ).toString(
        ...
        : CloudHouseholdState.findID(id).subscriptionExpires === null
          ? "Unknown"
          : CivilDate.fromString(
              CloudHouseholdState.findID(id).subscriptionExpires as string,
            ).toString(
              AuthAccountState.data().preferences.formatDateOrder,
              AuthAccountState.data().preferences.formatDateSeparator,
            );
  ...
  async function updatePrices(): Promise<void> {
    await PaymentState.readPaddle().catch(() => {});
  }
      ...
      s = AuthHouseholdState.data.map(async () => {
        if (state.authHousehold.id === null) {
          ...
          if (InfoState.data().cloud) {
            ...
            for (const ah of AuthSessionState.data().permissionsHouseholds!) {
              // eslint-disable-line @typescript-eslint/no-non-null-assertion
              await CloudHouseholdState.read(ah.authHouseholdID).catch(
                () => {},
              );
            }
        ...
        if (
          ...
          CloudHouseholdState.findID(state.authHousehold.id) !== null
        ) {
          return updatePrices();
        }
      ...
      return m(
        Form,
        {
          ...
          title: {
            name:
              !InfoState.data().cloud &&
              CloudHouseholdState.findID(vnode.attrs.authHouseholdID).id ===
                null
        ...
        [
          InfoState.data().cloud ||
          CloudHouseholdState.findID(vnode.attrs.authHouseholdID).id !== null
            ? [
                m(FormItem, {
                  input: {
                    disabled: true,
                    ...
                    value:
                      Translations.subscriptionProcessors[
                        state.processor(vnode.attrs.authHouseholdID)
                      ],
                  },
                ...
                m("div.Form__buttons", [
                  m(Button, {
                    ...
                    onclick: async (): Promise<void | Err> => {
                      setTimeout(async () => {
                        await CloudHouseholdState.readJWT(
                          vnode.attrs.authHouseholdID,
                        );
                      }, 1000);
...
${PaymentState.prices().monthly}`,
                    onclick: async (): Promise<void | Err> => {
                      if (
                        state.processor(vnode.attrs.authHouseholdID) ===
                        AuthHouseholdSubscriptionProcessorEnum.PaddleYearly
                      ) {
                        return PaymentState.update(
                          vnode.attrs.authHouseholdID,
                          AuthHouseholdSubscriptionProcessorEnum.PaddleMonthly,
                        );
                      }

                      return PaymentState.createPaddle(
                        vnode.attrs.authHouseholdID,
                        AuthHouseholdSubscriptionProcessorEnum.PaddleMonthly,
                      )
                        .then((response) => {
                  ...
                  m(
                    ...
                    `${
                      state.authHousehold.subscriptionReferralCount === 0
                        ? ""
                        : `${AuthAccountState.translate(WebFormSubscriptionReferralStatus1)} ${state.authHousehold.subscriptionReferralCount} ${AuthAccountState.translate(WebFormSubscriptionReferralStatus2)}.`
                    }${
                      state.authHousehold.subscriptionProcessor ===
                      AuthHouseholdSubscriptionProcessorEnum.PaddleLifetime
                        ? ""
                ...
                m(FormItem, {
                  input: {
                    ...
                    value: InfoState.data().cloud
                      ...
                      : AuthHouseholdState.findID(vnode.attrs.authHouseholdID)
                          .id,
                  },
                  name: AuthAccountState.translate(
                    WebFormSubscriptionSelfHostedID,
                ...
                InfoState.data().cloud
                  ...
                  : [
                        ...
                        onclick: async () => {
                          return navigator.clipboard
                            ...
                            .then(() => {
                              ...
                              window.open(
                                "https://web.homechart.app/subscription",
                                "_blank",
                              );
                            });
                        },
                      ...
                      m(Button, {
                        ...
                        onclick: async () => {
                          return CloudHouseholdState.read(
                            vnode.attrs.authHouseholdID,
                          ).then(async () => {
                            await CloudHouseholdState.readJWT(
                              vnode.attrs.authHouseholdID,
