...
test("FormOverlayPlanTask", async () => {
  ...
  expect(PlanTaskState.delete).toBeCalledWith("1");
  testing.click("#button-cancel");
  testing.click("#button-update");
  expect(PlanTaskState.update).toBeCalledTimes(1);
  testing.click("#button-cancel");
  testing.click("#button-copy");
  expect(planTask.id).toBeNull();
  testing.click("#button-add");
  expect(PlanTaskState.create).toBeCalledTimes(1);
  ...
  testing.click(done);
  expect(planTask.done).toBe(true);

  // Project
  testing.click(`#form-item-option-project-${seed.planProjects[6].id}`);
  expect(planTask.planProjectID).toBe(seed.planProjects[6].id);
  testing.hasClass(
    `#form-item-option-project-${seed.planProjects[6].id}`,
  ...
  expect(planTask.assignees).toHaveLength(2);
  ...
  expect(planTask.duration).toBe(30);
  ...
  expect(planTask.dueDate).toBe("2020-01-01T06:00:00.000Z");
  ...
  expect(planTask.notify).toBe(true);

  // Due Time
  const dueTime = testing.find("#form-item-input-due-time");
  testing.input(dueTime, "00:30");
  expect(planTask.dueDate).toBe("2020-01-01T06:30:00.000Z");
  ...
  expect(planTask.recurOnDone).toBeTruthy();

  // Template
  testing.click("#form-checkbox-input-use-as-template");
  await testing.sleep(100);
  expect(planTask.template).toBeTruthy();
  testing.click("#form-checkbox-input-global-template");
  expect(planTask.authAccountID).toBeNull();
  expect(planTask.authHouseholdID).toBeNull();
