...
import { ObjectMeal, ObjectMealPlan, ObjectMealTime, ObjectRecipe, WebFormOverlayCookMealPlanChef, WebFormOverlayCookMealPlanChefTooltip, WebFormOverlayCookMealPlanDateTooltip, WebFormOverlayCookMealPlanFilterTags, WebFormOverlayCookMealPlanFilterTagsTooltip, WebFormOverlayCookMealPlanMealTooltip, WebFormOverlayCookMealPlanRecipeDetails, WebFormOverlayCookMealPlanRecipeTooltip, WebFormOverlayCookMealPlanReminderCook, WebFormOverlayCookMealPlanReminderPrep, WebFormOverlayCookMealPlanReminderTooltip, WebFormOverlayCookMealPlanSurpriseMe, WebFormOverlayCookMealPlanTimeTooltip, WebGlobalDate, WebGlobalImage, WebGlobalRecipeIngredients, WebGlobalRecipeIngredientsTooltip, WebGlobalRecipeRating, WebGlobalRecipeRatingTooltip, WebGlobalRecipeScale, WebGlobalRecipeScaleTooltip } from "../yaml8n";

function calculateCookPrep (cookMealPlan: CookMealPlan): void {
	if (cookMealPlan.cookRecipeID !== null && cookMealPlan.date !== null && cookMealPlan.time !== null) {
		const cookRecipe = CookRecipeState.findID(cookMealPlan.cookRecipeID);
		const startTime = Timestamp.fromCivilDate(CivilDate.fromString(cookMealPlan.date));

		startTime.timestamp.setHours(parseInt(cookMealPlan.time.split(":")[0], 10));
		startTime.timestamp.setMinutes(parseInt(cookMealPlan.time.split(":")[1], 10));
...
export function FormOverlayCookMealPlan (): m.Component<FormOverlayComponentAttrs<CookMealPlan>> {
	let showRecipe = true;
	...
	return {
		...
		view: (vnode): m.Children => {
			return m(FormOverlay, {
				...
				permitted: GlobalState.permitted(PermissionComponentsEnum.Cook, true, vnode.attrs.data.authHouseholdID),
			}, [
				...
				m(FormItemInputDate, {
					...
					required: true,
					tooltip: AuthAccountState.translate(WebFormOverlayCookMealPlanDateTooltip),
						...
						selected: () => {
							return [
								vnode.attrs.data.cookMealTimeID === null ?
									"" :
									CookMealTimeState.findID(vnode.attrs.data.cookMealTimeID).name,
							];
						},
						value: CookMealTimeState.names(),
					},
					name: AuthAccountState.translate(ObjectMeal),
					tooltip: AuthAccountState.translate(WebFormOverlayCookMealPlanMealTooltip),
				}),
				m(FormItem, {
					input: {
						oninput: (e: string): void => {
					...
					tooltip: AuthAccountState.translate(WebFormOverlayCookMealPlanTimeTooltip),
				}),
				m(FormItemSelectAuthHouseholdMembers, {
					...
					tooltip: AuthAccountState.translate(WebFormOverlayCookMealPlanChefTooltip),
				}),
				m(FormItem, {
					input: {
						...
						required: true,
						type: "text",
						value: vnode.attrs.data.cookRecipeID === null ?
							vnode.attrs.data.customRecipe :
							CookRecipeState.findID(vnode.attrs.data.cookRecipeID).name,
					},
					name: AuthAccountState.translate(ObjectRecipe),
					tooltip: AuthAccountState.translate(WebFormOverlayCookMealPlanRecipeTooltip),
				}),
				m(Button, {
					name: AuthAccountState.translate(WebFormOverlayCookMealPlanSurpriseMe),
					onclick: async (): Promise<void> => {
						return new Promise((resolve) => {
							vnode.attrs.data.cookRecipeID = CookRecipeState.data()[Math.floor(Math.random() * CookRecipeState.data().length)].id;

							while (tags.length !== 0 && CookRecipeState.findID(vnode.attrs.data.cookRecipeID).tags.findIndex((tag) => {
				...
				vnode.attrs.data.cookRecipeID === null ?
					...
					[
						m(FormItem, {
							...
							name: AuthAccountState.translate(WebFormOverlayCookMealPlanFilterTags),
							tooltip: AuthAccountState.translate(WebFormOverlayCookMealPlanFilterTagsTooltip),
						}),
						m(FormExpander, {
							expand: showRecipe,
							name: AuthAccountState.translate(WebFormOverlayCookMealPlanRecipeDetails),
							onclick: () => {
						...
						showRecipe ?
							[
								m(FormImage, {
									...
									oninput: () => {},
									value: CookRecipeState.findID(vnode.attrs.data.cookRecipeID).image,
								}),
								m(FormItemIcons, {
									disabled: true,
									...
									value: CookRecipeState.findID(vnode.attrs.data.cookRecipeID).rating,
								}),
								m(FormItem, {
									...
									textArea: {
										disabled: true,
										oninput: (): void => {},
										value: CookRecipeState.scaleIngredients(CookRecipeState.findID(vnode.attrs.data.cookRecipeID).ingredients, vnode.attrs.data.cookRecipeScale),
									},
								...
								m(FormItem, {
									input: {
										...
										type: "text",
										value: vnode.attrs.data.cookRecipeScale !== undefined && vnode.attrs.data.cookRecipeScale !== null ?
						...
						m(FormItem, {
							input: {
								oninput: (e: string): void => {
									...
									vnode.attrs.data.notificationTimePrep = date.toISOString();
								},
								type: "datetime-local",
								value: vnode.attrs.data.notificationTimePrep === null ?
									null : // null for value
									Timestamp.fromString(vnode.attrs.data.notificationTimePrep)
										.toHTMLDate(),
							},
							name: AuthAccountState.translate(WebFormOverlayCookMealPlanReminderPrep),
							tooltip: AuthAccountState.translate(WebFormOverlayCookMealPlanReminderTooltip),
						}),
						m(FormItem, {
							input: {
								oninput: (e: string): void => {
									...
									vnode.attrs.data.notificationTimeCook = date.toISOString();
								},
								type: "datetime-local",
								value: vnode.attrs.data.notificationTimeCook === null ?
									null : // null for value
									Timestamp.fromString(vnode.attrs.data.notificationTimeCook)
										.toHTMLDate(),
							},
							name: AuthAccountState.translate(WebFormOverlayCookMealPlanReminderCook),
							tooltip: AuthAccountState.translate(WebFormOverlayCookMealPlanReminderTooltip),
						}),
					],
			]);
