...
describe("FormOverlayBudgetTransaction", () => {
  ...
  test("buttons", async () => {
    ...
    data.template.date = CivilDate.now().toJSON();
    testing.click("#button-delete");
    testing.click("#button-confirm-delete");
    expect(BudgetRecurrenceState.delete).toBeCalledWith(id);
    testing.click("#button-cancel");
    data.template.id = null;
    testing.click("#button-update");
    const tomorrow = Timestamp.now();
    tomorrow.addDays(1);
    expect(data.template.date).toBe(CivilDate.now().toJSON());
    expect(BudgetRecurrenceState.update).toBeCalledTimes(1);
    testing.redraw();
    data.budgetAccountID = null;
    data.template.id = id;
    testing.click("#button-update");
    expect(BudgetTransactionState.update).toBeCalledTimes(1);
    data.id = null;
    data.template.date = CivilDate.now().toJSON();
    data.template.id = null;
    data.recurrence.separation = 1;
    ...
    expect(BudgetTransactionState.create).toBeCalledTimes(1);
    expect(BudgetRecurrenceState.create).toBeCalledTimes(1);
    data.id = id;
    ...
    testing.click("#button-update");
    expect(BudgetTransactionState.data()[0].amount).toBe(0);
    testing.click("#button-delete");
    testing.click("#button-confirm-delete");
    expect(BudgetTransactionState.data()).toHaveLength(0);
    ...
    testing.click("#button-delete");
    testing.click("#button-confirm-delete");
    expect(BudgetTransactionState.delete).toBeCalledWith(id);
  });
  ...
  test("form", async () => {
    ...
    expect(data.template.accounts![0].budgetAccountID).toBe(
      seed.budgetAccounts[0].id,
    );

    // Date
    testing.input("#form-item-input-date", "2019-11-01");
    expect(data.template.date).toBe("2019-11-01");
    ...
    expect(data.template.amount).toBe(0);
    expect(data.template.accounts![0].amount).toBe(1000);

    // Payee field, transfer to off budget, amount
    data.template.categories = seed.budgetTransactions[1].categories;
    testing.findAll(
      "#form-item-datalist-payee > option",
      seed.budgetAccounts.length - 1 + seed.budgetPayees.length,
    );
    testing.input("#form-item-input-payee", "Transfer To Cash Buried Outside");
    expect(data.template.accounts![1].budgetAccountID).toBe(
      seed.budgetAccounts[1].id,
    );
    expect(data.template.accounts![1].amount).toBe(-1000);
    testing.input("#form-item-input-amount", "5000");
    expect(data.template.amount).toBe(5000);
    expect(data.template.accounts![0].amount).toBe(5000);
    expect(data.template.accounts![1].amount).toBe(-5000);
    ...
    expect(data.template.accounts![1].budgetAccountID).toBe(
      seed.budgetAccounts[2].id,
    );
    expect(data.template.accounts![1].amount).toBe(-5000);
    testing.input("#form-item-input-amount", "6000");
    testing.redraw();
    expect(data.template.amount).toBe(0);
    expect(data.template.accounts![0].amount).toBe(6000);
    expect(data.template.accounts![1].amount).toBe(-6000);
    expect(data.template.categories).toHaveLength(0);
    ...
    await testing.sleep(100);
    testing.text(
      "#table-container-accounts #subtitle-remaining",
      "Remaining: $30.00",
    );
    ...
    expect(data.template.accounts!).toHaveLength(1);
    expect(data.template.accounts![0].amount).toBe(6000);
    expect(data.template.amount).toBe(6000);
    testing.input("#form-item-input-amount", "7000");
    testing.redraw();
    expect(data.template.amount).toBe(7000);
    expect(data.template.accounts![0].amount).toBe(7000);
    testing.find("#form-item-select-category");
    expect(data.template.categories[0].amount).toBe(7000);
    expect(data.template.categories[0].budgetCategoryID).toBe(
      seed.budgetPayees[0].budgetCategoryID,
    );

    testing.input("#form-item-input-note", "a note");
    expect(data.template.note).toBe("a note");
    testing.click("#form-checkbox-input-dont-rollup-transaction");
    expect(data.template.keep).toBe(true);
  ...
  test("categories", async () => {
    ...
    testing.input(
      "#form-item-input-payee",
      `Transfer To ${seed.budgetAccounts[1].name}`,
    );
    testing.input("#form-item-input-amount", "1000");
    testing.redraw();
    testing.value("#form-item-select-category", "");
    testing.input(
      "#form-item-select-category",
      BudgetCategoryState.findName("Utilities").id as string,
    );
    expect(data.template.categories).toStrictEqual([
      {
        ...
        ...{
          ...
          yearMonth: CivilDate.now().toYearMonth().toNumber(),
        },
      },
    ]);
    testing.input(
      "#form-item-select-category",
      BudgetCategoryState.findName("Restaurants").id as string,
    );
    expect(data.template.categories).toStrictEqual([
      {
        ...BudgetTransactionState.newCategory(),
    ...
    testing.value(
      ...
      CivilDate.now().toYearMonth().toValue(),
    );
    ...
    const categoryName = categoryForm.querySelector(
      "#form-item-select-name",
    ) as HTMLSelectElement;
    expect(categoryName.options).toHaveLength(seed.budgetCategories.length);
    testing.input(
      categoryName,
    ...
    const categoryAmount = categoryForm.querySelector(
      "#form-item-input-amount",
    ) as HTMLInputElement;
    testing.input(categoryAmount, "1000");
    const categoryYearMonth = categoryForm.querySelector(
      "#form-item-input-month",
    ) as HTMLInputElement;
    ...
    await testing.sleep(500);
    expect(data.template.categories![1].budgetCategoryID).toBe(
      seed.budgetCategories[1].id,
    );
    expect(data.template.categories![1].amount).toBe(1000);
    expect(data.template.categories![1].yearMonth).toBe(201910);
    ...
    expect(data.template.categories).toHaveLength(1);
  });
});
