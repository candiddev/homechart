...
test("FormShopItemIngredients", async () => {
	...
	testing.text(`#table-data-${id}-name`, "1 1/2 cup all-purpose flour (plus more for rolling)");
	testing.text(`#table-data-${id}-shopcategoryid`, seed.shopCategories[10].name);
	...
	testing.click(`#table-row_${id}`);
	testing.find("#form-item-input-name");
	testing.findAll("#form-item-select-category option", seed.shopCategories.length + 1);
