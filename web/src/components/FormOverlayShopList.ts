...
import { ObjectBudget, ObjectCategory, ObjectList, WebGlobalName, WebGlobalNameTooltip } from "../yaml8n";

export function FormOverlayShopList (): m.Component<FormOverlayComponentAttrs<ShopList>> {
	return {
		...
		view: (vnode): m.Children => {
			return m(FormOverlay, {
				...
				onDelete: async (): Promise<void | Err> => {
					return ShopListState.delete(vnode.attrs.data.id)
						.then(() => {
							if (m.route.get()
								.includes(`/shop/lists/${vnode.attrs.data.id}`)) {
