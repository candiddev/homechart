...
import { ObjectOwner,  WebFormItemSelectAuthHouseholdTooltip,  WebGlobalPersonal } from "../yaml8n";
...
export function FormItemSelectAuthHousehold (): m.Component<FormItemSelectAuthHouseholdAttrs> {
	return {
		view: (vnode): m.Children => {
			const ah = AuthHouseholdState.getPermitted(vnode.attrs.permissionComponent, PermissionEnum.Edit);

			return vnode.attrs.item.authAccountID !== undefined && ah.length > 0 || ah.length > 1 ?
				m(FormItem, {
					buttonArray: {
						...
						selected: () => {
							return vnode.attrs.item.authHouseholdID === null ?
								vnode.attrs.item.authAccountID !== undefined && vnode.attrs.item.authAccountID !== null ?
									[
						...
						value: vnode.attrs.item.authAccountID === undefined || vnode.attrs.noPersonal === true ?
							ah :
							[
								{
					...
					tooltip: AuthAccountState.translate(WebFormItemSelectAuthHouseholdTooltip),
				}) :
				[];
		},
	};
