...
test("FormOverlayCookMealPlan", async () => {
  ...
  expect(CookMealPlanState.delete).toBeCalledTimes(1);
  testing.click("#button-cancel");
  testing.click("#button-update");
  expect(CookMealPlanState.update).toBeCalledTimes(1);
  cookMealPlan.id = null;
  testing.redraw();
  testing.click("#button-add");
  expect(CookMealPlanState.create).toBeCalledTimes(1);
  ...
  const mealList = testing.findAll(
    "#button-array-meal p",
    seed.cookMealTimes.length,
  );
  ...
  testing.hasAttribute(
    "#form-item-input-recipe",
    "list",
    "form-item-datalist-recipe",
  );
  ...
  testing.text(
    "#form-item-text-area-ingredients",
    seed.cookRecipes[0].ingredients.replace(/\n/g, ""),
  );
