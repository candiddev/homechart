...
import { ObjectCategory, ObjectStore, WebFormOverlayShopCategoryDefaultStoreTooltip, WebFormOverlayShopCategoryMatch, WebFormOverlayShopCategoryMatchTooltip, WebGlobalName, WebGlobalNameTooltip } from "../yaml8n";

export function FormOverlayShopCategory (): m.Component<FormOverlayComponentAttrs<ShopCategory>> {
	return {
		...
		view: (vnode): m.Children => {
			return m(FormOverlay, {
				...
				permitted: GlobalState.permitted(PermissionComponentsEnum.Shop, true, vnode.attrs.data.authHouseholdID),
			}, [
				...
				m(FormItem, {
					...
					textArea: {
						...
						value: vnode.attrs.data.match === "" ?
							vnode.attrs.data.match :
							vnode.attrs.data.match.split("|")
								.join("\n"),
					},
					tooltip: AuthAccountState.translate(WebFormOverlayShopCategoryMatchTooltip),
				}),
				m(FormItem, {
					...
					tooltip: AuthAccountState.translate(WebFormOverlayShopCategoryDefaultStoreTooltip),
				}),
			]);
		},
	};
