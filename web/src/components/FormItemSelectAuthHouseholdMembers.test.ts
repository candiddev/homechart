...
describe("FormItemSelectAuthHouseholdMembers", () => {
  test("single", async () => {
    ...
    const array = testing.findAll(
      "#button-array-participants > p",
      seed.authHouseholds[0].countMembers,
    );
    testing.click(array[0]);
    testing.click(array[1]);
    expect(attrs.members[0]).toBe(AuthHouseholdState.members()[4].id);
    testing.click(array[1]);
    expect(attrs.members[0]).toBeNull();
  });
  test("multiple", async () => {
    const attrs = {
      authHouseholdID: seed.authHouseholds[0].id,
    ...
    testing.click(array[0]);
    testing.click(array[1]);
    expect(attrs.members).toHaveLength(2);
  });
});
