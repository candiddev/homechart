...
import { ObjectInput, ObjectInputs, ObjectLogs, ObjectOutput, ObjectOutputs, WebFormOverlayHealthLogDateTooltip, WebFormOverlayHealthLogInputsTooltip, WebFormOverlayHealthLogOutputsTooltip, WebGlobalDate, WebGlobalHouseholdMember, WebGlobalHouseholdMemberTooltip } from "../yaml8n";
...
export function FormOverlayHealthLogs (): m.Component<FormOverlayComponentAttrs<FormOverlayHealthLogs>> {
	...
	return {
		...
		oninit: (vnode): void => {
			...
			if (vnode.attrs.data.logs.length === 0) {
				vnode.attrs.data.logs.push(...HealthLogState.data()
					.filter((log) => {
						return log.authAccountID === vnode.attrs.data.authAccountID && log.date === vnode.attrs.data.date;
					}));
				m.redraw();
			}
		},
			...
			return m(FormOverlay, {
				...
				onSubmit: async (): Promise<void | Err> => {
					for (const log of vnode.attrs.data.logs) {
						...
						if (log.created === null) {
							await HealthLogState.create(log)
								.catch(() => {
									AppState.setComponentsButtonLoading("");
								});
						} else {
							await HealthLogState.update(log)
								.catch(() => {
									AppState.setComponentsButtonLoading("");
								});
						}
				...
				AuthHouseholdState.getPermitted(PermissionComponentsEnum.Auth, PermissionEnum.Edit).length > 0 && AuthHouseholdState.membersChildren().length > 0 ?
				...
				m(FormItemInputDate, {
					...
					tooltip: AuthAccountState.translate(WebFormOverlayHealthLogDateTooltip),
				...
				m("div.FormItem", {
					...
					m("div.FormItem__label", [
						...
						m(Tooltip, {
							value: AuthAccountState.translate(WebFormOverlayHealthLogInputsTooltip),
						}),
					]),
					vnode.attrs.data.logs
						.map((log, i) => {
							...
							if (item.output && log.nameInput === undefined || log.nameOutput !== undefined) {
								return m("p", {
									key: `${log.id}`,
								});
							}
							...
							return m("div.FormItem__multi", {
								...
								m(FormItemInput, {
									datalist: HealthItemState.findAuthAccountID(vnode.attrs.data.authAccountID, false)
										.map((item) => {
											return item.name;
										}),
									name: `inputs-${log.id}`,
									oninput: (e: string) => {
										const item = HealthItemState.findAuthAccountIDName(vnode.attrs.data.authAccountID, e);
				...
				m("div.FormItem", {
					...
					m("div.FormItem__label", [
						...
						m(Tooltip, {
							value: AuthAccountState.translate(WebFormOverlayHealthLogOutputsTooltip),
						}),
					]),
					vnode.attrs.data.logs
						.map((log, i) => {
							...
							return m("div.FormItem__multi", {
								...
								m(FormItemInput, {
									datalist: HealthItemState.findAuthAccountID(vnode.attrs.data.authAccountID, true)
										.map((item) => {
											return item.name;
										}),
									name: `outputs-${log.id}`,
									...
									type: "text",
									value: log.nameOutput === undefined ?
										HealthItemState.findID(log.healthItemID).name :
										log.nameOutput,
								}),
								m("i.GlobalButtonIconAdd", {
