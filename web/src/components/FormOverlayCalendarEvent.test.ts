...
describe("FormOverlayCalendarEvent", () => {
  ...
  test("buttons", async () => {
    ...
    expect(CalendarEventState.delete).toBeCalledWith(data.id);
    testing.click("#button-cancel");
    testing.click("#button-update");
    expect(CalendarEventState.update).toBeCalledTimes(1);
    data.id = null;
    testing.redraw();
    testing.click("#button-add");
    expect(CalendarEventState.create).toBeCalledTimes(1);
    ...
    data.id = id;
    data.recurrence = Recurrence.new();
    data.recurrenceDate = CivilDate.now().toJSON();
    testing.redraw();
    testing.click("#button-update-occurrence");
    await testing.sleep(100);
    expect(CalendarEventState.update).toBeCalledTimes(2);
    expect(CalendarEventState.create).toBeCalledTimes(2);
  ...
  test("inputs", async () => {
    ...
    const data = {
      ...
      ...{
        ...
        recurrenceDate: CivilDate.now().toJSON(),
      },
    };
    testing.mount(FormOverlayCalendarEvent, {
      data: data,
    ...
    expect(data.details).toBe("a test 1");
    ...
    expect(data.duration).toBe(25 * 60);
    ...
    expect(data.duration).toBe(26 * 60);
    ...
    expect(data.notifyOffset).toBe(120);
    ...
    expect(data.timeZone).toBe("US/Pacific");
    ...
    expect(data.recurrence).toStrictEqual({
      ...Recurrence.new(),
      ...{
        separation: 1,
      },
