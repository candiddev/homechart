...
test("FormOverlayBookmark", async () => {
  ...
  expect(BookmarkState.delete).toBeCalledWith(bookmark.id);
  testing.click("#button-cancel");
  testing.click("#button-update");
  expect(BookmarkState.update).toBeCalled();
  bookmark.id = null;
  testing.redraw();
  testing.click("#button-add");
  expect(BookmarkState.create).toBeCalled();
