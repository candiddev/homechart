...
test("FormOverlayBudgetAccount", async () => {
  ...
  expect(BudgetAccountState.delete).toBeCalledWith(budgetAccount.id);
  testing.click("#button-cancel");
  testing.click("#button-update");
  expect(BudgetAccountState.update).toBeCalled();
  budgetAccount.id = null;
  testing.redraw();
  testing.click("#button-add");
  expect(BudgetAccountState.create).toBeCalled();
  ...
  expect(budgetAccount.name).toBe("Name");

  testing.input("#form-item-input-icon", "Icon");
  expect(budgetAccount.icon).toBe("Icon");

  testing.click("#form-checkbox-input-on-budget");
  expect(budgetAccount.budget).not.toEqual(BudgetAccountState.new().budget);

  testing.click("#form-checkbox-input-hidden");
  expect(budgetAccount.hidden).toBeTruthy();
});
