...
import { ObjectHousehold, WebFormJoinHouseholdJoinToken, WebGlobalActionJoin } from "../yaml8n";
				...
				buttons: [
					{
						...
						onclick: async (): Promise<void | Err> => {
							return AuthHouseholdState.create({
								...
								...{
									...
									subscriptionReferrerCode: AuthAccountState.data().subscriptionReferrerCode,
								},
							})
						...
						permitted: vnode.attrs.add === true && !AuthAccountState.data().child,
						primary: true,
						requireOnline: true,
					},
				],
				onsubmit: async () => {
					const household = await AuthAccountAuthHouseholdState.inviteAccept(token);
					if (IsErr(household)) {
						return;
					}
