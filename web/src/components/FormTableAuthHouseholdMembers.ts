...
import {
  ObjectChild,
  ObjectOwner,
  WebFormTableAuthHouseholdMembersInviteToken,
    ...
    return {
      view: (vnode): m.Children => {
        return m(
          ...
          vnode.attrs.name === "" ? vnode.attrs.emailAddress : vnode.attrs.name,
        );
      },
    };
  }
          ...
          tableColumns: [
            ...
            },
            {
              formatter: (e: AuthHouseholdMember): boolean => {
                return e.permissions.auth === PermissionEnum.Edit;
              },
              ...
              Form,
              {
                buttons: [
                  ...
                  },
                  {
                    ...
                    onclick: async (): Promise<void> => {
                      const aaah = AuthHouseholdState.findMember(
                        state.form.data.id,
                      );

                      if (aaah.inviteToken !== "") {
                        return AuthAccountAuthHouseholdState.inviteDelete(
                          aaah.authHouseholdID,
                          aaah.inviteToken,
                        ).then(async () => {
                          state.form.deleteConfirm = false;
                          state.form.visible = false;
                ...
                onsubmit: async () => {
                  if (state.form.data.id === null) {
                    return AuthAccountAuthHouseholdState.inviteCreate(
                      state.form.data,
                    ).then(() => {
                      state.form.visible = false;
                    });
                  ...
                  return AuthAccountAuthHouseholdState.update(
                    state.form.data,
                  ).then(() => {
                    state.form.visible = false;
                  });
                ...
                title: {
                  name: `${
                    state.form.data.id === null
                      ...
                      : AuthAccountState.translate(ActionUpdate)
                  } ${AuthAccountState.translate(WebGlobalHouseholdMember)}`,
                },
              },
              [
                m(FormItem, {
