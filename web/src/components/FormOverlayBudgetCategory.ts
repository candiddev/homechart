...
import { ObjectBudgetCategoryGroupingIncome, ObjectCategory,   WebFormOverlayBudgetCategoryBudgetedTooltip,  WebFormOverlayBudgetCategoryBudgetZero,WebFormOverlayBudgetCategoryProjects,WebFormOverlayBudgetCategoryTargetAmount,WebFormOverlayBudgetCategoryTargetAmountTooltip, WebFormOverlayBudgetCategoryTargetDate, WebFormOverlayBudgetCategoryTargetDateTooltip, WebFormOverlayBudgetCategoryTargetMonth, WebFormOverlayBudgetCategoryTargetMonthTooltip, WebFormOverlayBudgetCategoryTargetType, WebFormOverlayBudgetCategoryTargetTypeTooltip, WebGlobalActionBudgetTargetAmount, WebGlobalBudgeted, WebGlobalGrouping, WebGlobalGroupingTooltip, WebGlobalName, WebGlobalNameTooltip, WebRecurrenceMonthly, WebRecurrenceYearly } from "../yaml8n";
		...
		view: (vnode): m.Children => {
			return m(FormOverlay, {
				buttons: [
					...
					{
						name: AuthAccountState.translate(WebFormOverlayBudgetCategoryBudgetZero),
						onclick: async (): Promise<void | Err> => {
							if (vnode.attrs.data.grouping === true && BudgetMonthState.data().budgetMonthCategories !== null) {
								BudgetMonthState.data().budgetMonthCategories.map(async (category) => {
									if (category.budgetCategory.grouping === vnode.attrs.data.budgetCategory.name) {
										await BudgetMonthCategoryState.update({
											...category,
											...{
				...
				onDelete: async (): Promise<void | Err> => {
					if (vnode.attrs.data.grouping === true) {
						for (const category of BudgetCategoryState.data()) {
							if (category.grouping === vnode.attrs.data.budgetCategory.name) {
					...
					return BudgetCategoryState.delete(id)
						.then(async () => {
							if (m.route.get()
								.includes("/budget/categories")) {
								return BudgetMonthState.read(vnode.attrs.data.authHouseholdID);
				...
				onSubmit: async (): Promise<void | Err> => {
					...
					if (vnode.attrs.data.id === null) {
						...
						return BudgetCategoryState.create(vnode.attrs.data.budgetCategory)
							.then(async (category) => {
								...
								vnode.attrs.data.yearMonth = BudgetMonthState.yearMonth.toNumber();
					...
					vnode.attrs.data.budgetCategory.id = vnode.attrs.data.budgetCategoryID;
					return BudgetCategoryState.update(vnode.attrs.data.budgetCategory)
						.then(async () => {
							if (vnode.attrs.data.yearMonth === BudgetMonthState.yearMonth.toNumber()) {
								return BudgetMonthCategoryState.update(vnode.attrs.data)
									.then(async () => {
										if (m.route.get()
											.includes("/budget/categories")) {
											return BudgetMonthState.read(vnode.attrs.data.authHouseholdID);
				...
				vnode.attrs.data.grouping === true ?
					...
					[
						...
						m(FormCheckbox, {
							name: AuthAccountState.translate(ObjectBudgetCategoryGroupingIncome),
							onclick: (): void => {
								vnode.attrs.data.budgetCategory.income = !vnode.attrs.data.budgetCategory.income;
							},
						...
						vnode.attrs.data.budgetCategory.income ?
							...
							m(FormItemInputCurrency, {
								...
								tooltip: AuthAccountState.translate(WebFormOverlayBudgetCategoryBudgetedTooltip),
								...
								m(FormItem, {
									buttonArray:{
										...
										value: [
											{
												id: "monthly",
												name: AuthAccountState.translate(WebRecurrenceMonthly),
											},
											{
												id: "yearly",
												name: AuthAccountState.translate(WebRecurrenceYearly),
											},
											{
												id: "date",
												name: AuthAccountState.translate(FormRecurrenceSpecificDate),
											},
										],
									},
									name: AuthAccountState.translate(WebFormOverlayBudgetCategoryTargetType),
									tooltip: AuthAccountState.translate(WebFormOverlayBudgetCategoryTargetTypeTooltip),
								}),
								picker === "" ?
									[] :
									m(FormItemInputCurrency, {
										...
										tooltip: AuthAccountState.translate(WebFormOverlayBudgetCategoryTargetAmountTooltip),
										value: vnode.attrs.data.budgetCategory.targetAmount,
									}),
								picker === "yearly" ?
									m(FormItem, {
										name: AuthAccountState.translate(WebFormOverlayBudgetCategoryTargetMonth),
										select: {
											oninput: (e: string): void => {
												if (e === "") {
													vnode.attrs.data.budgetCategory.targetMonth = 0;
												} else {
													vnode.attrs.data.budgetCategory.targetMonth = Translations.monthValues.indexOf(e);
												}
											},
											options: Translations.monthValues,
											required: false,
											value: Translations.monthValues[vnode.attrs.data.budgetCategory.targetMonth],
										},
										tooltip: AuthAccountState.translate(WebFormOverlayBudgetCategoryTargetMonthTooltip),
									}) :
									[],
								picker === "date" ?
									m(FormItem, {
										input: {
											oninput: (e: string): void => {
												if (e === "") {
													...
													vnode.attrs.data.budgetCategory.targetMonth = parseInt(e.split("-")[1], 10);
													vnode.attrs.data.budgetCategory.targetYear = parseInt(e.split("-")[0], 10);
												}
											},
											required: false,
											type: "month",
											value: vnode.attrs.data.budgetCategory.targetYear !== 0 && vnode.attrs.data.budgetCategory.targetMonth !== 0 ?
												`${vnode.attrs.data.budgetCategory.targetYear}-${`${vnode.attrs.data.budgetCategory.targetMonth}`.padStart(2, "0")}` :
												"",
										},
										name: AuthAccountState.translate(WebFormOverlayBudgetCategoryTargetDate),
										tooltip: AuthAccountState.translate(WebFormOverlayBudgetCategoryTargetDateTooltip),
									}) :
									[],
							],
						PlanProjectState.findNamesBudgetCategoryID(vnode.attrs.data.budgetCategoryID).length > 0 ?
							m(FormItem, {
								name: AuthAccountState.translate(WebFormOverlayBudgetCategoryProjects),
								textArea: {
									disabled: true,
									value: PlanProjectState.findNamesBudgetCategoryID(vnode.attrs.data.budgetCategoryID)
										.map((project) => {
