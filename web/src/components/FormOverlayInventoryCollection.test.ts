...
test("FormOverlayInventoryCollection", async () => {
  ...
  expect(InventoryCollectionState.delete).toBeCalledWith("1");
  expect(testing.mocks.route).toBe("/inventory");
  testing.click("#button-cancel");
  testing.click("#button-update");
  expect(InventoryCollectionState.update).toBeCalledTimes(1);
  inventoryCollection.id = null;
  testing.redraw();
  testing.click("#button-add");
  await testing.sleep(100);
  expect(InventoryCollectionState.create).toBeCalledTimes(1);
  expect(testing.mocks.route).toBe("/inventory/1");
  ...
  expect(inventoryCollection.name).toBe("Test1");
  ...
  expect(inventoryCollection.grouping).toBe("Test1");

  testing.input("#form-item-input-icon", "a");
  testing.value("#form-item-input-icon", "a");
  expect(inventoryCollection.icon).toBe("a");
