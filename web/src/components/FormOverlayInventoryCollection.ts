...
import { ObjectCollection, WebFormOverlayInventoryCollectionPropertyFilters, WebFormOverlayInventoryCollectionPropertyFiltersTooltip, WebGlobalActionFilter, WebGlobalGrouping, WebGlobalGroupingTooltip, WebGlobalImage, WebGlobalName, WebGlobalNameTooltip, WebGlobalQRCode } from "../yaml8n";

export function FormOverlayInventoryCollection (): m.Component<FormOverlayComponentAttrs<InventoryCollection>> {
	return {
		...
		oninit: async (vnode): Promise<void> => {
			if (vnode.attrs.data.id !== null && vnode.attrs.data.qrlink === undefined) {
				vnode.attrs.data.qrlink = InventoryCollectionState.getLink(vnode.attrs.data.id);
		...
		view: (vnode): m.Children => {
			return m(FormOverlay, {
				...
				onDelete: async (): Promise<void | Err> => {
					return InventoryCollectionState.delete(vnode.attrs.data.id)
						.then(() => {
							if (m.route.get()
								.includes(`/inventory/${vnode.attrs.data.id}`)) {
				...
				onSubmit: async (): Promise<InventoryCollection | void | Err> => {
					if (vnode.attrs.data.id === null) {
						return InventoryCollectionState.create(vnode.attrs.data)
							.then((collection) => {
								if (IsErr(collection)) {
									return collection;
								}
				...
				m(FormItemInputProperties, {
					...
					name: AuthAccountState.translate(WebFormOverlayInventoryCollectionPropertyFilters),
					nameSingle: AuthAccountState.translate(WebGlobalActionFilter),
					properties: InventoryItemState.properties(),
					tooltip: AuthAccountState.translate(WebFormOverlayInventoryCollectionPropertyFiltersTooltip),
