...
describe("FormItemAutocomplete", () => {
  ...
  test.each([
    ...
    [
      "Secrets > Value",
      ["Homechart", "Jennifer Doe", "Local Credit Union", "Mom's Garage Code"],
      "Homechart",
    ...
    expect(FormItemAutocomplete.parse(input1)).toStrictEqual({
      options: options,
      splice: `${input1.toLowerCase().replace(" > ", "")}/`,
      visible: true,
    });
    expect(FormItemAutocomplete.parse(input2)).toStrictEqual({
      options: [],
      splice: `${splice} `,
      visible: false,
    });
