...
export function CalendarDay(): m.Component<CalendarDayAttrs> {
  return {
    view: (vnode): m.Children => {
      return m(
        "div.CalendarDay__container#calendar-day",
        {
          class:
            vnode.attrs.picker.month === vnode.attrs.date.month
              ? undefined
        ...
        [
          m(
            "div.CalendarDay__date",
            {
              class:
                CivilDate.now().valueOf() === vnode.attrs.date.valueOf()
              ...
              m(Icon, {
                ...
                onclick: async (e: m.Event<MouseEvent>) => {
                  ...
                  AppState.setComponentsDropdownMenu(
                    `calendarday_${vnode.attrs.date.toJSON()}`,
                    e.clientY,
                  );
                },
              }),
