...
test("FormOverlayBudgetPayee", async () => {
  ...
  expect(BudgetPayeeState.delete).toBeCalledTimes(1);
  testing.click("#button-cancel");
  testing.click("#button-update");
  expect(BudgetPayeeState.update).toBeCalledTimes(1);
  budgetPayee.id = null;
  testing.redraw();
  testing.click("#button-add");
  expect(BudgetPayeeState.create).toBeCalledTimes(1);
  ...
  testing.findAll(
    "#form-item-datalist-name > option",
    seed.budgetPayees.length,
  );
  testing.input("#form-item-input-name", "bread");
  expect(budgetPayee.name).toBe("bread");
  testing.input("#form-item-text-area-address", "123");
  expect(budgetPayee.address).toBe("123");
  testing.input("#form-item-input-icon", "test");
  expect(budgetPayee.icon).toBe("test");
  testing.input("#form-item-select-default-category", "Food > Groceries");
  expect(budgetPayee.budgetCategoryID).toBe(
    BudgetCategoryState.findGroupName("Food > Groceries").id,
  );
  ...
  expect(budgetPayee.shopStore).toBeTruthy();
  testing.input("#form-item-input-name", BudgetPayeeState.data()[1].name);
  expect(budgetPayee.budgetCategoryID).toStrictEqual(
    BudgetPayeeState.data()[1].budgetCategoryID,
  );
});
