...
import { ObjectEvent, WebFormOverlayCalendarEventAllDay, WebFormOverlayCalendarEventCustomReminder, WebFormOverlayCalendarEventCustomReminderTooltip,  WebFormOverlayCalendarEventEndTime, WebFormOverlayCalendarEventEndTimeTooltip, WebFormOverlayCalendarEventLocation, WebFormOverlayCalendarEventLocationTooltip, WebFormOverlayCalendarEventParticipants, WebFormOverlayCalendarEventParticipantsTooltip, WebFormOverlayCalendarEventStartTime, WebFormOverlayCalendarEventStartTimeTooltip, WebFormOverlayCalendarEventTimeZoneTooltip, WebFormOverlayCalendarEventTravelTime, WebFormOverlayCalendarEventTravelTimeTooltip, WebGlobalActionCopy, WebGlobalDetails, WebGlobalDetailsTooltips, WebGlobalName, WebGlobalNameTooltip, WebGlobalRecurring, WebGlobalSendReminder, WebGlobalSendReminderTooltip, WebGlobalTimeZone, WebRecurrenceOccurrence } from "../yaml8n";
...
function findReminderTime (offset: null | number): number {
	const index = Translations.calendarEventReminderTimes
		.findIndex((reminder) => {
...
export function FormOverlayCalendarEvent (): m.Component<FormOverlayComponentAttrs<CalendarEvent>> {
	let notifyCustom = false;
	const dateStart = Stream("");
	const duration = Stream(0);
	const timeStart = Stream("");
	const start = Stream.lift((d, t) => {
		return Timestamp.fromString(`${d}T${t}:00.000Z`);
	}, dateStart, timeStart);

	const end = Stream.lift((d, s) => {
	...
	return {
		...
		view: (vnode): m.Children => {
			return m(FormOverlay, {
				buttons: [
					{
						...
						onclick: async (): Promise<void> => {
							if (vnode.attrs.data.recurrenceDate !== undefined && vnode.attrs.data.recurrenceDate !== null) {
								vnode.attrs.data.skipDays.push(vnode.attrs.data.recurrenceDate);
						...
						permitted: GlobalState.permitted(PermissionComponentsEnum.Calendar, true) && vnode.attrs.data.id !== null && vnode.attrs.data.recurrence !== null,
						requireOnline: true,
					},
					{
						...
						permitted: vnode.attrs.data.id !== null && GlobalState.permitted(PermissionComponentsEnum.Calendar, true),
						requireOnline: true,
					},
					{
						name: `${AuthAccountState.translate(ActionUpdate)} ${AuthAccountState.translate(WebRecurrenceOccurrence)}`,
						onclick: async (): Promise<void> => {
							if (vnode.attrs.data.recurrenceDate !== undefined && vnode.attrs.data.recurrenceDate !== null && ! vnode.attrs.data.skipDays.includes(vnode.attrs.data.recurrenceDate)) {
						...
						permitted: GlobalState.permitted(PermissionComponentsEnum.Calendar, true) && vnode.attrs.data.recurrence !== null && vnode.attrs.data.id !== null,
						primary: true,
						requireOnline: true,
					},
				],
				...
				permitted: GlobalState.permitted(PermissionComponentsEnum.Calendar, true, vnode.attrs.data.authHouseholdID),
			}, [
				...
				vnode.attrs.data.authHouseholdID === null ?
					...
					m(FormItemSelectAuthHouseholdMembers, {
						...
						multiple: true,
						name: AuthAccountState.translate(WebFormOverlayCalendarEventParticipants),
						oninput: (members: string[]): void => {
							vnode.attrs.data.participants = members;
						},
						tooltip: AuthAccountState.translate(WebFormOverlayCalendarEventParticipantsTooltip),
					}),
				m(FormItem, {
				...
				m(FormItem, {
					...
					name: AuthAccountState.translate(WebFormOverlayCalendarEventLocation),
					tooltip: AuthAccountState.translate(WebFormOverlayCalendarEventLocationTooltip),
				}),
				m(FormItemDuration, {
					getDuration: () => {
						return vnode.attrs.data.travelTime;
					},
					name: AuthAccountState.translate(WebFormOverlayCalendarEventTravelTime),
					setDuration: (duration: number) => {
						vnode.attrs.data.travelTime = duration;
					},
					tooltip: AuthAccountState.translate(WebFormOverlayCalendarEventTravelTimeTooltip),
				}),
				m("div.FormItem", {
					...
					m("div.FormItem__label", [
						...
						}, AuthAccountState.translate(WebFormOverlayCalendarEventStartTime)),
						m(Tooltip, {
							value: AuthAccountState.translate(WebFormOverlayCalendarEventStartTimeTooltip),
						}),
					]),
				...
				m("div.FormItem", {
					...
					m("div.FormItem__label", [
						m("label", {
							for: "form-item-input-start",
							id: "form-item-label-start",
						}, AuthAccountState.translate(WebFormOverlayCalendarEventEndTime)),
						m(Tooltip, {
							value: AuthAccountState.translate(WebFormOverlayCalendarEventEndTimeTooltip),
						}),
					]),
					m("div.FormItem__multi", [
						m(FormItemInput, {
							...
							oninput: (e): void => {
								...
								vnode.attrs.data.duration = start()
									.getDiffMinutes(endT);
							},
							required: true,
							type: "date",
				...
				m(FormItem, {
					...
					select: {
						oninput: (e: number): void => {
							...
							vnode.attrs.data.notifyOffset = Translations.calendarEventReminderTimes[e].offset;
						},
						options: Translations.calendarEventReminderTimes
							.map((reminder, index) => {
								return {
									id: `${index}`,
									name: reminder.name,
				...
				notifyCustom ?
					m(FormItemDuration, {
						...
						name: AuthAccountState.translate(WebFormOverlayCalendarEventCustomReminder),
						setDuration: (duration: number) => {
							vnode.attrs.data.notifyOffset = duration;
						},
						tooltip: AuthAccountState.translate(WebFormOverlayCalendarEventCustomReminderTooltip),
					}) :
					[],
				...
				m(FormItem, {
					...
					tooltip: AuthAccountState.translate(WebFormOverlayCalendarEventTimeZoneTooltip),
				}),
				m(FormCheckbox, {
