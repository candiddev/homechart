...
class HealthLogManager extends DataArrayManager<HealthLog> {
	...
	findDateRange (from: CivilDate, to: CivilDate): Stream<CalendarEventRange> {
		return this.data.map((logs) => {
			...
			for (const log of logs) {
				...
				if (date.valueOf() <= toValue && date.valueOf() >= fromValue && ! AuthAccountState.data().preferences.hideCalendarHealthLogs.includes(log.authAccountID as string)) {
					const index = events.findIndex((event) => {
						return event.authAccountID === log.authAccountID && event.timestampStart === ts.toString();
					});
	...
	findAuthAccountID (authAccountID: NullUUID, date?: NullCivilDate): Stream<HealthLog[]> {
		return this.data.map((items) => {
			return items.filter((item) => {
				return item.authAccountID === authAccountID && (date === undefined || item.date === date);
			});
		});
	}
