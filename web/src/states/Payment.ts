...
export const PaymentState: {
	...
	createPaddle(authHouseholdID: NullUUID, processor: AuthHouseholdSubscriptionProcessorEnum): Promise<PaddleURL | Err>,
	inResponsePaddleConfig(response: APIResponse<unknown>): response is APIResponse<PaddleConfig[]>,
	inResponsePaddleURL(response: APIResponse<unknown>): response is APIResponse<PaddleURL[]>,
	paddle: PaddleConfig,
	prices: Stream<{
		monthly: string,
		lifetime: string,
	...
	update(authHouseholdID: NullUUID, processor: AuthHouseholdSubscriptionProcessorEnum): Promise<void | Err>,
} = {
	cancel: async (authHouseholdID: NullUUID): Promise<void | Err> => {
		return API.delete("/api/v1/payments", {
			authHouseholdID: InfoState.data().cloud ?
				authHouseholdID :
				undefined,
			selfHostedID: InfoState.data().cloud ?
				undefined :
				authHouseholdID,
		})
			.then(async (err) => {
	...
	createPaddle: async (authHouseholdID: NullUUID, processor: AuthHouseholdSubscriptionProcessorEnum): Promise<PaddleURL | Err> => {
	...
	readPaddle: async (): Promise<void | Err> => {
		...
		return new Promise((resolve) => {
			...
			e.onload = async (): Promise<void> => {
				return API.read("/api/v1/payments/paddle", {})
					.then(async (response) => {
						if (IsErr(response)) {
							AppState.setLayoutAppAlert(response);
						...
						if (PaymentState.inResponsePaddleConfig(response)) {
							...
							Paddle.Product.Prices(PaymentState.paddle.planIDMonthly, (price) => {
								PaymentState.prices({
									...PaymentState.prices(),
									...{
							...
							Paddle.Product.Prices(PaymentState.paddle.productIDLifetime, (price) => {
								PaymentState.prices({
									...PaymentState.prices(),
									...{
										...{
	...
	update: async (authHouseholdID: NullUUID, processor: AuthHouseholdSubscriptionProcessorEnum): Promise<void | Err> => {
