...
import { ObjectMealPlanCreated, ObjectMealPlanDeleted, ObjectMealPlanUpdated } from "../yaml8n";
...
class CookMealPlanManager extends DataArrayManager<CookMealPlan> {
	...
	findDateRange (from: CivilDate, to: CivilDate): Stream<CalendarEventRange> {
		return this.data.map((plans) => {
			...
			for (const plan of plans) {
				...
				const color = Colors.cookMealPlan(AuthHouseholdState.findID(plan.authHouseholdID).preferences.colorCookMealPlanEvents);
				...
				if (date <= toValue && date >= fromValue) {
					...
					if (plan.cookRecipeID !== null && plan.notificationTimePrep !== null && recipe.timePrep !== 0) {
						...
						events.push({
							...
							...{
								...
								participants: plan.authAccountID === null ?
									[] :
									[
										plan.authAccountID,
									],
								timeStart: time.toCivilTime()
									.toString(true),
					...
					if (plan.cookRecipeID !== null && plan.notificationTimeCook !== null && recipe.timeCook !== 0) {
					...
					const start = Timestamp.fromString(new Date(date.year, date.month - 1, date.day, time.hour, time.minute, 0, 0)
						.toISOString());
					const end = Timestamp.fromString(start.toString());
					end.addMinutes(30);

					const index = events.findIndex((event) => {
						return event.dateStart === date.toJSON() && event.name === meal.name; // eslint-disable-line @typescript-eslint/no-non-null-assertion
					});
					...
					if (index < 0) {
						...
						if (plan.authAccountID !== null && ! events[index].participants.includes(plan.authAccountID)) {
	...
	toShopItems (plans?: CookMealPlan[], cookRecipeShortIDs?: string[]): ShopItem[] {
		let items: ShopItem[] = [];
		...
		for (const plan of plans) {
			...
			if (recipe.id !== null) {
				...
				const ingredients = CookRecipeState.scaleIngredients(recipe.ingredients, plan.cookRecipeScale);
				...
				for (let i = 0; i < lines.length; i++) {
					...
					if (shortID.test(name)) {
						...
						if (ids.includes(recipeID))  {
							...
							const newMealPlan: CookMealPlan = { ...this.new(),
								...{
									cookRecipeID: subrecipe.id,
									cookRecipeScale: recipeQuantity,
								},
							};
							items = items.concat(this.toShopItems([
								newMealPlan,
							], ids));
							continue;
						}
