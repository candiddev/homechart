...
export class DataArrayManager<T extends Data> {
      ...
      if (this.new().tags !== undefined) {
        this.tags = this.data.map((data) => {
          return this.formatTags(
            AggregateTags(this.getTaggable(data) as DataTags[]),
          );
        });
  ...
  async create(data: T, hideAlert?: boolean): Promise<T | Err> {
    ...
    return API.create(this.path, data).then(async (response) => {
  ...
  async delete(id: NullUUID, hideAlert?: boolean): Promise<void | Err> {
    ...
    return API.delete(`${this.path}/${id}`).then(async (response) => {
  ...
  findBatch(id: NullUUID): T {
    const o = this.batch.find((object) => {
      return object.id === id;
    });
  ...
  findName(name: string, personal?: boolean): T {
    const i = this.data().findIndex((object) => {
      if (personal === true) {
        return (
          object.authAccountID !== null &&
          object.authHouseholdID === null &&
          object.name === name
        );
      }
  ...
  async read(
    ...
    if (
      (updated !== undefined && updated === this.findID(id).updated) ||
      updated === this.findBatch(id).updated
    ) {
      return;
    }
  ...
  async update(data: T, hideAlert?: boolean): Promise<void | Err> {
    ...
    return API.update(`${this.path}/${data.id}`, data).then(
      async (response) => {
          ...
          this.alertAction(ActionsEnum.Update, hideAlert, [
            {
              ...
              onclick: async (): Promise<void> => {
                return this.update(this.undo as T, true).then((res) => {
                  if (IsErr(res)) {
