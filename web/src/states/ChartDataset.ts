...
export const ChartDatasetState = {
  ...
  ): response is APIResponse<ChartDataset[]> {
    return response.dataType === "ChartDatasets";
  },
  read: async (
    authHouseholdID: NullUUID,
    dateRange: YearMonthDateRange,
    type: ChartDatasetTypesEnum,
  ): Promise<ChartDataset[] | Err> => {
    return API.read(
      `/api/v1/charts/datasets/${authHouseholdID}/${type}?from=${dateRange.from.toNumber()}&to=${dateRange.to.toNumber()}`,
      {},
    ).then((response) => {
      if (IsErr(response)) {
        return response;
