...
describe("BudgetCategoryState", () => {
  ...
  test("findHeaderName", async () => {
    expect(
      BudgetCategoryState.findGroupName(
        `${seed.budgetCategories[1].grouping} > ${seed.budgetCategories[1].name}`,
      ),
    ).toStrictEqual(seed.budgetCategories[1]);
    expect(
      BudgetCategoryState.findGroupName("Hidden > Starting Balance"),
    ).toStrictEqual(seed.budgetCategories[0]);
  });

  test("findIDHeaderName", async () => {
    expect(
      BudgetCategoryState.findIDHeaderName(seed.budgetCategories[0].id),
    ).toBe(`Hidden > ${seed.budgetCategories[0].name}`);
    expect(
      BudgetCategoryState.findIDHeaderName(seed.budgetCategories[1].id),
    ).toBe(
      `${seed.budgetCategories[1].grouping} > ${seed.budgetCategories[1].name}`,
    );
    expect(BudgetCategoryState.data()[0].grouping).toBe("");
  });

  test("groupingNames", () => {
    expect(BudgetCategoryState.groupingNames()).toStrictEqual([
      "Bills",
      "Food",
      "Home",
      "Income",
  ...
  test("names", async () => {
    expect(BudgetCategoryState.names()).toStrictEqual([
      "Bills > Mortgage/Rent",
      "Bills > Utilities",
      "Food > Groceries",
      "Food > Pistachios",
  ...
  test("nameOptions", async () => {
    expect(BudgetCategoryState.nameOptions()[0]).toStrictEqual({
