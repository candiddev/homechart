...
describe("PlanProject", () => {
	...
	PlanProjectState.data.map((data) => {
		PlanProjectState.nested(FilterSortChildren({
			input: Clone(data),
		}));
	});
	...
	test("allTags", () => {
		...
		expect(PlanProjectState.allTags()[2])
			.toStrictEqual({
				count: 5,
				name: "chores",
			});

		expect(PlanProjectState.allTagNames()[0])
			.toBe("book");
	});
	...
	test("create", async () => {
		...
		expect(await PlanProjectState.create({
			...PlanProjectState.data()[0],
			...{
				name: "a1",
			},
	...
	test("household", () => {
		expect(PlanProjectState.household()[0])
			.toStrictEqual({
				...seed.planProjects[5],
				...{
					children: [
	...
	test("nested", () => {
		expect(PlanProjectState.nested()[0].children)
			.toStrictEqual(filterAndSorted);
	});

	test("personal", () => {
		expect(PlanProjectState.personal()[0].children)
			.toStrictEqual(filterAndSorted);
	});

	test("findAdjacent", () => {
		expect(PlanProjectState.findAdjacent(seed.planProjects[5]))
			.toStrictEqual([
				seed.planProjects[5],
			]);
		expect(PlanProjectState.findAdjacent(seed.planProjects[0]))
			.toStrictEqual([
				seed.planProjects[0],
				seed.planProjects[10],
			]);
		expect(PlanProjectState.findAdjacent(seed.planProjects[2]))
			.toStrictEqual([
				seed.planProjects[2],
				seed.planProjects[3],
				seed.planProjects[4],
	...
	test("findChildren", () => {
		expect(PlanProjectState.findChildren(seed.planProjects[0].id))
			.toStrictEqual(filterAndSorted);
		expect(PlanProjectState.findChildren(seed.planProjects[3].id))
			.toStrictEqual([]);
	});
	...
	test("findCount", () => {
		...
		expect(PlanProjectState.findCount(input))
			.toBe(10);
	...
	test("findNamesBudgetCategoryID", () => {
		expect(PlanProjectState.findNamesBudgetCategoryID(seed.budgetCategories[2].id))
			.toStrictEqual([
				seed.planProjects[5],
			]);
		expect(PlanProjectState.findNamesBudgetCategoryID(null))
			.toStrictEqual([]);
	});

	test("findTag", () => {
		expect(PlanProjectState.findTag("todo"))
			.toStrictEqual([
				PlanProjectState.data()[0],
			]);
	});
	...
	test("getColorNamesID", () => {
		...
		expect(PlanProjectState.getColorNamesIDs())
			.toStrictEqual(output.slice(0, 7));
		...
		expect(PlanProjectState.getColorNamesIDs())
			.toStrictEqual(output.slice(0, 1));
		AuthAccountState.data(seed.authAccounts[0]);
	});
});
