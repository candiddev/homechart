...
export const BudgetMonthState = {
	containsResponse: (response: APIResponse<unknown>): response is APIResponse<BudgetMonth[]> => {
	...
	inResponse: (response: APIResponse<unknown>): response is APIResponse<BudgetMonth[]> => {
	...
	load: async (): Promise<BudgetMonth> => {
		return IndexedDB.get("BudgetMonth")
			.then(async (data) => {
	...
	read: async (authHouseholdID: NullUUID): Promise<void | Err> => {
		return API.read(`/api/v1/budget/months/${authHouseholdID}/${BudgetMonthState.yearMonth.toNumber()}`, {})
			.then(async (response) => {
				if (IsErr(response)) {
					return response;
				}
				...
				if (BudgetMonthState.containsResponse(response)) {
					response.dataValue[0].budgetMonthCategories = BudgetMonthCategoryState.groupingSort(response.dataValue[0]);

					for (const category of response.dataValue[0].budgetMonthCategories) {
						if (category.budgetCategory.grouping === "" || BudgetMonthState.groupings.includes(category.budgetCategory.grouping)) {
							continue;
						}
					...
					m.redraw();
					if (BudgetMonthState.data().yearMonth === Timestamp.now()
						.toCivilDate()
						.toYearMonth()
						.toNumber()) {
