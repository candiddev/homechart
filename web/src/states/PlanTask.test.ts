...
describe("PlanTaskState", () => {
	...
	PlanTaskState.data.map((data) => {
		PlanTaskState.nested(FilterSortChildren({
			input: Clone(data),
		}));
	});
	...
	test("household", () => {
		...
		expect(PlanTaskState.household())
			.toStrictEqual([
				{
					...data[1],
					...{
	...
	test("dueDatePast", () => {
		...
		expect(PlanTaskState.dueDatePast())
			.toStrictEqual([
				{
					...data[0],
					...{
	...
	test("findAdjacent", () => {
		PlanTaskState.data(seed.planTasks);
		expect(PlanTaskState.findAdjacent(seed.planTasks[1]))
			.toStrictEqual([
				seed.planTasks[0],
				seed.planTasks[1],
			]);
		expect(PlanTaskState.findAdjacent(seed.planTasks[5]))
			.toStrictEqual([
				seed.planTasks[5],
				seed.planTasks[6],
				seed.planTasks[7],
	...
	test("findDateRange", async () => {
		...
		expect(PlanTaskState.findDateRange(from.toCivilDate(), to.toCivilDate())())
			.toStrictEqual(CalendarEventState.toCalendarEventsRange([
				...
				},
				{
					...
					...{
						...
						timeStart: midnight.toCivilTime()
							.toString(true),
				...
				},
				{
					...
					...{
						...
						name: data[2].name,
						planTask: data[2],
						timeStart: to.toCivilTime()
							.toString(true),
						timestampEnd: to.toString(),
						timestampStart: to.toString(),
					},
				},
			], from.toCivilDate(), to.toCivilDate()));
	});
	...
	test("findDueDate", () => {
		...
		expect(PlanTaskState.findDueDate(yesterday.toCivilDate()
			.toJSON()))
			.toStrictEqual([
				{
					...data[0],
		...
		expect(PlanTaskState.findDueDate(today.toCivilDate()
			.toJSON()))
			.toStrictEqual([
				{
					...data[2],
		...
		expect(PlanTaskState.findDueDate(tomorrow.toCivilDate()
			.toJSON()))
			.toStrictEqual([
				{
					...data[3],
	...
	test("findParentIDProjectID", () => {
		...
		expect(PlanTaskState.findParentIDProjectID("0"))
			.toBe("0");
	});
	...
	test("findPlanProjectID", () => {
		...
		expect(PlanTaskState.findPlanProjectID("0"))
			.toStrictEqual([
				{
					...data[0],
					...{
		...
		expect(PlanTaskState.findPlanProjectID(null))
			.toStrictEqual([]);
	});
	...
	test("findInventoryItemID", () => {
		...
		expect(PlanTaskState.findInventoryItemID("0"))
			.toStrictEqual([
				data[0],
			]);
	...
	test("findTag", () => {
		...
		expect(PlanTaskState.findTag("b"))
			.toStrictEqual([
				data[1],
				data[0],
			]);
	...
	test("findTemplates", () => {
		...
		expect(PlanTaskState.findTemplates(PlanTaskTemplateType.Homechart))
			.toStrictEqual([
				{
					...data[3],
					...{
		...
		expect(PlanTaskState.findTemplates(PlanTaskTemplateType.Account))
			.toStrictEqual([
				{
					...data[4],
					...{
		...
		expect(PlanTaskState.findTemplates(PlanTaskTemplateType.Household))
			.toStrictEqual([
				{
					...data[5],
					...{
	...
	test("getChildrenDone", () => {
		...
		expect(PlanTaskState.getChildrenDone({
			...task,
			...{
				children: [
					{
	...
	test("getCountToday", () => {
		...
		expect(PlanTaskState.getCountToday(PlanTaskState.data()))
			.toBe(2);
	});
	...
	test("getCountUpcoming", () => {
		...
		expect(PlanTaskState.getCountUpcoming(PlanTaskState.data()))
			.toBe(1);
	});

	test("getDuration", () => {
		expect(PlanTaskState.getDuration([
			{
				...PlanTaskState.new(),
				...{
					children: [
	...
	test("updateDone", async () => {
		...
		expect(AppState.getLayoutAppAlerts()[0].message)
			.toBe(`Next due date ${AppState.formatCivilDate(timestamp1.toCivilDate())}`);
