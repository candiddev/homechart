...
export class DataManager<T extends Data> {
	...
	async create (hideAlert?: boolean): Promise<void | Err> {
		...
		return API.create(this.path, this.data())
			.then(async (response) => {
	...
	async load (): Promise<void | Err> {
		return IndexedDB.get(this.typeObject)
			.then(async (data) => {
				if (this.is(data)) {
					return this.set(
						{
	...
	async read (n?: TableNotify): Promise<void | Err> {
		if (!this.readLock && (n === undefined || this.data().updated !== n.updated)) {
