...
export const OIDCState = {
	...
	inResponseProviders: (response: APIResponse<unknown>): response is APIResponse<OIDCProviderTypeEnum[]> => {
		return response.dataType === "OIDCProviders";
	},
	inResponseRedirect: (response: APIResponse<unknown>): response is APIResponse<OIDCRedirect[]> => {
	...
	readRedirect: async (providerType: OIDCProviderTypeEnum): Promise<OIDCRedirect | Err> => {
		return API.read(`/api/v1/oidc/${OIDCProviderType.values[providerType].toLowerCase()}`, {})
			.then((response) => {
				if (IsErr(response)) {
					return response;
				}
	...
	readResponse: async (code: string, providerString: string, state: string): Promise<void> => {
		const provider = parseInt(providerString, 10);
		...
		case OIDCAction.SignIn:
			...
			return AuthAccountState.createSession(session, await API.getHostname())
				.then(async (err) => {
					if (IsErr(err)) {
