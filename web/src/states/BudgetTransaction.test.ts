...
describe("BudgetTransactionState", () => {
  ...
  test("load/set", async () => {
    ...
    expect(transactions).toStrictEqual(seed.budgetTransactions);
  });
    ...
    testing.mocks.responses = [
      {
        ...
        dataValue: [BudgetTransactionState.new(), BudgetTransactionState.new()],
      },
    ];

    BudgetTransactionState.offset = 50;
    await BudgetTransactionState.readAccount("1");
    expect(BudgetTransactionState.data()).toHaveLength(2);
  ...
  test("readCategory", async () => {
    testing.mocks.responses = [
      {
        dataType: "BudgetTransactions",
        dataValue: [BudgetTransactionState.new()],
      },
    ];

    BudgetTransactionState.offset = 50;
    await BudgetTransactionState.readCategory("1");
    expect(BudgetTransactionState.data()).toHaveLength(1);
