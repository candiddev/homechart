...
import {
  ObjectHouseholdCreated,
  ObjectHouseholdDeleted,
  ObjectHouseholdUpdated,
} from "../yaml8n";
...
class AuthHouseholdManager extends DataArrayManager<AuthHousehold> {
  ...
  async deleteMember(authHouseholdID: NullUUID, id: NullUUID): Promise<void> {
    return API.delete(
      `/api/v1/auth/households/${authHouseholdID}/members/${id}`,
    )
      .then(async () => {
        AppState.setLayoutAppAlert({
  ...
  findMember(
    ...
    if (authHouseholdID === undefined) {
      ...
      const i = ah.members.findIndex((member) => {
        return (
          member.id === detail ||
  ...
  findMemberNames(authHouseholdID: NullUUID, children?: boolean): string[] {
    if (authHouseholdID === null) {
      ...
      for (const member of this.members()) {
        if (
          (member.id === AuthAccountState.data().id ||
            member.child ||
            children !== true) &&
  ...
  getPermitted(
    ...
    return this.data().filter((household) => {
  ...
  isExpired(id: NullUUID): boolean {
    return (
      CivilDate.fromString(this.findID(id).subscriptionExpires!) <=
      CivilDate.now()
  ...
  async readAdmin(
    filter?: string,
    offset?: number,
  ): Promise<APIResponse<AuthHousehold[]> | Err> {
  ...
  async updateHideComponents(
    authHouseholdID: NullUUID,
    component: string,
  ): Promise<void | Err> {
    const newComponents: string[] = Clone(
      this.findID(authHouseholdID).preferences.hideComponents,
    );
