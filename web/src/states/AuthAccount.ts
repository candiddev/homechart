...
import {
  ...
  ObjectAccountVerified,
  Translate,
  WebAlertWebCrypto,
  WebAuthAccountPasswordReset,
  WebAuthAccountVerificationSent,
  WebAuthAccountVerifyEmail,
...
interface AuthAccountPreferences {
  [key: string]:
    | AuthAccountPreferencesNotificationsHouseholds[]
    | boolean
    | number
    | string
    | string[]
...
export const AuthAccountPrivateKeyProviderNone =
  "none" as AuthAccountPrivateKeyProvider;
export const AuthAccountPrivateKeyProviderPasswordPBKDF2 =
  "passwordPBKDF2" as AuthAccountPrivateKeyProvider;
...
export class AuthAccountManager extends DataManager<AuthAccount> {
  ...
  async createAccount(
    ...
    if (account.password !== "") {
      ...
      if (IsErr(keys)) {
        ...
        const key = await this.newPrivateKeyPBKDF2(
          account.password,
          (keys.privateKey as Key).string(),
        );
    ...
    return API.create(this.path, account).then(async (response) => {
      if (IsErr(response)) {
  ...
  async createReset(account: AuthAccount): Promise<void> {
    return API.create("/api/v1/auth/reset", account).then(() => {
      AppState.setLayoutAppAlert({
  ...
  async createSession(
    ...
    return API.create("/api/v1/auth/signin", account).then(async (response) => {
      if (IsErr(response)) {
        return response;
      }
  ...
  async createTOTP(): Promise<AuthAccount | Err> {
    return API.create(`${this.path}/${this.data().id}/totp`).then(
      (response) => {
        if (IsErr(response)) {
          return response;
        }
  ...
  async decryptPrivateKeys(
    ...
    if (this.privateKey().key === "") {
      for (let i = 0; i < this.data().privateKeys.length; i++) {
        ...
        if (!IsErr(e)) {
          if (
            this.data().privateKeys[i].provider ===
            AuthAccountPrivateKeyProviderNone
          ) {
            const key = await new Key("", "", "").decrypt(e);
            if (!IsErr(key)) {
  ...
  async deleteICalendarID(): Promise<void> {
    return API.delete("/api/v1/icalendar").then(() => {});
  }
  ...
  async deleteSessions(): Promise<void> {
    return API.delete(`${this.path}/${this.data().id}/sessions`).then(() => {});
  }
  ...
  findNotificationsHousehold(
    notifications: AuthAccountPreferencesNotificationsHouseholds[] | null,
    id: NullUUID,
  ): AuthAccountPreferencesNotificationsHouseholds {
  ...
  async hideCalendarHealthLogs(id: NullUUID): Promise<void | Err> {
    ...
    a.preferences.hideCalendarHealthLogs = PushPopStringArray(
      id,
      a.preferences.hideCalendarHealthLogs,
    );
  ...
  async loadPrivateKey(): Promise<void> {
    return IndexedDB.get("AuthAccountPrivateKey").then((key) => {
  ...
  async newPrivateKeyPBKDF2(
    password: string,
    privateKey?: string,
  ): Promise<string | undefined> {
    const salt = ArrayBufferToBase64(
    ...
    const ev = await k.encrypt(
      privateKey === undefined ? this.privateKey().string() : privateKey,
    );
  ...
  async readAll(
    filter?: string,
    offset?: number,
  ): Promise<APIResponse<AuthAccount[]> | Err> {
  ...
  async readTOTPBackup(): Promise<string | Err> {
    return API.read(`${this.path}/${this.data().id}/totp`, {}).then(
      (response) => {
        if (IsErr(response)) {
          return response;
        }
  ...
  async readVerify(): Promise<void> {
    return API.read("/api/v1/auth/verify", {}).then(() => {
      AppState.setLayoutAppAlert({
  ...
  async savePrivateKey(): Promise<void | Err> {
    return IndexedDB.set("AuthAccountPrivateKey", this.privateKey().string());
  }
  ...
  async updateICalendarID(): Promise<void | Err> {
    return API.update("/api/v1/icalendar").then(async (response) => {
      if (IsErr(response)) {
        return response;
      }
  ...
  async updateHideComponents(component: string): Promise<void | Err> {
    const newComponents: string[] = Clone(
      this.data().preferences.hideComponents,
    );
  ...
  async updateVerify(id: string, token: string): Promise<void | Err> {
    return API.update(`/api/v1/auth/verify?id=${id}&token=${token}`, {}).then(
      (response) => {
        m.route.set("/home");
        ...
        if (response.status !== 200) {
          return NewErr(
            "AuthAccount.updateVerify: wrong status",
            "Error verifying your account, try again later",
          );
        }
