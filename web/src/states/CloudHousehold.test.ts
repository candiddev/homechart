...
describe("CloudHouseholdState", () => {
	...
	test("create", async () => {
		...
		await CloudHouseholdState.create(seed.authHouseholds[0].id, seed.authAccounts[0].emailAddress, "test");
	...
	test("findID", () => {
		...
		expect(CloudHouseholdState.findID("1"))
			.toStrictEqual(a);
	...
	test("read", async () => {
		...
		expect(CloudHouseholdState.data()[0])
	...
	test("readJWT", async () => {
		...
		expect(AuthHouseholdState.data()[0].subscriptionProcessor)
			.toBe(1);
	});
	test("update", async () => {
		const update = {
		...
		expect(CloudHouseholdState.data()[0].selfHostedURL)
