...
import { ObjectHousehold, ObjectProjectCreated, ObjectProjectDeleted, ObjectProjectUpdated, WebGlobalPersonal } from "../yaml8n";
...
class PlanProjectManager extends DataArrayManager<PlanProject> {
	allTags = Stream.lift((projectTags, taskTags) => {
		const tags = [
			...taskTags,
		];
	...
	override async create (t: PlanProject, hideAlert?: boolean): Promise<PlanProject | Err> {
		return super.create({
			...t,
			...{
				children: undefined,
			...
			.then(async (n) => {
				...
				if (t.shortID !== "") {
					for (const a of PlanTaskState.findPlanProjectID(PlanProjectState.findShortID(t.shortID).id)) {
						const task = Clone(a);
	...
	findNamesChildren (level: number, children?: PlanProject[]): FormItemSelectNestedSelector[] {
		...
		if (children !== undefined) {
			for (const project of children) {
				...
				if (Array.isArray(project.children) && project.children.length !== 0) {
					names = names.concat(this.findNamesChildren(level + 1, project.children));
				}
			}
		}
		return names;
	...
	getColorNamesIDs (): FormItemSelectNestedSelector[] {
		...
		names = names.concat(this.findNamesChildren(1, FilterSortChildren({
			input: this.data()
				.filter((project) => {
					return project.authAccountID === AuthAccountState.data().id;
				}),
		...
		if (Permission.isPermitted(AuthSessionState.data().permissionsHouseholds, PermissionComponentsEnum.Plan, PermissionEnum.Edit)) {
			names.push({
				id: "household",
				level: 0,
