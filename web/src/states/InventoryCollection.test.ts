...
describe("InventoryCollectionState", () => {
	...
	test("groupingNames", () => {
		InventoryCollectionState.data(seed.inventoryCollections);
		expect(InventoryCollectionState.groupingNames())
			.toStrictEqual([
				"Filters",
				"Storage",
			]);
	...
	test("getLink", () => {
		...
		expect(InventoryCollectionState.getLink("1"))
			.toBe("http://example.com/inventory/1");
	});

	test("toView", () => {
		expect(InventoryCollectionState.toView({
