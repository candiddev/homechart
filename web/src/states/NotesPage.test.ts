...
const now = Timestamp.now().toString();
...
describe("NotesPage", () => {
  ...
  test("findTag", () => {
    ...
    expect(NotesPageState.findTag("b")).toStrictEqual([data[0], data[1]]);
  });

  test("getNested", () => {
    expect(NotesPageState.nested()).toStrictEqual(filterSorted);
  });

  test("getHousehold", () => {
    expect(NotesPageState.household()).toStrictEqual([filterSorted[1]]);
  });
  ...
  test("getColorNamesID", () => {
    ...
    expect(NotesPageState.getColorNamesIDs()).toStrictEqual(output.slice(0, 4));
    AuthSessionState.data().permissionsHouseholds![0].permissions.notes = 0;
    expect(NotesPageState.getColorNamesIDs()).toStrictEqual(output);
    AuthSessionState.data().permissionsHouseholds![0].permissions.notes = 2;
    AuthAccountState.data(seed.authAccounts[1]);
    expect(NotesPageState.getColorNamesIDs()).toStrictEqual([
      {
        id: "personal",
        level: 0,
        name: "Personal",
  ...
  test("getPersonal", () => {
    expect(NotesPageState.personal()).toStrictEqual([
      filterSorted[0],
      filterSorted[2],
    ]);
  });
