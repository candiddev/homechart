...
describe("OIDCState", () => {
  test("load/set", async () => {
    ...
    expect(state).toStrictEqual(input);
  });
  ...
  test("readProviders", async () => {
    ...
    expect(OIDCState.data()).toStrictEqual([OIDCProviderTypeEnum.Google]);
  });
  ...
  test("readRedirect", async () => {
    ...
    expect(redirect).toStrictEqual(data[0]);
  });
