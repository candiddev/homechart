...
describe("AuthHousehold", () => {
  ...
  test("data", () => {
    ...
    expect(AuthHouseholdState.members()).toHaveLength(6);
    expect(AuthHouseholdState.membersChildren()).toHaveLength(1);

    AuthHouseholdState.data([]);
  });

  test("deleteMember", async () => {
    AuthHouseholdState.data([seed.authHouseholds[0]]);
  ...
  test("findMember/members/name", async () => {
    ...
    expect(
      AuthHouseholdState.findMember(seed.authHouseholds[0].members[0].id),
    ).toStrictEqual(seed.authHouseholds[0].members[0]);
    expect(
      AuthHouseholdState.findMember(seed.authHouseholds[0].members[1].name),
    ).toStrictEqual(seed.authHouseholds[0].members[1]);
    expect(
      AuthHouseholdState.findMember(
        seed.authHouseholds[0].members[2].emailAddress,
      ),
    ).toStrictEqual(seed.authHouseholds[0].members[2]);
    expect(AuthHouseholdState.findMember("1")).toStrictEqual(
      AuthHouseholdState.newMember(),
    );

    expect(
      AuthHouseholdState.findMemberName(seed.authHouseholds[0].members[0].id),
    ).toBe(seed.authHouseholds[0].members[0].name);
    expect(
      AuthHouseholdState.findMemberNames(seed.authHouseholds[0].id),
    ).toStrictEqual([
    ...
    expect(
      AuthHouseholdState.findMemberNames(seed.authHouseholds[0].id, true),
    ).toStrictEqual([
      AuthHouseholdState.members()[3].name,
      AuthHouseholdState.members()[4].name,
    ]);
  ...
  test("getPermitted", () => {
    ...
    ).toStrictEqual([seed.authHouseholds[0]]);
  });
  ...
  test("isExpired", async () => {
    ...
    expect(
      AuthHouseholdState.isExpired(seed.authHouseholds[0].id),
    ).toBeTruthy();
    const now = Timestamp.now();
    now.addDays(100);
    AuthHouseholdState.data()[0].subscriptionExpires = now
      .toCivilDate()
      .toJSON();
    expect(
      AuthHouseholdState.isExpired(seed.authHouseholds[0].id),
  ...
  test("isHidden", () => {
    AuthHouseholdState.data(seed.authHouseholds);
    AuthHouseholdState.data()[0].preferences.hideComponents = ["Bookmarks"];

    expect(AuthHouseholdState.isHidden("Bookmarks")).toBeFalsy();

    AuthHouseholdState.data()[1].preferences.hideComponents = ["Bookmarks"];

    expect(AuthHouseholdState.isHidden("Bookmarks")).toBeTruthy();
  ...
  test("updateHideComponents", async () => {
    ...
    await AuthHouseholdState.updateHideComponents(
