...
class SecretsValueManager extends DataArrayManager<SecretsValue> {
  ...
  constructor() {
    ...
    this.names = this.values.map((values) => {
      const names = Object.keys(values).map((id) => {
      ...
      async (values, vaults) => {
        ...
        for (let i = 0; i < values.length; i++) {
          ...
          v[values[i].id as string] = {
            authHouseholdID: SecretsVaultState.findID(values[i].secretsVaultID)
              .authHouseholdID,
  ...
  async encryptValue(s: SecretsValueDecrypted): Promise<SecretsValue | Err> {
    ...
    return {
      ...
      ...{
        dataEncrypted: [data.string(), ...s.dataEncrypted.slice(0, 4)],
        id: s.id,
  ...
  override findName(name: string, _personal?: boolean): SecretsValue {
    const i = this.data().findIndex((object) => {
      return (
        this.values()[`${object.id}`] !== undefined &&
        this.values()[`${object.id}`].name === name
      );
    });
