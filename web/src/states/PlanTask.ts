...
import { ObjectTaskCompleted, ObjectTaskCreated, ObjectTaskDeleted, ObjectTaskNextDate, ObjectTaskUpdated, WebGlobalActionUndo } from "../yaml8n";
...
class PlanTaskManager extends DataArrayManager<PlanTask> {
	...
	dueDatePast = this.nested.map((nested) => {
		...
		const filter = function (tasks: PlanTask[]): void {
			for (let i = 0; i < tasks.length; i++) {
				if (! tasks[i].template && tasks[i].dueDate !== null && Timestamp.fromString(tasks[i].dueDate as string)
					.toCivilDate() < today) {
					output.push(tasks[i]);
				}

				if (! tasks[i].template && tasks[i].children !== undefined && (tasks[i].children as PlanTask[]).length > 0) {
		...
		Sort(output, {
			formatter: (task: PlanTask): string => {
				if (task.dueDate !== null && Timestamp.fromString(task.dueDate)
					.toCivilTime()
					.toString(true) !== midnight) {
					return `${task.dueDate}`;
				}
				return "9999";
			},
	...
	household = this.nested.map((tasks) => {
		return tasks.filter((task) => {
			return task.authHouseholdID !== null && task.planProjectID === null && ! task.template;
		});
	});
	personal = this.nested.map((tasks) => {
		return tasks.filter((task) => {
			return task.authHouseholdID === null && task.authAccountID === AuthAccountState.data().id && task.planProjectID === null && ! task.template;
		});
	});
	...
	override async create (t: PlanTask, hideAlert?: boolean): Promise<PlanTask | Err> {
		let dueDateDiff = 0;

		if (t.shortID !== "" && t.dueDate !== null) {
			dueDateDiff = Timestamp.fromString(t.dueDate)
				.getDiffDays(Timestamp.fromString(`${PlanTaskState.findShortID(t.shortID).dueDate}`));
		}
		...
		return this.data()
			.filter((task) => {
				if (t.parentID !== null) {
					...
					return task.parentID === null && task.planProjectID === null && task.authHouseholdID !== null;
				}
	...
	findDueDate (date: NullCivilDate, nestedTasks?: PlanTask[]): PlanTask[] {
		return this.findDueDateTasks(date, nestedTasks === undefined ?
			this.nested() :
			nestedTasks);
	}
	...
	findDueDateTasks (date: NullCivilDate, children: PlanTask[]): PlanTask[] {
		...
		for (const task of children) {
			if (task.dueDate !== null && Timestamp.fromString(task.dueDate)
				?.toCivilDate()
				.toJSON() === date
				&& (task.authAccountID === null || task.authAccountID === AuthAccountState.data().id)) {
				tasks.push(task);
		...
		Sort(tasks, {
			formatter: (task: PlanTask): string => {
				if (task.dueDate !== null && Timestamp.fromString(task.dueDate)?.timestamp !== midnight.timestamp) {
					return `${task.dueDate}`;
				}
	...
	findInventoryItemID (inventoryItemID: NullUUID, tasks?: PlanTask[]): PlanTask[] {
		if (tasks === undefined) {
	...
	findPlanProjectID (planProjectID: NullUUID, nestedTasks?: PlanTask[]): PlanTask[] {
		...
		return this.templates()
			.filter((task) => {
				...
				case PlanTaskTemplateType.Account:
					return task.authAccountID === AuthAccountState.data().id && task.authHouseholdID === null;
	...
	getChildrenDone (task: PlanTask): childrenStatus {
		...
		for (const child of task.children) {
			if (child.done) {
				allDone += 1;
			} else if (this.getChildrenDone(child) > 0) {
				someDone += 1;
			}
		}
		...
		const today = tasks
			.filter((task) => {
				if (task.dueDate !== null) {
					return Timestamp.fromString(task.dueDate)
						.toCivilDate() <= CivilDate.now()
						&& ! task.done
						&& (task.authAccountID === null || task.authAccountID === AuthAccountState.data().id)
						&& ! task.template;
				}
	...
	override new (): PlanTask {
		...
		const p = Permission.isPermitted(AuthSessionState.data().permissionsHouseholds, PermissionComponentsEnum.Plan, PermissionEnum.Edit, AuthAccountState.data().primaryAuthHouseholdID);
				...
				if (! task.done && task.dueDate !== null) {
					...
					events.push({
						...
						...{
							...
							color: Colors.planTask(AuthHouseholdState.findID(task.authHouseholdID).preferences.colorPlanTaskEvents, task.color),
							details: task.planProjectID === null ?
								task.authHouseholdID === null ?
									`[icon@${Icons.PlanProject} Personal](/tasks?filter=personal)` :
									`[icon@${Icons.PlanProject} Household](/tasks?filter=household)` :
								`#planproject/${PlanProjectState.findID(task.planProjectID).shortID}${task.authHouseholdID === null ?
									"?p" :
									""}`,
							duration: d.toCivilTime()
								.toString(true) === "00:00" ?
								0 :
							...
							icon: "done_all",
							name: task.name,
							participants: task.authHouseholdID !== null && task.authAccountID !== null ?
								[
									task.authAccountID,
								] :
								[],
	...
	override async update (data: PlanTask, hideAlert?: boolean): Promise<void | Err> {
		const old = this.findID(data.id);

		if (old.dueDate !== null && data.dueDate !== null && old.dueDate !== data.dueDate && data.children !== undefined && data.children.length > 0) {
	...
	async updateDone (task: PlanTask, done?: boolean): Promise<void | Err> {
		...
		if (t.recurrence === null && done !== undefined) {
			...
			while (timestamp.toCivilDate()
				.valueOf() < tomorrow.toCivilDate()
				.valueOf()) {
				timestamp = Recurrence.nextTimestamp(t.recurrence, timestamp);
			}

			if (t.dateEnd !== undefined && t.dateEnd !== null && CivilDate.fromString(t.dateEnd) < timestamp.toCivilDate()) {
				t.done = true;
			} else {
				t.done = false;
		...
		return this.update(t, true)
			.then((err) => {
				...
				if (done === undefined) {
					if (t.recurrence !== null && t.recurrence.separation !== 0 && t.dueDate !== null) {
						AppState.setLayoutAppAlert({
							actions: [
								{
							...
							message: `${AuthAccountState.translate(ObjectTaskNextDate)} ${AppState.formatCivilDate(Timestamp.fromString(t.dueDate)
								.toCivilDate())}`,
						});
					} else {
						AppState.setLayoutAppAlert({
