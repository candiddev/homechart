...
import { ObjectAccount, ObjectAccounts, ObjectBookmarks, ObjectBudget, ObjectCalendar, ObjectCard, ObjectCards, ObjectCategories, ObjectCategory, ObjectChangeCreated, ObjectChangeDeleted, ObjectChangeUpdated, ObjectCollection, ObjectCollections, ObjectCook, ObjectEvent, ObjectEvents, ObjectHealth, ObjectInventory, ObjectItem, ObjectItems, ObjectList, ObjectLists, ObjectLog, ObjectLogs, ObjectMealPlan, ObjectMealPlans, ObjectMealTime, ObjectMealTimes, ObjectNote, ObjectNotes, ObjectPayee, ObjectPayees, ObjectPlan, ObjectProject, ObjectProjects, ObjectRecipe, ObjectRecipes, ObjectRecurringTransaction, ObjectRecurringTransactions, ObjectReward, ObjectSecrets, ObjectShop, ObjectTask, ObjectTasks, ObjectVault, ObjectVaults } from "../yaml8n";
...
class ChangeManager extends DataArrayManager<Change> {
	table = this.data.map((changes) => {
		...
		for (let i = 0; i < changes.length; i++) {
			...
			while (j < changes.length && changes[i].authAccountID === changes[j].authAccountID && changes[i].operation === changes[j].operation && changes[i].tableName === changes[j].tableName) {
