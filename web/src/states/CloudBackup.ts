...
export const CloudBackupState = {
  create: async (authHouseholdID: NullUUID): Promise<void | Err> => {
    return API.create(`/api/v1/cloud/${authHouseholdID}/backups`).then(
      (err) => {
        if (IsErr(err)) {
  ...
  delete: async (
    ...
    return API.delete(`/api/v1/cloud/${authHouseholdID}/backups/${id}`).then(
      (err) => {
        if (IsErr(err)) {
          AppState.setLayoutAppAlert(err);
  ...
  ): response is APIResponse<CloudBackup[]> {
  ...
  readAll: async (authHouseholdID: NullUUID): Promise<CloudBackup[] | Err> => {
    ...
    return API.read(`/api/v1/cloud/${authHouseholdID}/backups`, {}).then(
      (response) => {
        if (IsErr(response)) {
  ...
  restore: async (
    ...
    return API.read(`/api/v1/cloud/${authHouseholdID}/backups/${id}`, {}).then(
      (err) => {
        if (IsErr(err)) {
          AppState.setLayoutAppAlert(err);
