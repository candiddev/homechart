...
describe("GlobalState", () => {
	...
	test.each([
		...
		if (messageRegex === "") {
			expect(AppState.getLayoutAppAlerts())
				.toHaveLength(0);
		} else {
			expect(AppState.getLayoutAppAlerts()[0].message)
				.toMatch(messageRegex);
		}
	...
	test("permitted", () => {
		...
		expect(GlobalState.permitted(PermissionComponentsEnum.Auth, true))
			.toBeFalsy();
		...
		expect(GlobalState.permitted(PermissionComponentsEnum.Auth, true))
			.toBeTruthy();
		expect(GlobalState.permitted(PermissionComponentsEnum.Auth, true, seed.authHouseholds[0].id))
			.not.toBeTruthy();
		expect(GlobalState.permitted(PermissionComponentsEnum.Calendar, false))
			.toBeTruthy();
		expect(GlobalState.permitted(PermissionComponentsEnum.Calendar, true))
			.not.toBeTruthy();
		expect(GlobalState.permitted(PermissionComponentsEnum.Cook, true, seed.authHouseholds[0].id))
			.not.toBeTruthy();
	});
