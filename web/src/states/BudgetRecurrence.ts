...
import {
  ObjectRecurringTransactionCreated,
  ObjectRecurringTransactionDeleted,
  ObjectRecurringTransactionUpdated,
} from "../yaml8n";
...
class BudgetRecurrenceManager extends DataArrayManager<BudgetRecurrence> {
  ...
  findBudgetAccountIDToday(id: NullUUID): number {
    ...
    return transactions.filter((transaction) => {
      return (
        CivilDate.fromString(`${transaction.template.date}`) <=
        today.toCivilDate()
      );
    }).length;
  }
            ...
            events.push({
              ...
              ...{
                ...
                color: Colors.budgetRecurrrence(
                  AuthHouseholdState.findID(recurrence.authHouseholdID)
                    .preferences.colorBudgetRecurrenceEvents,
                ),
                details: `#budgetaccount/${
                  BudgetAccountState.findID(recurrence.budgetAccountID).shortID
                }`,
                duration: 0,
