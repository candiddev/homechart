...
import {
  ObjectAccountInvited,
  ObjectChildCreated,
  ObjectInviteDeleted,
  ObjectMemberDeleted,
  ObjectMemberUpdated,
  WebGlobalActionJoinHouseholdMessage,
  WebGlobalActionLeaveHouseholdMessage,
} from "../yaml8n";
...
export const AuthAccountAuthHouseholdState = {
  ...
  inviteAccept: async (token: string): Promise<AuthHousehold | Err> => {
    return API.read(
      `/api/v1/auth/households/${AuthAccountState.data().id}/invites/${token}`,
      {},
    ) // Household ID doesn't matter
      .then(async (err) => {
        if (IsErr(err)) {
          return err;
        }
        ...
        await AuthSessionState.validate();
        return AuthHouseholdState.read(
  ...
  inviteCreate: async (data: AuthAccountAuthHousehold): Promise<void | Err> => {
    return API.create(
      `/api/v1/auth/households/${data.authHouseholdID}/invites`,
      data,
    ).then((err) => {
      if (IsErr(err)) {
        return err;
  ...
  inviteDelete: async (
    ...
    return API.delete(
      `/api/v1/auth/households/${authHouseholdID}/invites/${id}`,
    )
      .then(async () => {
        AppState.setLayoutAppAlert({
  ...
  update: async (data: AuthAccountAuthHousehold): Promise<void> => {
    return API.update(
      `/api/v1/auth/households/${data.authHouseholdID}/members/${data.id}`,
      data,
    ).then(() => {
      AppState.setLayoutAppAlert({
