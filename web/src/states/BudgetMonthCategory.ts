...
export const BudgetMonthCategoryState = {
  create: async (category: BudgetMonthCategory): Promise<void | Err> => {
    return API.create("/api/v1/budget/month-categories", category).then(
      (err) => {
        if (IsErr(err)) {
  ...
  groupingSort: (b: BudgetMonth): BudgetMonthCategory[] => {
    ...
    if (
      b.budgetMonthCategories === null ||
      b.budgetMonthCategories.length === 0
    ) {
      b.budgetMonthCategories = BudgetCategoryState.data().map((category) => {
        return {
    ...
    for (const budget of b.budgetMonthCategories) {
      budget.targetAmount = BudgetMonthCategoryState.targetAmount(
        budget,
        b.yearMonth,
      );
      ...
      if (budget.budgetCategory.grouping === "") {
        ...
        budget.budgetCategory.grouping =
          AuthAccountState.translate(WebGlobalHidden);
        bhidden.push(budget);
      } else {
        let i = BudgetMonthCategoryState.getIndex(
          bnew,
          budget.budgetCategory.grouping,
        );
        if (i >= 0) {
          ...
          if (b.yearMonth === budget.yearMonth) {
            ...
            bnew[i - 1].budgetTransactionAmount =
              budget.budgetTransactionAmount;
          }
  ...
  targetAmount: (category: BudgetMonthCategory, y: number): number => {
    ...
    const month = parseInt(y.toString().slice(4, 6), 10);
    const year = parseInt(y.toString().slice(0, 4), 10);

    let m = 1;
    if (category.budgetCategory.targetAmount !== 0) {
      if (category.budgetCategory.targetMonth !== MonthEnum.None) {
    ...
    const amount =
      category.budgetCategory.targetAmount - category.balance - category.amount;

    if (
      m <= 0 ||
      (category.budgetCategory.targetMonth === MonthEnum.None &&
        category.budgetCategory.targetYear === 0)
    ) {
      return amount;
    }
  ...
  update: async (data: BudgetMonthCategory): Promise<void> => {
    return API.update("/api/v1/budget/month-categories", data).then(() => {
