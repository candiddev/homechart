...
import {
  ObjectNotificationCreated,
  ObjectNotificationDeleted,
  ObjectNotificationUpdated,
} from "../yaml8n";
import { AuthAccountState } from "./AuthAccount";

export interface Notification {
  // TODO: Make webpush work via Admin
...
export const NotificationState = {
  create: async (data: Notification): Promise<void> => {
    return API.create("/api/v1/notifications", data).then(() => {
  ...
  delete: async (id: NullUUID): Promise<void> => {
    return API.delete(`/api/v1/notifications/${id}`).then(() => {
      AppState.setLayoutAppAlert({
  ...
  ): response is APIResponse<Notification[]> => {
  ...
  read: async (): Promise<Notification[] | Err> => {
    return API.read("/api/v1/notifications", {}).then((response) => {
      if (IsErr(response)) {
        return response;
      }
  ...
  update: async (data: Notification): Promise<void> => {
    return API.update(`/api/v1/notifications/${data.id}`, data).then(() => {
      AppState.setLayoutAppAlert({
