...
import { ObjectSessionCreated, ObjectSessionDeleted, ObjectSessionUpdated } from "../yaml8n";
...
export class AuthSessionManager extends DataManager<AuthSession> {
	...
	async deleteAll (): Promise<void> {
		return API.delete("/api/v1/auth/sessions")
			.then(() => {});
	}

	// Don't log AuthSession failures
	override inResponse (response: APIResponse<unknown>): response is APIResponse<AuthSession[]> {
		if (
	...
	isArray (response: APIResponse<unknown>): response is APIResponse<AuthSession[]> {
	...
	async readAll (): Promise<AuthSession[] | Err> {
		return API.read("/api/v1/auth/sessions", {})
			.then(async (response) => {
				if (IsErr(response)) {
					return response;
				}
	...
	async toggleNotifications (): Promise<void | Err> {
		...
		if ("Notification" in window && process.env.NODE_ENV !== "test" && ! AuthAccountState.isDemo()) {
			if (session.webPush === null && InfoState.data().vapid !== "") {
				return Notification.requestPermission(async (permission: string) => {
					...
					return AppState.getSessionServiceWorkerRegistration()?.pushManager.subscribe({
					...
					.then(() => {})
					.catch((err) => {
						return NewErr(`AuthSession.toggleNotifications: error requesting permissions: ${err}`);
					});
	...
	async unsubscribeWebPush (): Promise<void> {
		return AppState.getSessionServiceWorkerRegistration()?.pushManager.getSubscription()
	...
	async validate (): Promise<void | Err> {
		...
		return API.read("/api/v1/auth/signin", {})
			.then(async (response) => {
				if (IsErr(response)) {
					return response;
				}
