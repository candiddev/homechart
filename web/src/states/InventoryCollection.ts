...
import { ObjectCollectionCreated, ObjectCollectionDeleted, ObjectCollectionUpdated } from "../yaml8n";
...
class InventoryCollectionManager extends DataArrayManager<InventoryCollection> {
	counts = Stream.lift((collections, items) => {
		return collections.reduce((c, d) => {
	...
	}, this.data, InventoryItemState.data);
	...
	constructor () {
		super("/api/v1/inventory/collections", "name", false, DataTypeEnum.InventoryCollection);
	}
