...
describe("Authccount", () => {
  ...
  test("data", () => {
    ...
    expect(AuthAccountState.keys()).toBe(seed.authAccounts[0].privateKeys);
    ...
    testing.mocks.responses = [
      {
        ...
        dataValue: [AuthAccountState.data()],
      },
    ];
  ...
  test("collapsePlanProject", async () => {
    ...
    expect(AuthAccountState.data().collapsedPlanProjects).toContain(id);
  });
  ...
  test("createAuthAccount", async () => {
    ...
    expect(account.privateKeys.length).toBe(0);
    expect(AuthSessionState.data().admin).toBeTruthy();

    account.password = "hello";

    await AuthAccountState.createAccount(account, "");

    expect(account.privateKeys.length).toBe(1);
  ...
  test("createSession", async () => {
    ...
    expect(AuthSessionState.data().id).toBe("1");
  });
  ...
  test("createTOTP", async () => {
    ...
    expect(IsErr(response) ? "" : response.totpQR).toBe("Not an image");
  });
  ...
  test("decryptPrivateKeys/loadPrivateKey/newPrivateKeyPBKDF2/savePrivateKey", async () => {
    ...
    AuthAccountState.data({
      ...
      ...{
        privateKeys: [
          {
            key: (await AuthAccountState.newPrivateKeyPBKDF2(
              "password",
            )) as string,
            name: "Testing",
          ...
          {
            key: (
              (await new Key(KeyTypeNone, "", "").encrypt(
    ...
    expect(AuthAccountState.privateKey()).toStrictEqual(keys.privateKey);
    ...
    expect(AuthAccountState.privateKey()).toStrictEqual(new Key("", "", ""));
  });
  ...
  test("findNotificationsHousehold", () => {
    ...
    expect(
      AuthAccountState.findNotificationsHousehold(
        AuthAccountState.data().preferences.notificationsHouseholds,
        "1",
      ),
    ).toStrictEqual(
      AuthAccountState.data().preferences.notificationsHouseholds![0],
    );
  });
  ...
  test("isDemo", () => {
    expect(AuthAccountState.isDemo()).toBeFalsy();
  });
  ...
  test("newPrivatePublicKey", async () => {
    ...
    expect(a.publicKey).not.toBe(seed.authAccounts[0].privateKeys);
    expect(a.privateKeys).not.toBe(seed.authAccounts[0].publicKey);

    AuthAccountState.privateKey(new Key("", "", ""));
    AuthAccountState.data(a);
    await AuthAccountState.decryptPrivateKeys("Password");

    expect(AuthAccountState.privateKey()).toStrictEqual(key);
  });
  ...
  test("readAll", async () => {
    ...
    expect(IsErr(response) ? [] : response.dataValue).toHaveLength(3);
  });
  ...
  test("set", async () => {
    ...
    expect(AuthAccountState.data()).toStrictEqual({
      ...AuthAccountState.new(),
      ...{
        preferences: {
  ...
  test("setPrivateKey", async () => {
    ...
    expect(
      (testing.mocks.requests[0].body as AuthAccount).privateKeys[1],
    ).toStrictEqual(key);
  });
  ...
  test("updateICalendarID", async () => {
    ...
    expect(AuthAccountState.data().icalendarID).toBe("1");
  });
