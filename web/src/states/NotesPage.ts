...
import { ObjectHousehold, ObjectNoteCreated, ObjectNoteDeleted, ObjectNoteUpdated, WebGlobalPersonal } from "../yaml8n";
...
class NotesPageManager extends DataArrayManager<NotesPage> {
	...
	personal = this.nested.map((pages) => {
		return pages.filter((page) => {
			return page.authHouseholdID === null && page.deleted === null && page.authAccountID === AuthAccountState.data().id;
		});
	});
	...
	findNamesChildren (level: number, children?: NotesPage[]): FormItemSelectNestedSelector[] {
		...
		if (children !== undefined) {
			for (const page of children) {
				...
				if (Array.isArray(page.children) && page.children.length !== 0) {
					names = names.concat(this.findNamesChildren(level + 1, page.children));
				}
			}
		}
		return names;
	...
	getColorNamesIDs (): FormItemSelectNestedSelector[] {
		...
		names = names.concat(this.findNamesChildren(1, FilterSortChildren({
			input: this.data()
				.filter((page) => {
					return page.deleted === null && page.authAccountID === AuthAccountState.data().id;
				}),
		})));

		if (Permission.isPermitted(AuthSessionState.data().permissionsHouseholds, PermissionComponentsEnum.Notes, PermissionEnum.Edit)) {
			names.push({
				id: "household",
				level: 0,
