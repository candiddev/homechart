...
import {
  ...
  ObjectRecipeDeleted,
  ObjectRecipeUpdated,
  WebGlobalActionImportRecipeFailure,
  WebGlobalActionImportRecipeSuccess,
} from "../yaml8n";
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (!lines[i].startsWith("#")) {
        for (const item of InventoryItemState.data()) {
          if (
            item.quantity > 0 &&
            lines[i].toLowerCase().includes(item.name.toLowerCase())
          ) {
  ...
  async importURL(url: string, authHouseholdID: NullUUID): Promise<void | Err> {
    return API.create("/api/v1/import/recipe", {
      ...
      if (this.containsResponse(response)) {
        ...
        m.route.set(
          `/cook/recipes/${response.dataValue[0].id}`,
          {},
          {
            state: {
              key: Date.now(),
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      if (isNumber.test(words[i]) || isFraction.test(words[i])) {
        if (isFraction.test(words[i + 1])) {
          ...
          newLine += new fraction(words[i]).mul(scale).toFraction(true);
        }
      } else {
        newLine += words[i];
      }
