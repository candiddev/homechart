...
import { ObjectCardCreated, ObjectCardDeleted, ObjectCardUpdated } from "../yaml8n";
...
class RewardCardManager extends DataArrayManager<RewardCard> {
	...
	findSent (cards: RewardCard[], id?: NullUUID): RewardCard[] {
		return cards.filter((card) => {
			return card.senders.includes(`${AuthAccountState.data().id}`) && (id === undefined || id !== undefined && card.recipients.includes(`${AuthHouseholdState.findMember(`${id}`).id}`));
		});
	}
	...
	getInProgress (cards?: RewardCard[]): RewardCard[] {
		...
		return cards.filter((card) => {
			return (! card.senders.includes(`${AuthAccountState.data().id}`) || card.recipients.length === 0) && !card.invert && card.stampCount < card.stampGoal;
		});
	}
	...
	getRedeemable (cards?: RewardCard[]): RewardCard[] {
		...
		return cards.filter((card) => {
			return ! card.senders.includes(`${AuthAccountState.data().id}`) && (card.invert && card.stampCount < card.stampGoal || !card.invert && card.stampCount === card.stampGoal);
		});
	}
