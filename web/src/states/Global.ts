...
import { ISO639Codes, ObjectChange, Translate, WebGlobalWrongLocale, WebGlobalWrongTimeZone1, WebGlobalWrongTimeZone2, WebGlobalWrongVersion } from "../yaml8n";
...
export const GlobalState = {
	alertSubscription: (): void => {
		if (! AuthAccountState.isDemo() && ! AuthAccountState.data().child) {
			for (const household of AuthHouseholdState.data()) {
				if (household.subscriptionProcessor === 0) {
					if (m.route.get()
						.startsWith("/setup") || AppState.getSessionRedirects().length === 1 && AppState.getSessionRedirects()[0].startsWith("/setup")) {
						return;
					}

					const expires = CivilDate.fromString(household.subscriptionExpires as string);
					const today = Timestamp.now();
					const soon = Timestamp.now();
					soon.addDays(5);

					if (! InfoState.data().cloud && household.id === null && AuthAccountState.data().setup) {
	...
	hideComponentIncludes (component: string): boolean {
		...
		return AuthHouseholdState.isHidden(name) || AuthAccountState.data().preferences.hideComponents.includes(name);
	},
	init: (): void => {
		IndexedDB.init();
		Controller.init(GlobalState.handleResponse);
	},
	permitted: (component: PermissionComponentsEnum, edit?: boolean, authHouseholdID?: NullUUID): boolean => {
		let permission = PermissionEnum.View;

		if (! AppState.isSessionAuthenticated() || edit === true && authHouseholdID !== undefined && authHouseholdID !== null && component !== PermissionComponentsEnum.Auth && ! AuthAccountState.isDemo() && AuthHouseholdState.isExpired(authHouseholdID)) {
			return false;
		}
		...
		case PermissionComponentsEnum.Reward:
			return Permission.isPermitted(AuthSessionState.data().permissionsHouseholds, component, permission);
		...
		case PermissionComponentsEnum.Shop:
			...
			case true:
				return AuthSessionState.data().permissionsAccount[Permission.components[component]] <= permission;
			case false:
				return Permission.isPermitted(AuthSessionState.data().permissionsHouseholds, component, permission, authHouseholdID);
			}
		}
	},
	readAll: (): void => {
		if (process.env.NODE_ENV !== "test") {
			for (let i = 0 as DataTypeEnum; i < Object.keys(DataTypeEnum).length; i++) {
				switch (i) {
				case DataTypeEnum.AuthAccount:
				case DataTypeEnum.AuthAccountAuthHousehold:
				case DataTypeEnum.AuthSession:
		...
		return AuthSessionState.validate()
			...
			.then(async (err) => {
				...
				if (AuthAccountState.data().setup) {
					...
					if (ISO639Codes[iso639] !== undefined && AuthAccountState.data().iso639Code !== iso639) {
				...
				if ((window as any).dataLayer === undefined) { // eslint-disable-line @typescript-eslint/no-explicit-any
				...
				if (AuthSessionState.data().webPush === null) {
					await AuthSessionState.toggleNotifications();
				}

				if (AppState.getSessionDisplay() === DisplayEnum.XLarge && m.route.get() !== undefined && ! m.route.get()
					.includes("/setup")) {
			...
			.catch(async (err) => {
				...
				NewErr(`Global.Signin: error during signin: ${JSON.stringify(err)}`, "There was a problem signing you in.");

				if (m.route.get()
					.includes("/reset") || m.route.get()
					.includes("/signin") || m.route.get()
					.includes("/signup") || m.route.get()
					.includes("/about") || m.route.get()
					.includes("/cook/recipes/")) {
					return;
				}

				if (!AppState.isSessionOnline() && AuthSessionState.data().id !== null) {
					AppState.setSessionAuthenticated(true);
					AppState.redirect();
					return;
				}
		...
		return IndexedDB.clear()
			.then(() => {
				...
				if (noRedirect !== true) {
					...
					if (m.route.get() !== "/signin" && m.route.get() !== "/signout" && m.route.get() !== undefined) {
