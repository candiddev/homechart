    ...
    return this.data.map((calendarEvents) => {
      const allEvents = [
        ...calendarEvents.filter((event) => {
          return (
            event.calendarICalendarID === null ||
            !AuthAccountState.data().hideCalendarICalendars.includes(
              event.calendarICalendarID,
            )
          );
        }),
      ];
      ...
      if (AuthAccountState.data().preferences.showCalendarEventAstronomy) {
        ...
        for (const event of events) {
          const timestamp = Timestamp.fromString(
            `${event.date}T${
              event.time === undefined ? "00:00" : event.time
            }:00Z`,
          ).toString();
          ...
          allEvents.push({
            ...
            ...{
              ...
              details: event.details === undefined ? "" : event.details,
              duration: 0,
      ...
      if (AuthAccountState.data().preferences.showCalendarEventHolidaysUS) {
        ...
        for (const holiday of HolidaysUS) {
          ...
          allEvents.push({
            ...
            ...{
              authAccountID: AuthAccountState.data().id,
              details: holiday.details === undefined ? "" : holiday.details,
              duration: 0,
              icon: Icons.CalendarHoliday,
              name: holiday.name,
              recurrence: {
  ...
  toCalendarEventsRange(
    ...
    for (const event of allEvents) {
      ...
      if (event.recurrence === null) {
        if (
          start.toCivilDate().valueOf() <= to.valueOf() &&
          end.toCivilDate().valueOf() >= from.valueOf()
        ) {
        ...
        for (let i = 0; i <= diff; i++) {
          ...
          for (const date of rec) {
            ...
            if (!events[date].includes(event)) {
              const tStart = Timestamp.fromString(
                event.timestampStart as string,
              );
