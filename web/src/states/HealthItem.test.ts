...
describe("HealthItemState", () => {
	...
	test("findAuthAccountID", () => {
		...
		expect(HealthItemState.findAuthAccountID("1"))
			.toHaveLength(3);
		expect(HealthItemState.findAuthAccountID("1", false))
			.toHaveLength(1);
		expect(HealthItemState.findAuthAccountID("1", true))
			.toHaveLength(2);
	});
	...
	test("findAuthAccountIDName", () => {
		...
		expect(HealthItemState.findAuthAccountIDName("1", "a"))
			.toStrictEqual(data[0]);
		expect(HealthItemState.findAuthAccountIDName("2", "a"))
			.toStrictEqual(data[3]);
	});
