...
describe("SecretsVault", () => {
  ...
  test("keys", async () => {
    expect(Object.keys(SecretsVaultState.keys())).toHaveLength(0);
    ...
    expect(Object.keys(SecretsVaultState.keys())).toHaveLength(2);
  });
    ...
    testing.mocks.responses = [
      {
        ...
        dataValue: [seed.secretsVaults[1]],
      },
    ];

    await SecretsVaultState.deleteKey(
    ...
    testing.requests([
      {
        body: {
          ...seed.secretsVaults[1],
          ...{
            keys: [seed.secretsVaults[1].keys[1]],
          },
        },
        method: "PUT",
  ...
  test("setKey", async () => {
    ...
    await SecretsVaultState.setKey(
      seed.secretsVaults[0],
      seed.authAccounts[2].id,
    );

    expect((testing.mocks.requests[0].body as any).keys[1].authAccountID).toBe(
      seed.authAccounts[2].id,
    );
  });
});
