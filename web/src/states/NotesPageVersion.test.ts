...
describe("NotesPageVersion", () => {
	test("findNotesPageIDAll", () => {
		expect(NotesPageVersionState.findNotesPageIDAll(seed.notesPages[0].id))
			.toStrictEqual([
				seed.notesPageVersions[1],
				seed.notesPageVersions[0],
			]);
	});

	test("findNotesPageIDLatest", () => {
		expect(NotesPageVersionState.findNotesPageIDLatest(seed.notesPages[0].id))
