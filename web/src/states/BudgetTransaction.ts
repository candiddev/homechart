...
import {
  ObjectTransactionCreated,
  ObjectTransactionDeleted,
  ObjectTransactionUpdated,
} from "../yaml8n";
...
export const BudgetTransactionState = {
  ...
  create: async (budgetTransaction: BudgetTransaction): Promise<void | Err> => {
    return API.create("/api/v1/budget/transactions", budgetTransaction).then(
      (err) => {
        if (IsErr(err)) {
  ...
  delete: async (id: NullUUID): Promise<void> => {
    return API.delete(`/api/v1/budget/transactions/${id}`).then(() => {
      AppState.setLayoutAppAlert({
  ...
  ): response is APIResponse<BudgetTransaction[]> {
  ...
  lastDate: CivilDate.now().toJSON(),
  load: async (): Promise<BudgetTransaction[]> => {
    return IndexedDB.get("BudgetTransactions").then(async (data) => {
  ...
  read: async (noLoad?: boolean): Promise<void | Err> => {
    ...
    if (m.route.param().account !== undefined) {
      ...
      if (m.route.param().month === undefined) {
        err = await BudgetTransactionState.readCategory(
          m.route.param().category,
        );
      } else {
        err = await BudgetTransactionState.readCategoryMonth(
          m.route.param().category,
          parseInt(m.route.param().month, 10),
        );
      }
  ...
  readAccount: async (budgetAccountID: NullUUID): Promise<void | Err> => {
  ...
  readCategory: async (budgetCategoryID: NullUUID): Promise<void | Err> => {
    return API.read(
      `/api/v1/budget/categories/${budgetCategoryID}/transactions`,
      {
        offset: BudgetTransactionState.offset,
      },
    ).then((response) => {
  ...
  readCategoryMonth: async (
    ...
    return API.read(
      `/api/v1/budget/categories/${budgetCategoryID}/transactions/${yearMonth}`,
      {
        offset: BudgetTransactionState.offset,
      },
    ).then((response) => {
  ...
  readPayee: async (budgetPayeeID: NullUUID): Promise<void | Err> => {
  ...
  update: async (data: BudgetTransaction): Promise<void | Err> => {
    return API.update(`/api/v1/budget/transactions/${data.id}`, data).then(
      (err) => {
        if (IsErr(err)) {
          AppState.setLayoutAppAlert(err);
