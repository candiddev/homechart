...
describe("DataArrayManager", () => {
	...
	beforeAll(async () => {
		Controller.init((res) => {
			...
			const d = res.data as WorkerResponseReadAll["data"] as ReadAllResponse<any>;
	...
	beforeEach(async () => {
		data = new DataArrayManager<Data>("/api/v1/data", "id", false, DataTypeEnum.Data);
	});
	...
	test("findBatch", () => {
		...
		expect(data.findBatch("1"))
			.toStrictEqual(data.batch[0]);
		expect(data.findBatch("2"))
			.toStrictEqual(data.new());
	});
	...
	test("read", async () => {
		...
		expect(await data.read("1"))
			.toBeUndefined();

		await testing.sleep(100);

		expect(AppState.isSessionOnline())
			.toBeFalsy();
		...
		expect(await data.read("1", undefined, true))
			.toStrictEqual(d);
	});
	...
	test("getTagNames/findTag/setTags", () => {
		...
		expect(data.findTag("a"))
			.toStrictEqual([
				data.data()[0],
			]);
		expect(data.findTag("b"))
			.toStrictEqual([
				data.data()[0],
				data.data()[1],
			]);
